...
const buildLayersFromTree = async (node: DirNode, resourcesByDir: Map<string, { drones: string[]; deps: string[] }>, out: Map<string, string>): Promise<string> => {
...
const buildNamespaceDependency = async (namespaceRelDir: string, directMemberFiles: SourceFile[], allNamespaceSpecifiers: string[]): Promise<{ sig: string; bytes: Uint8Array; namespaceSpecifier: string }> => {
...
const writeDevNameManifest = (imports: Record<string, string>): void => {
...
const main = async (): Promise<void> => {
  ...
  writeDevNameManifest(devImports)
