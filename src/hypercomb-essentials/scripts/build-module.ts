...
const main = async (): Promise<void> => {
  ...
  const rootDependencies = uniqSorted(Array.from(dependencyBytes.keys()))
  ...
  for (const domain of devDomains) {
    ...
    for (const [nsRelDir, depSig] of nsToSig) {
      ...
      devImports[specifierFromNamespaceRelDir(nsRelDir)] =
        `/dev/${domain}/__dependencies__/${depSig}`
    }
  }

  const devResourcesByDomain: Record<string, string[]> = {}
  for (const domain of devDomains) {
    devResourcesByDomain[domain] = Array.from(devDomainRes.get(domain) ?? []).sort()
  }
