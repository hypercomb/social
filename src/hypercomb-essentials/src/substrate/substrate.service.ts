// substrate/substrate.service.ts
//
...
export class SubstrateService extends EventTarget {
  ...
  // ───────────── availability (the hidden set) ─────────────
  //
  // View and flip which images in the current pool are available for
  // assignment. Never written to the registry and never to a layer, so it is
  // invisible to peers — but it IS written to localStorage, because a picture
  // you have taken out of the rotation is a decision, not a mood. It used to
  // live only in memory and came back on every reload, which made "hide the
  // ones I don't want" a thing you had to do again every morning; the same
  // rule the colour theme and the screen backdrop keep (participant-local,
  // persisted, never in a layer) is the right one here too.
  ...
  unpinImages(): void { this.#disabledImages.clear(); this.#persistHidden() }
  ...
  /** Hide or show ONE image, said outright rather than flipped. A surface that
   *  draws a row of eyes knows which state it is asking for; a toggle makes it
   *  guess, and two clicks racing each other land on the wrong one. */
  setImageHidden(signature: string, hidden: boolean): void {
    const sig = signature.trim().toLowerCase()
    if (!sig) return
    if (hidden === this.#disabledImages.has(sig)) return
    if (hidden) this.#disabledImages.add(sig)
    else this.#disabledImages.delete(sig)
    this.#persistHidden()
    EffectBus.emit('substrate:changed', { scope: 'hidden', signature: sig })
  }

  /** Everything back in the rotation. */
  showAllImages(): void {
    if (this.#disabledImages.size === 0) return
    this.#disabledImages.clear()
    this.#persistHidden()
    EffectBus.emit('substrate:changed', { scope: 'hidden' })
  }

  /** The signatures currently out of the rotation. */
