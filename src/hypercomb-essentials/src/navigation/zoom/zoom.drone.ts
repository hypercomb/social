...
export class ZoomDrone extends Drone {
  ...
  protected override heartbeat = async (): Promise<void> => {
    ...
    this.onEffect<{ count: number }>('render:cell-count', ({ count }) => {
      ...
      // A framed page is always fitted to its frame, so every render is a
      // chance to notice that the frame, or the canvas it has to fit into,
      // changed. #refitFramed is the one that decides whether anything
      // actually needs to move — see the key it compares.
      this.#refitFramed()
    })

    // A frame was bound or released. The frame decides the framing as well as
    // the arrangement — "fit to the window, three rows tall" is half of what
    // the participant asked for — so re-fit once the placement has landed.
    // 'auto' deliberately: the participant framed the branch, they did not
    // hand-frame this viewport, and marking it manual would exclude the page
    // from every later fit.
  ...
  public zoomToScale = (scale: number, pivotClient: Pt, source: ViewportSource = 'user'): void => {
    ...
    // A framed page owns its own scale: the pattern is fit to the window and
    // three rows tall, and that IS the frame. Automatic callers still pass
    // through — the fit below is one of them.
    if (source === 'user' && viewportIsFramed()) return
  ...
  /** What the last framed fit was computed for: the frame's identity and the
   *  canvas it had to fill. A framed page fits a FIXED rectangle, so re-fitting
   *  on every render would animate to the scale it is already at — a stutter
   *  once per paint. Refit only when this key moves. */
  #framedFitKey = ''

  /** Fit a framed page to its frame, if it is framed and something changed.
   *  'auto' deliberately: the participant framed the branch, they did not
   *  hand-frame this viewport, and marking it manual would exclude the page
   *  from every later fit. */
  #refitFramed = (): void => {
    ...
    const contentLayer = this.renderContainer ? this.#findContentLayer(this.renderContainer) : null
    const measured = (contentLayer ?? this.renderContainer)?.getLocalBounds()
    const bounds = measured ? expandToFrame(measured) : null
    if (!bounds) return
  ...
  public zoomToFit = (
    ...
    // A FRAMED page fits its FRAME, not its contents. The slots are a fixed
    // window onto the pattern, so a half-full frame must fit the same
    // rectangle a full one does — otherwise the tiles would resize every time
    // one was added, which is the opposite of what a frame is for.
    const measured = (contentLayer ?? target).getLocalBounds()
    if (!measured || measured.width <= 0 || measured.height <= 0) return false
    const bounds = expandToFrame(measured) ?? measured
