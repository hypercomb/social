// input/pan/spacebar-pan.input.ts
import type { InputGate } from '../input-gate.service.js'
import { framedHere, scrollFramedBy, stepWidthOnScreen } from '../../sequence/frame-lock.js'
...
export class SpacebarPanInput {
  ...
  private onMove = (e: MouseEvent): void => {
    ...
    // On a FRAMED page the same gesture means something else: the canvas is
    // fixed, so the drag walks the TILES through the frame instead of walking
    // the camera over them. Drag right and the tiles follow your hand — the
    // ones that were waiting off the left edge arrive, the ones at the right
    // edge leave. Vertical movement is ignored: the frame's step is a
    // horizontal translation, so there is nothing up or down to travel to.
    if (this.framedScroll(delta.x)) return

    this.pan.panBy(delta)
  }

  // -------------------------------------------------
  // framed drag — the tiles travel, not the canvas
  // -------------------------------------------------

  /** Leftover drag distance not yet worth a whole step. Kept so a slow drag
   *  accumulates into a step rather than being rounded away every mousemove. */
  private frameScrollAccum = 0

  private framedScroll = (dx: number): boolean => {
    const page = framedHere()
    if (!page) return false
    if (!this.canvas) return true            // framed: never fall through to pan

    const stepWidth = stepWidthOnScreen(page, this.canvas.getBoundingClientRect().width)
    if (!(stepWidth > 0)) return true

    this.frameScrollAccum += dx
    // Dragging RIGHT pulls earlier tiles back into view, so the offset goes
    // DOWN — the tiles move with the hand, which is the whole feel of it.
    while (Math.abs(this.frameScrollAccum) >= stepWidth) {
      const direction = this.frameScrollAccum > 0 ? 1 : -1
      scrollFramedBy(page, -direction)
      this.frameScrollAccum -= direction * stepWidth
    }
    return true
  }
