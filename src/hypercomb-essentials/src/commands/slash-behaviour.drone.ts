...
const wrapQueen = (queen: ReturnType<typeof isQueen> extends true ? never : any): SlashBehaviourProvider => ({
  ...
  execute(_behaviourName: string, args: string): Promise<void> | void {
    ...
    // where the two meet, and it refuses to touch a dot it cannot account for.
    return queen.invoke(dottedToSpaced(queen, args))
  },
...
// ── DOT SYNTAX FOR EVERY BEEHAVIOUR ───────────────────────────────────
//
// `/background` was the first citizen of the command-object protocol and for a
// while the only one, which made the dot a feature of ONE behaviour rather
// than of the language. Everything else still answered a flat string it split
// on spaces, so the same question — WHAT CAN COME NEXT HERE? — was answered
// one way in one place and its own way everywhere else.
//
...
// documents). That IS membership, already written down. So a behaviour gets a
// command object derived from what it already says, and walks with dots for
// free — no per-behaviour edit, and none possible to forget.
//
...
//
// It is derived only for queens this file also wraps, and deliberately so:
...
// own space-splitting parser cannot read, which is a dropdown that writes a
// line the behaviour then rejects.

/** The one word that follows `path` in `offer`, or null when `offer` is not
 *  about this path at all. */
const segmentAfter = (path: readonly string[], offer: string): string | null => {
  const tokens = offer.toLowerCase().split(/[.\s]+/).filter(Boolean)
  if (tokens.length <= path.length) return null
  for (let i = 0; i < path.length; i++) if (tokens[i] !== path[i]) return null
  return tokens[path.length] ?? null
}

/** The words a behaviour offers after `path`, from what it already says. */
const derivedMembers = (queen: any, path: readonly string[]): CommandMember[] => {
  const out = new Map<string, CommandMember>()

  // Its own completer first: it answers IN CONTEXT, which `options` cannot.
  if (typeof queen.slashComplete === 'function') {
    // A trailing separator is what says "this segment is complete" to every
    // completer in the codebase, this derived walk included.
    const asked = path.length ? path.join(' ') + ' ' : ''
    let offered: readonly string[] = []
    try { offered = queen.slashComplete(asked) ?? [] } catch { offered = [] }
    for (const offer of offered) {
      const next = segmentAfter(path, String(offer))
      if (next && !out.has(next)) out.set(next, { name: next })
    }
  }

  // The documented forms fill in for a behaviour with no completer at all.
  if (Array.isArray(queen.options)) {
    for (const form of queen.options as readonly string[]) {
      const next = segmentAfter(path, String(form))
      // `<name>` is a placeholder for something the participant supplies, not
      // a word they can pick — offering it would type the brackets into the
      // line.
      if (!next || next.startsWith('<') || out.has(next)) continue
      out.set(next, { name: next, description: form })
    }
  }
  return [...out.values()]
}
...
/**
 * Dots, turned back into the spaces a behaviour's own parser expects — but
 * ONLY when every segment is a word that behaviour actually offers.
 *
 * This is the half that makes the walk real rather than decorative: the
 * dropdown can complete `substrate.set.steel` all day, and if the string handed
 * to the behaviour still has dots in it the command fails on arrival. It
 * cannot simply replace every dot, because a dot is load-bearing in the
 * arguments people actually pass — `susan.hypercomb.com`, `1.5`, `notes.md`.
 * So the segments are CHECKED against the walk, and a string with even one
...
const dottedToSpaced = (queen: any, args: string): string => {
  ...
  for (const segment of segments) {
    ...
    // An empty or spaced segment is prose with a full stop in it, not a walk.
    if (!word || /\s/.test(word)) return args
    if (!derivedMembers(queen, walked).some(member => member.name === word)) return args
    walked.push(word)
  }
  return walked.join(' ')
}
