// commands/decoration-manifest.ts
//
...
export async function writeDecoration<TPayload>(opts: {
  ...
  segments: readonly string[]
  mark?: 'persistent'
  /** EXTRA closure the payload cannot be walked for.
   *
   *  `collectSigsDeep` finds every signature nested in the PAYLOAD, which is
   *  the whole answer while a record's dependencies are written into it. It is
   *  not the answer when the payload holds one signature and the thing behind
   *  it is a TREE of further records — a layout arrangement, where the mark
   *  carries only the root and every level below it lives in its own resource.
   *  The push walk is single-level, so anything the closure does not name
   *  never travels, and the adopter gets the mark plus one orphan.
   *
   *  So a caller that knows its own deep closure declares it here, and it is
   *  merged with what the payload yields. Same capability the bridge path
   *  already uses for a website page's embedded assets. */
  ...
  const declared = (opts.refs ?? [])
    .map(sig => String(sig ?? '').toLowerCase())
    .filter(sig => /^[0-9a-f]{64}$/.test(sig))
  const refs = opts.kind === 'group'
    ? []
    : [...new Set([...collectSigsDeep(opts.payload), ...declared])]
...
export async function replaceDecoration<TPayload>(opts: {
  ...
  segments: readonly string[]
  mark?: 'persistent'
  /** See `writeDecoration` — the deep closure a payload cannot be walked for. */
  refs?: readonly string[]
}): Promise<string> {
