...
// `measure` — are data, so a seventh costs no code. `single` ANCHORS one part
// in the panel a container already has; the rest DIVIDE it. The set stops at
// three holes, the fourth being a nesting. Each is drawn ONE way and turned
...
import { findTemplate, knownTemplates, rememberTemplate, savedTemplates, targetTemplate } from './template-catalog.js'
