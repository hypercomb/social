...
//
// THE LANDING. A published site opened onto a black cover for seconds while
// the visitor's browser installed and evaluated the engine ("they seem
// broken when you get there" — jwize, 2026-09-24). The publisher has the
// creation on screen at the moment they publish, so the publish takes a
// LANDING of it:
//
//   - hexagons on screen  → a PICTURE: the Pixi canvas composited with the
//     DOM tile names (tile-name.drone.ts draws names as spans over the
//     canvas — the canvas alone is a hive with no words), WebP.
//   - a takeover view on screen (body.hc-view-covered, a website page) is
//     DOM the canvas never held, so its landing is the PAGE ITSELF: the
//     mounted cell page's bytes with their `resource:` refs rewritten to
//     heap URLs, exactly as the site view mounts them.
//
// The bytes become an ordinary resource, ride the same drain as the
// closure, and the signed hive index names them under `landing[lineageKey]`
// (hive-pointer.ts). The door serves that landing INSIDE the loading cover,
// so the visitor sees the site at HTML-parse time and the engine hydrates
// behind it. A derived record, never truth: nothing reads it back but the
// cover, and a publish that cannot take one is still a publish.
 ...
 *  landing would show what the visitor lands on. */
...
export async function captureLanding(): Promise<Landing | null> {
  try {
    ...
    const ratio = Math.min(1, MAX_EDGE / Math.max(canvasRect.width, canvasRect.height))
