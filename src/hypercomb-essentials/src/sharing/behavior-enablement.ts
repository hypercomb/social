// sharing/behavior-enablement.ts
//
...
/** ADOPT A PUBLICATION'S LIGHTS — the visitor's answer to "what should be on
 *  here", taken from the publisher instead of guessed.
 *
 *  A read-only visit is a brand-new install every time (the memory
 *  filesystem guarantees it), with no roster to opt in from and no history to
 *  infer from. Both guesses are wrong: nothing lit renders a published
 *  creation as shaded hexagons wearing default art, and everything lit shows
 *  behaviours the publisher never turned on. So the publication carries its
 *  own answer — `publish:lights` on the branch root, sealed into the closure
 *  (commands/publish-lights.ts) — and this writes it down.
 *
 *  Wholesale, not a merge: the list IS the publisher's arrangement, and a
 *  union with whatever a previous visit to a DIFFERENT site left behind on
 *  this origin would show neither site as it was published.
 *
 *  Stamps the cohort ledger `'*'` in the same gesture, for the same reason
 *  the dark start does: nothing may light itself afterwards behind a reader
 *  who never chose anything. Returns true when it actually changed something.
 *
 *  Callers: the visit path ONLY. A participant's own hive must never have its
 *  lights replaced by a branch it happens to be looking at. */
export function adoptPublishedLights(kinds: readonly string[]): boolean {
  wire()
  const list = [...new Set(kinds.map(k => String(k ?? '').trim()).filter(Boolean))]
  const before = readGlobalOnKinds()
  if (before && before.size === list.length && list.every(k => before.has(k))) return false
  try {
    localStorage.setItem(GLOBAL_ON_KEY, JSON.stringify(list))
    localStorage.setItem(SEEDED_COHORTS_KEY, JSON.stringify(['*']))
    // The off-list mirror is the swarm's withheld wire; keep it consistent
    // with the one switch rather than leaving a stale contradiction.
    localStorage.setItem(GLOBAL_OFF_KEY, JSON.stringify(
      readStringArray(GLOBAL_OFF_KEY).filter(k => !list.includes(k))))
  } catch { return false }
  dropCaches()
  EffectBus.emit(ENABLEMENT_CHANGED, { adopted: list.length })
  return true
}
