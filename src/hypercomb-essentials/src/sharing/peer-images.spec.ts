...
describe('peer image candidates', () => {
  ...
  it('opens the root pool and retains live-lineage compatibility witnesses', async () => {
    const calls: unknown[] = []
    ...
    expect(calls).toEqual([[], { sig: 'f'.repeat(64), force: true }])
    ...
    expect(found[1].peers).toEqual([{ pubkey: 'lineage-peer', label: '' }])
  })
