// atomizer-sidebar.i18n.spec.ts — the drift check the catalog split owes.
//
...
const EXPECTED = [
  'atomizer.close',
  'atomizer.reset',
  'atomizer.spacing-placeholder',
].sort()

describe('atomizer-sidebar catalog', () => {

  it('carries en — the fallback every other locale leans on', () => {
    expect(Object.keys(ATOMIZER_SIDEBAR_TRANSLATIONS)).toContain('en')
  })

  it('carries exactly the surface’s key set', () => {
    expect(Object.keys(ATOMIZER_SIDEBAR_TRANSLATIONS['en']).sort()).toEqual(EXPECTED)
  })
  ...
  it('every key belongs to this surface', () => {
    ...
    for (const catalog of Object.values(ATOMIZER_SIDEBAR_TRANSLATIONS)) {
