// diamondcoreprocessor.com/sharing/publish-branch.ts
//
...
export async function publishBranch(
  ...
  // 2. A merkle-coherent root from LIVE heads; heal once, retry, else fail
  ...
  // 3. Stage the sealed closure and start pushing.
  ...
  // 4. THE AVAILABILITY GATE — the index only ever names a served head.
  ...
  // 5. Merge + sign + PUT the index — behind the wipe guard.
  ...
  const read = await fetchHiveIndex(indexHost, pubkey)
  ...
  // 6. The stable bearer link: segments + pubkey + hosts (+ the sealed head
  ...
  // 7. Write the ledger record BEFORE confirming. The PUT already happened —
  ...
  // 8. Confirm: the world must be able to READ what we just wrote.
