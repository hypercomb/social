 ...
 *  formatting.
 *
 *  The canonical builder now lives in core because the Organizer is in shared
 *  and shared may not import essentials. These wrappers preserve the older
 *  exported call surface for existing callers while canonicalizing every route
 *  to its final fixed name. */
...
export class RequiresQueenBee extends QueenBee {
  ...
  async #rewrite(
    ...
    // Editing a legacy reference is its lazy migration: promote the old route
    // to the fixed-name root, then rewrite the payload so this leaf and every
    // future appearance share one canonical bag.
