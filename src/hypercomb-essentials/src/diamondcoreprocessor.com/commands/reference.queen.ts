...
import { buildReferenceRecord, normalizeRequiredMarks } from './requires.queen.js'
...
export class ReferenceQueenBee extends QueenBee {
  ...
  protected async execute(args: string): Promise<void> {
    ...
    let pathPart = body
    if (eq !== -1) {
      namePart = safeName(body.slice(0, eq))
    ...
    const marks = normalizeRequiredMarks(tail.filter(t => !t.startsWith('@')))
  ...
  async #createReference(
    ...
    try {
      ...
      // ONE builder, shared with `/requires` and (once it can reach essentials)
      // the portals drop. The bytes ARE the identity here — field order,
      // omitted empties, mark normalization and the selective `refs` closure
      // are all contract rather than formatting — so hand-assembling a second
      // copy of this shape is how two identical references stop deduplicating.
      const record = buildReferenceRecord({
        targetSegments, targetSig, requiredMarks, requiredBouquet,
      })
      ...
      const demand = requiredMarks.length || requiredBouquet
        ...
        : ''
      this.#log(`Reference — "${name}" → /${targetSegments.join('/')}${demand}`, '⇥')
    } catch (err) {
