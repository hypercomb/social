// diamondcoreprocessor.com/groups/sources/websites.source.ts
//
...
// the one index panel draws it and hands it drag-to-create-meaning for free.
//
...
//
// Resolves services through window.ioc at CALL TIME, never by import — see
// aggregate-source.ts.
...
const SIG = /^[0-9a-f]{64}$/
/** The cell's first-class page slot (`website-slot.ts`). Spelled here rather
 *  than imported: the slot name is protocol, not implementation. */
