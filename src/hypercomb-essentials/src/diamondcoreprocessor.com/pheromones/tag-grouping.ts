// tag-grouping.ts — how the pheromone vocabulary is ORGANISED in the
// Pheromones panel. Pure functions, kept out of the view so the rule can be
// tested without a DOM harness — it is a doctrine, not a layout detail.
//
// MOVED VERBATIM from hypercomb-shared/ui/tags-viewer/tag-grouping.ts in the
// everything-is-a-beehavior Phase 2 conversion: `tags-viewer.view.ts` is its
// only caller, and a module may not reach into shared. Not one line of the
// rule changed — only the file's address.
//
