// diamondcoreprocessor.com/presentation/tiles/behavior-enablement.ts
//
// The WRITE side of the behavior-enablement lens — the pool's global lights
// and the per-tile "wake here" exception. Moved here from
// `hypercomb-shared/ui/features-viewer/` with the Beehaviors panel it serves
// (everything-is-a-beehavior Phase 2); the file itself is unchanged.
//
// The READER is `../../sharing/behavior-enablement.ts`. The two still never
// import each other — they agree ONLY on the localStorage keys, the record
// shapes, and the `behavior:enablement-changed` event (the hidden/verified
// split). Both halves now live in essentials, but the seam is deliberate:
// the reader is consulted at render time by every bee, the writer only by
// the roster surface, and merging them would put the write path into every
// bundle that only ever reads.
//
...
/** Cohorts whose lights have been decided (the reader owns the seeding —
 ...
 *  reader's `behaviorPath` in `../../sharing/behavior-enablement.ts`
 *  (lockstep by convention, not import). */
 ...
 *  registered and long before the reader's census seed (`seedGlobalOnKinds`)
 ...
 *  (the reader's `seedCohortOn`) lights behaviour that already worked
