// diamondcoreprocessor.com/presentation/tiles/feature-verified.ts
//
// The WRITE side of the feature-verification gate. Moved here from
...
// recorded here. The READER — `../../sharing/feature-availability.ts` — checks
// the same `hc:feature-verified` key at render time and lets the feature
// activate. The two never import each other; they agree ONLY on key + shape.
//
...
// page after a refresh ("the site disappeared"). The reader —
// `../../sharing/feature-availability.ts` `isWithinAllowedRoot` — checks this
// same key at gate time; the two agree ONLY on key + shape (a JSON array of
// segment-path arrays, the same shape as `hc:adopted-roots`), never importing
// each other.
