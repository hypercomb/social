...
export class PublicationsViewDrone extends Drone {
  ...
  async #mount(gen: number): Promise<void> {
    ...
    if (isBehaviorDormant(PUBLICATIONS_KIND, segments) || await isFeatureHiddenWithin(segments, PUBLICATIONS_KIND)) {
      this.#targetSegments = null
      this.#teardown()
      this.#vm()?.setMode('hexagons')
      return
    ...
    const cards = await fetchPublicationCards({
    ...
    this.#teardown()
    this.#host = this.#build(title, payload?.tagline ?? '', cards)
