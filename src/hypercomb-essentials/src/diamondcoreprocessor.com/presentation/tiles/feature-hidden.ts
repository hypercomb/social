// diamondcoreprocessor.com/presentation/tiles/feature-hidden.ts
//
// The WRITE side of the feature-HIDDEN pool. Moved here from
...
// attached. The READER — `../../sharing/feature-hidden.ts` — scans the same
// pool and makes a matching feature inert at activation time. The two never
// import each other; they agree ONLY on the kind string + record shape,
...
/** Runtime service locator — Store is reached the same loose way everything
 *  else reaches it, never by import. */
...
// The write surface, reachable from any OTHER bundle. This file is the pool's
// ONE writer, and it is inlined into the Beehaviors panel's bundle — so a
// queen in another bundle offering a per-cell verb (`/postit tile` restores
// the hexagon by hiding the postit HERE) resolves this seam the same loose
// way everything reaches Store, rather than inlining a second copy of the
// writer. Registered at module scope; `ioc.register` is first-wins, so the
// shell's copy and this one are interchangeable during the transition.
