// sequence/sequence-editor.bee.ts
//
...
// The gesture — click empty hexes on a blank grid, Done, Cancel — is the same
// whether you are drawing where the NEXT tile lands or the SHAPE a branch is
// read through, so `/sequence` and `/pattern` share this bee and differ only
// in what Done writes:
//
//   • sequence — ordered spiral indexes, saved to SequenceService and BOUND to
//     the branch `/sequence` was launched from.
//   • pattern  — the same hexes as relative axial coords, saved to
//     FrameService as a `{ kind:'pattern', … }` resource and bound to NOTHING.
//     A pattern is a shape that stands alone; `/frame <name>` is what applies
//     it, and one pattern can be applied in as many places as you like.
//
// The click ORDER is meaningful for a sequence and not for a pattern: a frame
// derives its fill order from the shape's own geometry (`scrollOrder`), because
// that is the only order in which a scroll step translates the block rigidly.
// The numbers stay on screen in both modes — in pattern mode they are just a
// count and a click-to-remove handle.
...
type PatternCoord = { q: number; r: number }
type PatternDefinitionLike = { name: string; coords: readonly PatternCoord[]; step: PatternCoord }
type FrameServiceLike = {
  get(name: string): { pattern: PatternDefinitionLike } | null
  save(pattern: PatternDefinitionLike): Promise<string>
}
...
export class SequenceEditorBee extends Worker {
  ...
  // ── editor state ────────────────────────────────────────────────────
  #active = false
  /** Which artifact Done writes. See TWO MODES, ONE EDITOR above. */
  #mode: 'sequence' | 'pattern' = 'sequence'
  #name = 'default'
  #steps: number[] = []
  ...
  /** `/pattern <name>` entry point. Same blank grid, same clicking — but Done
   *  saves a SHAPE, and binds it nowhere. Anchored to no branch on purpose: a
   *  pattern is not "the shape of this place", it is a shape, and `/frame`
   *  is the verb that puts it somewhere. */
  public async openPatternEditor(name: string): Promise<void> {
    if (this.#active) return
    const lineage = window.ioc.get<LineageLike>('@hypercomb.social/Lineage')
    const committer = window.ioc.get<CommitterLike>('@diamondcoreprocessor.com/LayerCommitter')
    if (!lineage || !committer?.bootstrapIfEmpty) return
    ...
    this.#mode = 'pattern'
    this.#name = (name || 'pattern').trim() || 'pattern'
    this.#anchorSegments = []
    this.#returnSegments = this.#currentSegments()

    // Preload an existing pattern so re-running `/pattern <name>` edits it —
    // including a built-in, which is how you make your own honeycomb: open it,
    // change it, Done. The saved copy then wins in the palette.
    const frames = window.ioc.get<FrameServiceLike>('@FrameService')
    const existing = frames?.get(this.#name)
    this.#steps = existing ? this.#coordsToIndexes(existing.pattern.coords) : []
    this.#hoverIndex = null
  ...
  /** Save, then exit. A sequence is saved AND bound to the anchor branch; a
   *  pattern is only saved — `/frame` is what applies it. */
  async #done(): Promise<void> {
    if (!this.#active) return
    const name = (this.#nameInput?.value || this.#name).trim() || (this.#mode === 'pattern' ? 'pattern' : 'default')
    this.#name = name
    const steps = [...this.#steps]

    if (this.#mode === 'pattern') {
      const frames = window.ioc.get<FrameServiceLike>('@FrameService')
      const coords = this.#indexesToCoords(steps)
      if (frames && coords.length) {
        try {
          // Coords are saved EXACTLY where they were drawn. The editor's grid
          // origin is the grid origin, so what you drew is where the frame
          // sits — no re-centring to surprise you later. The step is the
          // horizontal one; the record carries it so a pattern authored with
          // another travel direction stays readable.
          await frames.save({ name, coords, step: { q: 1, r: 0 } })
        } catch (err) {
          console.warn('[/pattern] save failed', err)
        }
      }
      this.#teardown(true)
      return
  ...
  // ── spiral indexes ⇄ axial coords ───────────────────────────────────
  //
  // The editor thinks in spiral indexes (that is what a click resolves to and
  // what the target layer draws); a pattern is stored as axial coords, because
  // a shape has to survive being read anywhere. AxialService owns both sides
  // of the mapping, so neither is re-derived here.

  #indexesToCoords(indexes: readonly number[]): PatternCoord[] {
    const axial = window.ioc.get<AxialLike>('@diamondcoreprocessor.com/AxialService')
    if (!axial?.items) return []
    const out: PatternCoord[] = []
    const seen = new Set<string>()
    for (const index of indexes) {
      const coord = axial.items.get(index)
      if (!coord) continue
      const key = `${coord.q},${coord.r}`
      if (seen.has(key)) continue
      seen.add(key)
      out.push({ q: coord.q, r: coord.r })
    }
    return out
  }

  #coordsToIndexes(coords: readonly PatternCoord[]): number[] {
    const axial = window.ioc.get<AxialLike>('@diamondcoreprocessor.com/AxialService')
    if (!axial?.newCoordinate) return []
    const out: number[] = []
    for (const c of coords) {
      const index = axial.newCoordinate(c.q, c.r, -c.q - c.r)?.index
      if (Number.isFinite(index)) out.push(index)
    }
    return out
  }
  ...
  #showOverlay(): void {
    ...
    hint.textContent = this.#mode === 'pattern'
      ? this.#i18n('pattern.hint', 'Click hexes to draw the shape. Apply it with /frame <name>.')
      : this.#i18n('sequence.hint', 'Click hexes in order to set where new tiles land.')
    ...
    nameInput.placeholder = this.#mode === 'pattern'
      ? this.#i18n('pattern.namePlaceholder', 'pattern name')
      : this.#i18n('sequence.namePlaceholder', 'sequence name')
  ...
  #updateCount(): void {
    ...
    // A pattern's number is its CAPACITY — how many tiles the frame shows at
    // once — which is the one thing worth knowing while drawing one.
    this.#countLabel.textContent = this.#mode === 'pattern'
      ? this.#i18n('pattern.count', `${n} tiles`).replace('{count}', String(n))
      : this.#i18n('sequence.count', `${n} targets`).replace('{count}', String(n))
  }
