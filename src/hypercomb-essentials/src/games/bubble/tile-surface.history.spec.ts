...
import { BubbleTileSurface } from './tile-surface.js'

// Real HistoryService + LayerCommitter; only browser OPFS handles are replaced
// with an in-memory implementation. This exercises the actual marker,
// incidence, cascade, and current-head behavior used by the running hive.
vi.hoisted(() => {
...
it('creates, edits, and reopens an 800-cell round through real hive history services', async () => {
  ...
  const store = {
    ...
    getResourceLocal: async (sig: string) => root.files.get(sig)?.getFile() ?? null,
  ...
  const surface = new BubbleTileSurface({ history, committer, parentSegments: [], domain: () => 'test.example' })
  const loaded = await surface.ensureRound(0)
  expect(loaded.tiles).toHaveLength(CAVE_COLUMNS * CAVE_ROWS)
  const round = await history.currentLayerAt(await history.sign({ explorerSegments: () => loaded.roundSegments }))
  expect(round!.children).toHaveLength(CAVE_COLUMNS * CAVE_ROWS)
  for (const ref of round!.children!.slice(0, 3)) {
    ...
    expect((await history.getLayerBySig(ref))!['bubbleCell']).toBeDefined()
  }

  const edited = loaded.tiles[100]
  const old = edited.layer['bubbleCell'] as { value: number }
  await committer.update(edited.segments, {
    bubbleCell: { ...(edited.layer['bubbleCell'] as object), value: old.value ^ 1 },
  })
  const reopenedHistory = new HistoryService()
  services.set('@diamondcoreprocessor.com/HistoryService', reopenedHistory)
  const reopened = await new BubbleTileSurface({
    history: reopenedHistory, committer, parentSegments: [], domain: () => 'test.example',
  }).ensureRound(0)
  expect(reopened.level.nativeCells![edited.row][edited.col]).toBe(old.value ^ 1)
  ...
  expect(rootChildren.map(layer => layer?.name)).toEqual(['notes', 'bubble-bobble-dos-v1'])
