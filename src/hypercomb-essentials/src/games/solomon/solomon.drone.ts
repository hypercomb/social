// games/solomon/solomon.drone.ts
//
...
import { SolomonQueenBee } from './solomon.queen.js'
import { gameKind, isGameDormant, onEnablementChanged } from '../game-enablement.js'
import type { SolomonLabyrinthOverlay } from './labyrinth-overlay.js'
...
export class SolomonDrone extends Drone {
  ...
  readonly #overlay = new LazyOverlay<SolomonLabyrinthOverlay>(
    () => import('./labyrinth-overlay.js').then(m => (onClose: () => void) => new m.SolomonLabyrinthOverlay(onClose)),
    () => this.close(),
    () => this.#emitState(),
  )
  #wired = false
...
window.ioc.register('@diamondcoreprocessor.com/SolomonQueenBee', new SolomonQueenBee())
