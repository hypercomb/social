...
const CSS = `
.document-curator{position:fixed;inset:0;z-index:20;display:flex;flex-direction:column;overflow:hidden;
  background:var(--md-surface);color:var(--md-on-surface);font:14px/1.4 var(--hc-font,system-ui,sans-serif)}
.document-curator>header{display:flex;align-items:center;justify-content:space-between;gap:20px;padding:18px 24px;
  border-bottom:1px solid var(--md-outline-variant);background:var(--md-surface-c-low)}
.curator-heading{display:flex;flex-direction:column;gap:3px}
.curator-heading small{color:var(--md-primary);letter-spacing:.16em;font-size:9px}
.curator-heading strong{font-size:17px;color:var(--md-on-surface-strong)}
.curator-actions{display:flex;gap:8px}
.curator-actions button,.curator-back,.curator-link{border:1px solid var(--md-outline);
  border-radius:var(--hc-radius-control,2px);background:var(--md-surface-c-high);color:var(--md-on-surface);
  padding:8px 13px;cursor:pointer}
.curator-actions button:hover,.curator-back:hover,.curator-link:hover{background:var(--md-surface-c-highest)}
.curator-done{border-color:var(--md-primary)!important;background:var(--md-primary)!important;
  color:var(--md-on-primary)!important;font-weight:800}
.curator-nav{display:flex;align-items:center;gap:12px;min-height:48px;padding:0 24px;
  border-bottom:1px solid var(--md-outline-variant);color:var(--md-on-surface-var)}
.curator-back{padding:5px 9px}.curator-summary{display:flex;align-items:center;gap:12px;
  padding:12px 24px;color:var(--md-on-surface-var)}
.curator-summary strong{margin-right:auto}.curator-link{padding:4px 8px;background:transparent}
.curator-list{flex:1;overflow:auto;padding:6px 18px 32px}
.curator-row{display:grid;grid-template-columns:42px 1fr;align-items:center;max-width:900px;margin:8px auto;
  border:1px solid var(--md-outline-variant);border-radius:var(--hc-radius-control,2px);
  background:var(--md-surface-c-lowest)}
.curator-check{display:grid;place-items:center;width:24px;height:24px;margin:auto;
  border:1px solid var(--md-outline);border-radius:var(--hc-radius-floating,4px);
  background:var(--md-surface-c-low);color:var(--md-on-surface);cursor:pointer}
.curator-check.is-selected{border-color:var(--md-primary);background:var(--md-primary);color:var(--md-on-primary)}
.curator-label{display:grid;grid-template-columns:1fr auto;gap:3px 12px;min-width:0;padding:13px 14px;
  border:0;border-left:1px solid var(--md-outline-variant);background:transparent;color:inherit;text-align:left;cursor:pointer}
.curator-label:hover{background:var(--hc-window-tint)}
.curator-label strong{overflow:hidden;text-overflow:ellipsis}
.curator-label small{grid-column:1;color:var(--md-on-surface-var);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.curator-open{grid-column:2;grid-row:1/3;align-self:center;color:var(--md-primary)}
.curator-empty{padding:60px;text-align:center;color:var(--md-on-surface-var)}
.document-curator button:focus-visible{outline:2px solid var(--md-primary);outline-offset:2px}
@media(max-width:599px){.document-curator>header{padding:13px 14px}.curator-heading strong{font-size:14px}
  .curator-nav,.curator-summary{padding-inline:14px}.curator-list{padding-inline:8px}
  .curator-actions{gap:4px}.curator-actions button{padding:7px 9px}.curator-row{grid-template-columns:38px 1fr}}
`
