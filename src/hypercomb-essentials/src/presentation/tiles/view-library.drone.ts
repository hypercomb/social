...
const CSS = `
.hc-view-library{position:fixed;top:0;bottom:0;left:var(--hc-inset-left,0px);right:var(--hc-inset-right,0px);
  z-index:150;overflow:auto;background:var(--md-surface);color:var(--md-on-surface);
  font:15px/1.55 var(--hc-read,system-ui,sans-serif)}
.library-bar{position:sticky;top:0;z-index:4;display:grid;grid-template-columns:1fr auto 1fr;
  align-items:center;height:52px;padding:0 24px;background:var(--md-surface-c-low);
  border-bottom:1px solid var(--md-outline-variant);letter-spacing:.18em;font-size:10px}
.library-view-actions{display:flex;align-items:center;gap:10px}
.library-reach{color:var(--md-on-surface-var);font-size:9px}
.library-curate{padding:5px 8px!important;border:1px solid var(--md-primary)!important;
  border-radius:var(--hc-radius-floating,4px);background:var(--md-primary-container)!important;
  color:var(--md-on-primary-c)!important;font:700 10px/1 var(--hc-font,system-ui,sans-serif)!important;
  letter-spacing:0!important}
.library-bar>button{justify-self:end;border:0;background:none;color:inherit;font-size:28px;cursor:pointer}
.library-bar button:focus-visible,.studio-rail .view-jump:focus-visible{outline:2px solid var(--md-primary);outline-offset:2px}
.library-heading{padding:70px 0 34px}
.library-heading h1{margin:0;font:500 clamp(42px,7vw,76px)/1 Georgia,serif;letter-spacing:-.04em}
.library-heading p{color:var(--md-on-surface-var)}
.atlas-main,.studio-main{width:min(1400px,calc(100% - 40px));margin:auto}
.atlas-summary{display:grid;grid-template-columns:repeat(5,1fr);gap:10px;margin-bottom:42px}
.atlas-stat{padding:20px;background:var(--md-surface-c-low);border:1px solid var(--md-outline-variant)}
.atlas-stat strong{display:block;font:38px Georgia,serif}
.atlas-stat span{text-transform:uppercase;color:var(--md-on-surface-var);font-size:10px;letter-spacing:.12em}
.atlas-lanes{display:grid;grid-template-columns:repeat(3,1fr);gap:18px;padding-bottom:70px}
.atlas-lane{min-height:180px;padding:18px;background:var(--md-surface-c-lowest);border-top:3px solid var(--md-primary)}
.atlas-lane h2{text-transform:uppercase;font-size:11px;letter-spacing:.15em}
.atlas-card{margin:12px 0;padding:16px;background:var(--md-surface-c-low)}
.atlas-card small{color:var(--md-on-surface-var)}.atlas-card p{white-space:pre-wrap}
.atlas-tags,.studio-tags{display:flex;gap:5px;flex-wrap:wrap}
.atlas-tags span,.studio-tags span{padding:2px 7px;border:1px solid var(--md-outline);
  border-radius:var(--hc-radius-pill,999px);font-size:9px}
.role-question{border-color:var(--hc-status-info)}.role-risk{border-color:var(--hc-status-alert)}
.role-decision{border-color:var(--hc-status-warn)}.empty{color:var(--md-on-surface-var);font-style:italic}
.studio{background:var(--md-surface-dim);color:var(--md-on-surface)}
.studio-main{width:min(1060px,calc(100% - 36px))}
.studio-rail{position:sticky;top:52px;z-index:2;display:flex;gap:18px;overflow:auto;
  padding:14px 0;background:var(--md-surface-dim);border-bottom:1px solid var(--md-outline-variant)}
.studio-rail .view-jump{border:0;background:none;color:var(--md-on-surface-var);
  font:inherit;white-space:nowrap;cursor:pointer}
.studio-rail .view-jump:hover{color:var(--md-primary)}
.studio-scene{min-height:60vh;margin:22px 0 70px;padding:clamp(28px,6vw,72px);
  background:var(--md-surface-c-lowest);box-shadow:var(--md-elev-3)}
.studio-scene>header{display:grid;grid-template-columns:52px 1fr}
.scene-number{color:var(--md-on-surface-var)}
.studio-scene h2{margin:0;font:500 clamp(34px,5vw,58px)/1 Georgia,serif}
.scene-source{grid-column:2;margin:8px 0 0;color:var(--md-on-surface-var);font-size:11px}
.studio-tags{grid-column:2;margin-top:15px}
.scene-lead{max-width:800px;margin:70px 0 40px;font:28px/1.4 Georgia,serif;
  border-left:4px solid var(--md-primary);padding-left:28px}
.scene-note{max-width:720px;margin:24px 0 0 52px}
.scene-1{background:var(--md-primary-container);color:var(--md-on-primary-c)}
.scene-2{border-top:10px solid var(--md-secondary)}
