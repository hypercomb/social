...
describe('the container a frame derives', () => {
  ...
  it('keeps a long unbroken word from bursting its track', () => {
    ...
    // Three holes plus the container itself — a container that cannot shrink
    // is the thing that overflows, however well its holes behave.
    expect(derivedContainer(divisionPlan(3, 'row'))!.match(/min-width:0/g)).toHaveLength(4)
  })
