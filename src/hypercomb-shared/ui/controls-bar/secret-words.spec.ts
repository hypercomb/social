...
// FNV-1a hash + word lookup — copied from source to avoid module side-effects
...
describe('secretTag', () => {
  ...
  it('uses words from the ADJECTIVES and NOUNS arrays', () => {
  ...
  it('specific known values remain stable (snapshot)', () => {
    // Lock in specific outputs to detect accidental changes to the hash or word lists
    const tag1 = secretTag('test')
    const tag2 = secretTag('password')
    // These are deterministic — if this test breaks, the hash or word list changed
