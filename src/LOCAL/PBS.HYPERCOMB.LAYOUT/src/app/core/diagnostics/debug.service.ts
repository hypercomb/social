...
export class DebugService {
  ...
  /** expose a state class under a short name (dev-only) */
  public static expose<T>(name: string, instance: T): void {
    if (environment.production) return

    DebugService.state[name] = instance
    window.state = DebugService.state // namespaced for autocomplete
  }

  /** remove a state (dev-only) */
  public static remove(name: string): void {
    if (environment.production) return
    delete DebugService.state[name]
  }

  /** list all registered states (dev-only) */
