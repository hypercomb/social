...
import { Signature, HashService } from '../hash.service'
...
export class CapabilityManager {
  ...
  public async stage(data: Blob | string): Promise<Signature> {
    ...
    if (!this.staged.has(signature)) this.staged.set(signature, { signature, data })
    return signature
  }
  ...
  private async exists(dir: FileSystemDirectoryHandle, signature: Signature): Promise<boolean> {
    try {
