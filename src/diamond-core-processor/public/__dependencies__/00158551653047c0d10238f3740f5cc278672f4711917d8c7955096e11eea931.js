...
var generateThumbnailBlob = generatePreviewThumbnail;
...
var TileEditorService = class extends EventTarget {
  ...
  get hideText() {
    return this.#properties.hideText === true;
  }
  // ── state mutations ────────────────────────────────────────────
  open = (cell, properties, largeBlob) => {
    ...
    this.#mode = "editing";
    this.#emit();
    EffectBus2.emit("editor:mode", { active: true });
  };
  close = () => {
    this.#mode = "idle";
    this.#cell = "";
    ...
    this.#largeBlob = null;
    this.#emit();
    EffectBus2.emit("editor:mode", { active: false });
  };
  setLink = (value) => {
    if (value) {
  ...
  setHideText = (value) => {
    if (value) {
      this.#properties.hideText = true;
    } else {
      delete this.#properties.hideText;
    }
    this.#emit();
  };
  setLargeBlob = (blob) => {
...
var invalidateTileProperties = (cacheKey) => {
  if (cacheKey) tilePropertiesCache.delete(cacheKey);
};
var clearTilePropertiesCache = () => {
...
export {
  ...
  armFromClipboard,
  armImageBlob,
  clearTilePropertiesCache,
  generateHexThumbnails,
  generatePreviewThumbnail,
  generateThumbnailBlob,
  invalidateTileProperties,
  isSignature,
  readCellProperties,
  resolveResourceSignatures,
  tilePropertiesCacheKey,
  writeCellProperties
};
