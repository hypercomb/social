...
import { QueenBee as QueenBee12, EffectBus as EffectBus11 } from "@hypercomb/core";
var DISMISSED_KEY = "hc:player-dismissed";
var PlayerQueenBee = class extends QueenBee12 {
  namespace = "diamondcoreprocessor.com";
  command = "ebook";
  description = "Open the track player";
  execute(_args) {
    try {
      localStorage.removeItem(DISMISSED_KEY);
    } catch {
    }
    EffectBus11.emit("player:open", {});
  }
};
...
import { QueenBee as QueenBee13, EffectBus as EffectBus12, hypercomb as hypercomb3 } from "@hypercomb/core";
  ...
  async execute(args) {
    ...
    for (const name of targets) {
      try {
        ...
        EffectBus12.emit("cell:removed", { cell: name, groupId });
      } catch (e) {
...
import { QueenBee as QueenBee14, EffectBus as EffectBus13, hypercomb as hypercomb4 } from "@hypercomb/core";
var RenameQueenBee = class extends QueenBee14 {
  ...
  async execute(args) {
    ...
    try {
      ...
      EffectBus13.emit("cell:removed", { cell: oldName, groupId });
      EffectBus13.emit("cell:added", { cell: newName, groupId });
...
import { QueenBee as QueenBee15, EffectBus as EffectBus14 } from "@hypercomb/core";
...
import { EffectBus as EffectBus15, SignatureService as SignatureService2, I18N_IOC_KEY as I18N_IOC_KEY3 } from "@hypercomb/core";
...
import { QueenBee as QueenBee17, EffectBus as EffectBus16 } from "@hypercomb/core";
...
import { QueenBee as QueenBee18, EffectBus as EffectBus17 } from "@hypercomb/core";
...
import { QueenBee as QueenBee20, EffectBus as EffectBus18 } from "@hypercomb/core";
...
async function ensureInstructionsBootstrap() {
  ...
  EffectBus18.emit("cell:added", { cell: "instructions", segments: [] });
  ...
  for (const d of INSTRUCTIONS_DEFAULTS) {
    ...
    EffectBus18.emit("cell:added", { cell: d.name, segments: ["instructions"] });
...
var toast2 = (type, title, message) => {
  try {
    EffectBus18.emit("toast:show", { type, title, message });
  } catch {
  }
};
...
var WebsiteQueenBee = class extends QueenBee20 {
  ...
  execute(args) {
    ...
    if (trimmed === "upgrade" || trimmed.startsWith("upgrade ") || trimmed.startsWith("upgrade	")) {
      ...
      EffectBus18.emit("website:build", {
        mode: "upgrade",
        scope,
        scopeName,
