...
var MousewheelZoomInput = class {
  ...
  attach = (zoom, canvas) => {
    ...
    this.stack = window.ioc.get("@diamondcoreprocessor.com/InputModeStack") ?? null;
    if (this.stack) {
      this.#mode = {
  ...
  onWheel = (event) => {
    ...
    const pivot = this.gate?.locked ? { x: rect.left + rect.width / 2, y: rect.top + rect.height / 2 } : { x: event.clientX, y: event.clientY };
