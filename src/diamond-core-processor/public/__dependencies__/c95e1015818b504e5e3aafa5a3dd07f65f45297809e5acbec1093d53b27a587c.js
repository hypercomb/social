...
import { QueenBee as QueenBee6, EffectBus as EffectBus5, hypercomb as hypercomb2 } from "@hypercomb/core";
...
// src/diamondcoreprocessor.com/commands/skip-intro.queen.ts
import { QueenBee as QueenBee10, EffectBus as EffectBus8 } from "@hypercomb/core";
var SkipIntroQueenBee = class extends QueenBee10 {
  namespace = "diamondcoreprocessor.com";
  command = "skip-intro";
  aliases = ["bypass", "skip"];
  description = "Skip the intro audio (toggle to reset)";
  async execute(_args) {
    const episodeOneKey = "hc:intro:episode-1-watched";
    const episodeZeroKey = "hc:intro:episode-0-watched";
    const bothWatched = localStorage.getItem(episodeOneKey) === "true" && localStorage.getItem(episodeZeroKey) === "true";
    if (bothWatched) {
      localStorage.removeItem(episodeOneKey);
      localStorage.removeItem(episodeZeroKey);
      EffectBus8.emit("toast:show", { message: "intro will replay on next load" });
      return;
    }
    EffectBus8.emit("intro:skip", {});
  }
};
var _skipIntro = new SkipIntroQueenBee();
