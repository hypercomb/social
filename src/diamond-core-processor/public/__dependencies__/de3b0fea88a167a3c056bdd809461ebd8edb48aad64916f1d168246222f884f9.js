...
import { Drone, EffectBus as EffectBus3 } from "@hypercomb/core";
...
var TouchGestureCoordinator = class {
  ...
  #emitDragging(active) {
    ...
    EffectBus2.emit("touch:dragging", { active });
  }
  #emitSensitivityBar(visible) {
    EffectBus2.emit("touch:sensitivity-bar", {
...
var ViewportPersistence = class extends EventTarget {
  ...
  #persistTo = async (dir, pending) => {
    try {
      ...
      EffectBus3.emit("viewport:persisted", { dir, snapshot: viewport });
    } catch {
    }
  };
  #persist = async () => {
...
var ZoomDrone = class extends Drone {
  ...
  #registerKeybindings() {
    ...
    EffectBus3.emit("keymap:add-layer", { layer });
  }
  ...
  zoomToScale = (scale, pivotClient) => {
    ...
    EffectBus3.emitTransient("viewport:manual", {});
    this.#cancelAnim();
