...
import { EffectBus as EffectBus2, SignatureService } from "@hypercomb/core";
var EMPTY_LAYER_STATE_SIG = "892748ce914b902feb21d612f4652cc231b4455786683b23f9d82304ce061be1";
var EMPTY_LAYER_STATE = Object.freeze({ bees: [], layers: [], dependencies: [], resources: [] });
var EMPTY_LAYER_CONTENT_SIG = "a8a9aaacd1d7631b9d1b66a6b0e4b14fdd2f1052ffd5dfac2e92c0740020ee8d";
...
var HistoryService = class _HistoryService {
  ...
  commitLayer = async (locationSig, layer) => {
    ...
    EffectBus2.emit("content:wrote", { sig: layerSig, kind: "layer", bytes: bytes.buffer });
    return layerSig;
  };
  /**
   ...
   * rendered" desync.
   */
  listMarkerFilenames = async (locationSig) => {
    let bag;
    try {
   ...
   */
  readMarker = async (locationSig, filename) => {
    let bag;
    try {
      bag = await this.historyRoot.getDirectoryHandle(locationSig, { create: false });
    } catch {
  ...
  preloadAllBags = async () => {
    ...
    this.#preloadAllBagsPromise = (async () => {
      ...
      console.log(
        `[preload] preloadAllBags done: ${cachedCount} heads cached / ${bagCount} bags scanned / ${markerCount} markers seen (${elapsed}ms). first sigs: [${previewSigs.join(", ")}${cachedCount > previewSigs.length ? ", \u2026" : ""}]`
      );
      try {
   ...
   */
  preloadFromRoot = async (rootSig) => {
    if (!_HistoryService.#SIG_RE.test(rootSig)) return;
    const startMs = performance.now();
    const visited = /* @__PURE__ */ new Set();
    const depthHistogram = /* @__PURE__ */ new Map();
    let cacheHits = 0;
    let walked = 0;
    const walk = async (sig, depth) => {
      ...
      for (const childSig of children) {
        if (_HistoryService.#SIG_RE.test(childSig)) {
    ...
    const elapsed = Math.round(performance.now() - startMs);
    const depthSummary = [...depthHistogram.entries()].sort(([a], [b]) => a - b).map(([d, n]) => `d${d}:${n}`).join(" ");
    console.log(
      `[preload] preloadFromRoot done: ${walked} layers reachable from ${rootSig.slice(0, 12)} (${cacheHits} already cached, ${walked - cacheHits} newly warmed) in ${elapsed}ms. depths: ${depthSummary}`
    );
  };
  /**
...
var diffLayers = (prev, next) => {
  const prevChildren = prev?.children ?? EMPTY_LAYER_CONTENT.children;
  const nextChildren = next.children ?? EMPTY_LAYER_CONTENT.children;
  const diffs = [];
...
var HistoryCursorService = class _HistoryCursorService extends EventTarget {
   ...
   */
  async #findGroupLeader(position) {
   ...
   */
  async #isCascadeAtPosition(position) {
  ...
  async #loadContentForSig(signature) {
    ...
    try {
      ...
      const content = !parsed.name && !parsed.children ? EMPTY_LAYER_CONTENT : { name: parsed.name ?? "", children: parsed.children };
  ...
  #emit() {
    ...
    this.dispatchEvent(new CustomEvent("change"));
    EffectBus3.emit("history:cursor-changed", this.state);
  }
...
import { EffectBus as EffectBus4, SignatureService as SignatureService2 } from "@hypercomb/core";
...
var HiveParticipant = class {
  ...
  async #commitParticipant(item, parentSegments, history, store) {
    ...
    const bodySig = await SignatureService2.sign(bodyBytes.buffer);
...
// src/diamondcoreprocessor.com/history/layer-machine.ts
var SIG_REGEX2 = /^[a-f0-9]{64}$/;
var LayerMachine = class _LayerMachine {
   ...
   */
  static fromLayer(prev, fallbackName, segments) {
    const m = new _LayerMachine();
  ...
  static empty(name, segments) {
    return _LayerMachine.fromLayer(null, name, segments);
  }
  /**
   ...
   */
  static async atLineage(history, lineage) {
    const segs = (lineage.explorerSegments?.() ?? []).map((s) => String(s ?? "").trim()).filter((s) => s.length > 0);
    const name = segs.length === 0 ? "/" : segs[segs.length - 1];
    const locationSig = await history.sign({ explorerSegments: () => segs });
    const prev = await history.currentLayerAt(locationSig);
    return _LayerMachine.fromLayer(prev, name, segs);
  }
  /**
...
var LayerSlotRegistry = class {
  ...
  #announcedTriggers = /* @__PURE__ */ new Map();
...
var NurseBee = class extends Bee {
   ...
   * have one in hand.
   */
  async read(parentSegments, cellName, cellDir, cacheKey) {
...
var OrderProjection = class {
  ...
  #currentSig = null;
  constructor() {
    EffectBus7.on("cell:added", (payload) => {
    ...
    EffectBus7.on("cell:removed", (payload) => {
      if (!payload?.cell || !this.#currentSig) return;
      const order = this.#cache.get(this.#currentSig);
      if (order) {
    ...
    EffectBus7.on("cell:reorder", (payload) => {
...
import { QueenBee as QueenBee2, EffectBus as EffectBus8 } from "@hypercomb/core";
var ReviseQueenBee = class extends QueenBee2 {
  ...
  #enter() {
    this.#active = true;
    EffectBus8.emit("revise:mode-changed", { active: true });
  ...
  #exit() {
    ...
    this.#active = false;
    EffectBus8.emit("revise:mode-changed", { active: false });
...
export {
  ...
  EMPTY_LAYER_CONTENT,
  EMPTY_LAYER_CONTENT_SIG,
  EMPTY_LAYER_STATE,
  EMPTY_LAYER_STATE_SIG,
  GlobalTimeClock,
