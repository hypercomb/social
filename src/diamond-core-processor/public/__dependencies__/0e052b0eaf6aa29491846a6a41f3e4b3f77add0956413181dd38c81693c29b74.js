...
// src/diamondcoreprocessor.com/presentation/grid/center-slot-tracker.ts
import { EffectBus } from "@hypercomb/core";
var CenterSlotTracker = class {
  #dirty = true;
  #cached = /* @__PURE__ */ new Map();
  constructor() {
    EffectBus.on("viewport:persisted", () => {
      this.#dirty = true;
    });
    EffectBus.on("viewport:manual", () => {
      this.#dirty = true;
    });
  ...
  #compute = () => {
    ...
    const axial = ioc?.get?.("@diamondcoreprocessor.com/AxialService");
    if (!axial || axial.items.size === 0) return out;
    const vp = ioc?.get?.("@diamondcoreprocessor.com/ViewportPersistence");
...
var _centerSlotTracker = new CenterSlotTracker();
window.ioc?.register?.("@diamondcoreprocessor.com/CenterSlotTracker", _centerSlotTracker);
