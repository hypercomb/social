...
// src/diamondcoreprocessor.com/sharing/facade.nurse.ts
var FacadeNurse = class extends NurseBee {
...
var _facadeNurse = new FacadeNurse();
window.ioc?.register?.(_facadeNurse.iocKey, _facadeNurse);

// src/diamondcoreprocessor.com/sharing/mesh-block.queen.ts
import { get as get2 } from "@hypercomb/core";
var NOSTR_MESH_KEY = "@diamondcoreprocessor.com/NostrMeshDrone";
var SWARM_DRONE_KEY = "@diamondcoreprocessor.com/SwarmDrone";
var MeshBlockQueenBee = class {
  command = "block-peer";
  aliases = ["blockpeer", "block-pubkey", "kick-peer"];
  description = "Block a peer pubkey at the dev relay and drop its tiles locally. Args: full 64-hex or 8\u201316 hex prefix.";
  descriptionKey = "slash.block-peer";
  invoke(args) {
    ...
    const mesh = get2(NOSTR_MESH_KEY);
    const swarm = get2(SWARM_DRONE_KEY);
    const relayResult = mesh?.sendHcBlock?.(pubkey) ?? null;
    const swarmResult = swarm?.evictPubkey?.(pubkey) ?? null;
    console.log(
      `[block-peer] ${pubkey}: relay HC_BLOCK sent to ${relayResult?.sent ?? 0} socket(s), wiped ${relayResult?.cachedWiped ?? 0} local mesh cache entr${(relayResult?.cachedWiped ?? 0) === 1 ? "y" : "ies"}; swarm evicted ${swarmResult?.entriesEvicted ?? 0} peer entr${(swarmResult?.entriesEvicted ?? 0) === 1 ? "y" : "ies"} across ${swarmResult?.sigsAffected ?? 0} sig(s).`
    );
  }
};
window.ioc?.register?.(
...
// src/diamondcoreprocessor.com/sharing/mesh-clear.queen.ts
import { get as get3 } from "@hypercomb/core";
var NOSTR_MESH_KEY2 = "@diamondcoreprocessor.com/NostrMeshDrone";
var SWARM_DRONE_KEY2 = "@diamondcoreprocessor.com/SwarmDrone";
var MeshClearQueenBee = class {
  ...
  command = "clear-mesh";
  aliases = ["clearmesh", "clear-relay", "wipe-mesh"];
  description = "Wipe the dev relay event store and drop all peer caches. Live peers will reappear on their next heartbeat (\u226430s).";
  descriptionKey = "slash.clear-mesh";
  invoke(_args) {
    const mesh = get3(NOSTR_MESH_KEY2);
    const swarm = get3(SWARM_DRONE_KEY2);
    let relayResult = null;
    let swarmResult = null;
    if (mesh?.sendHcClear) {
      try {
        relayResult = mesh.sendHcClear();
      } catch (err) {
        console.warn("[clear-mesh] sendHcClear failed", err);
      }
    }
    if (swarm?.clearAllPeers) {
      try {
        swarmResult = swarm.clearAllPeers();
      } catch (err) {
        console.warn("[clear-mesh] clearAllPeers failed", err);
      }
    }
...
// src/diamondcoreprocessor.com/sharing/nostr-signer.ts
var SECRET_KEY_STORAGE = "hc:nostr:secret-key";
var NostrSigner = class {
  /**
  ...
  signEvent = async (evt) => {
    ...
    if (anyWin?.nostr?.signEvent) {
      const signed2 = await anyWin.nostr.signEvent(evt);
      if (!signed2) throw new Error("nostr signer: nip-07 returned empty result");
      return signed2;
    }
    const secretHex = this.resolveSecretKeyHex();
  ...
  resolveSecretKeyHex = () => {
    ...
    try {
      localStorage.setItem(SECRET_KEY_STORAGE, fresh);
    } catch {
      this.#ephemeralSecretKey = fresh;
    }
    return fresh;
  };
  ...
  #mintSecretKeyHex = () => {
    ...
    let hex = "";
    for (let i2 = 0; i2 < 32; i2++) hex += bytes[i2].toString(16).padStart(2, "0");
    return hex;
  };
  isHex64 = (value) => /^[0-9a-f]{64}$/.test(value);
  hexToBytes = (hex) => {
    const out = new Uint8Array(32);
    for (let i2 = 0; i2 < 32; i2++) {
      out[i2] = Number.parseInt(hex.slice(i2 * 2, i2 * 2 + 2), 16);
    }
    return out;
  };
};
...
var PairedChannelMachine = class {
  ...
  #join(e) {
    ...
    const join2 = { id: e.id, pubkey: e.pubkey, observedAt: e.createdAt };
    this.state.pendingJoins.set(e.id, join2);
...
async function tryChannelId(lineageSig, room, secret) {
  try {
...
var PushQueueService = class _PushQueueService extends EventTarget {
  ...
  static #ENTRY_RE = /^([a-f0-9]{64})\.(layer|bee|dependency|resource)$/;
  #draining = false;
  constructor() {
    super();
    EffectBus3.on(
    ...
    try {
      for (; ; ) {
        ...
        for (const entry of entries) {
          ...
          EffectBus3.emit("push:receipt", { sig: entry.sig });
        }
      }
    } finally {
      this.#draining = false;
  ...
  #listQueue = async () => {
    ...
    for await (const [name, handle] of dir.entries()) {
      ...
      const m = name.match(_PushQueueService.#ENTRY_RE);
      if (!m) continue;
      try {
        const file = await handle.getFile();
        items.push({ sig: m[1], kind: m[2], fileName: name, mtime: file.lastModified });
      } catch {
      }
    }
  ...
  #removeQueueEntry = async (fileName) => {
    try {
      const dir = await this.#getQueueDir();
      if (!dir) return;
   ...
   */
  #pushAndReceipt = async (entry) => {
    const bridge = globalThis.__sentinelBridge;
    if (!bridge?.intake) return false;
    let bytes;
    try {
      const dir = await this.#getQueueDir();
      if (!dir) return false;
