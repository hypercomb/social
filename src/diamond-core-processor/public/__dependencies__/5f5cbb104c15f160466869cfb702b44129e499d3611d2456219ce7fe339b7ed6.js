...
var tilePropertiesCache = /* @__PURE__ */ new Map();
var readCellProperties = async (cellDir, cacheKey) => {
  if (cacheKey) {
...
import { EffectBus as EffectBus4, SignatureService } from "@hypercomb/core";
...
var HistoryService = class _HistoryService {
  ...
  commitLayer = async (locationSig, layer) => {
    ...
    EffectBus4.emit("content:wrote", { sig: layerSig, kind: "layer", bytes: bytes.buffer });
    return layerSig;
  };
  /**
...
var armImageBlob = async (blob, opts = {}) => {
  ...
  EffectBus6.emit("command:arm-resource", {
    previewUrl,
    largeSig,
    smallPointSig,
    smallFlatSig,
...
var EmptyLongPressInput = class {
  ...
  #attached = false;
  constructor() {
    EffectBus7.on("render:host-ready", (payload) => {
      this.#canvas = payload.canvas;
      this.#container = payload.container;
      this.#renderer = payload.renderer;
      this.#attach();
    });
    EffectBus7.on("render:mesh-offset", (offset) => {
      this.#meshOffset = offset;
    });
    EffectBus7.on("render:set-orientation", ({ flat }) => {
      this.#flat = !!flat;
    });
    EffectBus7.on("render:cell-count", ({ coords }) => {
      this.#occupied.clear();
      if (!coords) return;
  ...
  #onPointerDown = (e) => {
    ...
    this.#holdTimer = setTimeout(() => {
      ...
      EffectBus7.emit("mobile:input-visible", { visible: true, mobile: true });
...
import { QueenBee as QueenBee8, EffectBus as EffectBus8 } from "@hypercomb/core";
var HelpQueenBee = class extends QueenBee8 {
  ...
  execute(_args) {
    ...
    if (queens.length === 0) {
      EffectBus8.emit("queen:help", { commands: [] });
...
import { QueenBee as QueenBee10, EffectBus as EffectBus9, hypercomb as hypercomb2 } from "@hypercomb/core";
  ...
  async execute(args) {
    ...
    if (selectedLabels.length > 0 && lineage) {
      ...
      if (dir) {
        ...
        if (updates.length > 0) {
          EffectBus9.emit("tags:changed", { updates });
        }
      }
    }
    if (registry) {
...
import { QueenBee as QueenBee12, EffectBus as EffectBus10 } from "@hypercomb/core";
...
import { QueenBee as QueenBee14, EffectBus as EffectBus12, SignatureService as SignatureService2, hypercomb as hypercomb4 } from "@hypercomb/core";
...
var ViewCurrentQueenBee = class extends QueenBee17 {
  ...
  async execute(args) {
    ...
    console.log(tree);
    EffectBus15.emit("queen:view-current", { path, layer: tree });
  }
};
