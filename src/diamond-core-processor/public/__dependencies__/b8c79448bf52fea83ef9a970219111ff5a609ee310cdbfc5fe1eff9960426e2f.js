...
var OrderProjection = class {
  ...
  #currentSig = null;
  constructor() {
    EffectBus4.on("cell:added", (payload) => {
    ...
    EffectBus4.on("cell:removed", (payload) => {
      if (!payload?.cell || !this.#currentSig) return;
      const order = this.#cache.get(this.#currentSig);
      if (order) {
    ...
    EffectBus4.on("cell:reorder", (payload) => {
...
import { QueenBee as QueenBee2, EffectBus as EffectBus5 } from "@hypercomb/core";
var ReviseQueenBee = class extends QueenBee2 {
  ...
  #enter() {
    this.#active = true;
    EffectBus5.emit("revise:mode-changed", { active: true });
  ...
  #exit() {
    ...
    this.#active = false;
    EffectBus5.emit("revise:mode-changed", { active: false });
