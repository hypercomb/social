...
import { EffectBus as EffectBus3, SignatureService } from "@hypercomb/core";
...
import { EffectBus as EffectBus5, SignatureService as SignatureService2 } from "@hypercomb/core";
...
var HistoryService = class _HistoryService {
  ...
  sign = async (lineage) => {
    ...
    return sigStore ? await sigStore.signText(key) : await SignatureService2.sign(new TextEncoder().encode(key).buffer);
  };
  /**
  ...
  commitLayer = async (locationSig, layer) => {
    ...
    const layerSig = await SignatureService2.sign(bytes.buffer);
    ...
    EffectBus5.emit("content:wrote", { sig: layerSig, kind: "layer", bytes: bytes.buffer });
    return layerSig;
  };
  /**
   ...
   */
    ...
    const sig = await SignatureService2.sign(bytes.buffer);
  ...
  latestMarkerSigFor = async (lineageSig, name) => {
    ...
    const sig = await SignatureService2.sign(bytes);
  ...
  listLayers = async (locationSig) => {
    ...
    for await (const [name, handle] of bag.entries()) {
      ...
      try {
        ...
        const layerSig = await SignatureService2.sign(bytes);
  ...
  #nextMarkerName = async (bag) => {
    ...
    try {
      ...
      for await (const [name, handle] of archive.entries()) {
        if (handle.kind !== "file") continue;
        if (!_HistoryService.#MARKER_RE.test(name)) continue;
        const n = parseInt(name, 10);
        if (!isNaN(n) && n > max) max = n;
    ...
    if (!bytes) {
      ...
      for await (const [name, handle] of bag.entries()) {
        ...
        try {
          ...
          const sig = await SignatureService2.sign(fileBytes);
   ...
   */
  #parsedLayerCache = /* @__PURE__ */ new Map();
  /**
   ...
   */
  currentLayerAt = async (locationSig) => {
    if (!_HistoryService.#SIG_RE.test(locationSig)) return null;
   ...
   */
  archiveEntries = async (locationSig, filenames) => {
    if (filenames.length === 0) return 0;
    let bag;
    try {
      bag = await this.historyRoot.getDirectoryHandle(locationSig, { create: false });
...
var DebugQueenBee = class extends QueenBee5 {
  ...
  execute(_args) {
    ...
    if (dbg && typeof dbg.toggle === "function") {
      ...
      EffectBus6.emit("queen:debug", { active: dbg.active });
    } else {
      console.warn("[debug] PixiDebugDrone not loaded \u2014 no __pixiDebug on window");
      EffectBus6.emit("queen:debug", { active: false, error: "not-loaded" });
    }
  }
};
...
var armImageBlob = async (blob, opts = {}) => {
  ...
  EffectBus7.emit("command:arm-resource", {
    previewUrl,
    largeSig,
    smallPointSig,
    smallFlatSig,
...
var EmptyLongPressInput = class {
  ...
  #attached = false;
  constructor() {
    EffectBus8.on("render:host-ready", (payload) => {
      this.#canvas = payload.canvas;
      this.#container = payload.container;
      this.#renderer = payload.renderer;
      this.#attach();
    });
    EffectBus8.on("render:mesh-offset", (offset) => {
      this.#meshOffset = offset;
    });
    EffectBus8.on("render:set-orientation", ({ flat }) => {
      this.#flat = !!flat;
    });
    EffectBus8.on("render:cell-count", ({ coords }) => {
      this.#occupied.clear();
      if (!coords) return;
  ...
  #onPointerDown = (e) => {
    ...
    this.#holdTimer = setTimeout(() => {
      ...
      EffectBus8.emit("mobile:input-visible", { visible: true, mobile: true });
...
import { QueenBee as QueenBee8, EffectBus as EffectBus9 } from "@hypercomb/core";
var HelpQueenBee = class extends QueenBee8 {
  ...
  execute(_args) {
    ...
    if (queens.length === 0) {
      EffectBus9.emit("queen:help", { commands: [] });
...
import { QueenBee as QueenBee10, EffectBus as EffectBus10, hypercomb as hypercomb2 } from "@hypercomb/core";
  ...
  async execute(args) {
    ...
    if (selectedLabels.length > 0 && lineage) {
      ...
      if (dir) {
        ...
        if (updates.length > 0) {
          EffectBus10.emit("tags:changed", { updates });
        }
      }
    }
    if (registry) {
...
import { QueenBee as QueenBee13, EffectBus as EffectBus11 } from "@hypercomb/core";
var RemoveQueenBee = class extends QueenBee13 {
  ...
  async execute(args) {
    ...
    for (const name of targets) {
      EffectBus11.emit("cell:removed", { cell: name, segments, groupId });
    }
...
EffectBus11.on("controls:action", (payload) => {
  if (payload?.action === "remove") void _remove.invoke("");
});
EffectBus11.on("keymap:invoke", (payload) => {
  if (payload?.cmd === "selection.remove") void _remove.invoke("");
});

// src/diamondcoreprocessor.com/commands/rename.queen.ts
import { QueenBee as QueenBee14, EffectBus as EffectBus12, hypercomb as hypercomb3 } from "@hypercomb/core";
var RenameQueenBee = class extends QueenBee14 {
  ...
  async execute(args) {
    ...
    try {
      ...
      EffectBus12.emit("cell:removed", { cell: oldName, groupId });
      EffectBus12.emit("cell:added", { cell: newName, groupId });
      EffectBus12.emit("cell:renamed", { oldName, newName });
      selection.clear();
...
import { QueenBee as QueenBee15, EffectBus as EffectBus13 } from "@hypercomb/core";
...
var SaveSessionQueenBee = class extends QueenBee15 {
  ...
  async execute(args) {
    ...
    if (trimmed === "auto on" || trimmed === "auto") {
      ...
      EffectBus13.emit("activity:log", { message: "auto-save session on leave: ON", icon: "\u{1F4BE}" });
      return;
    }
    if (trimmed === "auto off") {
      localStorage.removeItem(AUTO_SAVE_KEY);
      EffectBus13.emit("activity:log", { message: "auto-save session on leave: OFF", icon: "\u{1F4BE}" });
      return;
    }
...
import { EffectBus as EffectBus14, SignatureService as SignatureService3, I18N_IOC_KEY as I18N_IOC_KEY3 } from "@hypercomb/core";
...
var TranslationService = class extends EventTarget {
  ...
  constructor() {
    super();
    EffectBus14.on("locale:changed", (payload) => {
      void (async () => {
  ...
  async hydrateCatalog(targetLocale) {
    ...
    if (Object.keys(catalog).length) {
      ...
      if (i18n.locale === targetLocale) {
        EffectBus14.emit("labels:invalidated", { locale: targetLocale });
      }
    }
  }
  // ── sweep internals ─────────────────────────────────
  async #runSweep(targetLocale, opts) {
    ...
    try {
      ...
      if (!apiKey && plan.toTranslate.length) {
        EffectBus14.emit("llm:api-key-required", {});
        return;
      }
      EffectBus14.emit("translation:tile-start", { labels: plan.tileNames, locale: targetLocale });
...
${source}`,
      ...
      for (const tileName of plan.tileNames) {
        ...
        if (!propsSig) {
          EffectBus14.emit("translation:tile-done", { label: tileName });
          continue;
        }
      ...
      EffectBus14.emit("translation:complete", {
        locale: targetLocale,
...
var VerifyHistoryQueenBee = class extends QueenBee17 {
  namespace = "diamondcoreprocessor.com";
  ...
  async execute(_args) {
    ...
    EffectBus15.emit("visibility:show-hidden", { active: true });
    ...
    try {
      ...
      EffectBus15.emit("visibility:show-hidden", { active: originalShowHidden });
    }
  ...
  #seekAndAwaitRender = async (cursor, position) => {
    return new Promise((resolve) => {
      ...
      unsub = EffectBus15.on("render:cell-count", (payload) => {
...
var ViewCurrentQueenBee = class extends QueenBee18 {
  namespace = "diamondcoreprocessor.com";
  ...
  async execute(args) {
    ...
    if (!rootLayer) {
      ...
      EffectBus16.emit("queen:view-current", { path, layer: null });
      return;
    }
    const SIG_RE2 = /^[a-f0-9]{64}$/;
    const expandLayerNode = async (layer, depth) => {
      ...
      const expandedChildren = await Promise.all(
        rawChildren.map(async (entry) => {
          if (typeof entry !== "string" || !SIG_RE2.test(entry)) return entry;
    ...
    console.log(tree);
    EffectBus16.emit("queen:view-current", { path, layer: tree });
  }
};
...
import { QueenBee as QueenBee19 } from "@hypercomb/core";
var VIEW_MODE_KEY = "@hypercomb.social/ViewMode";
var ViewQueenBee = class extends QueenBee19 {
  namespace = "diamondcoreprocessor.com";
  command = "view";
...
async function ensureInstructionsBootstrap() {
  ...
  EffectBus17.emit("cell:added", { cell: "instructions", segments: [] });
  ...
  for (const d of INSTRUCTIONS_DEFAULTS) {
    ...
    EffectBus17.emit("cell:added", { cell: d.name, segments: ["instructions"] });
...
var toast2 = (type, title, message) => {
  try {
    EffectBus17.emit("toast:show", { type, title, message });
  } catch {
  }
};
...
var WebsiteQueenBee = class extends QueenBee20 {
  namespace = "diamondcoreprocessor.com";
  command = "website";
  aliases = [];
  ...
  execute(args) {
    ...
    if (trimmed === "upgrade" || trimmed.startsWith("upgrade ") || trimmed.startsWith("upgrade	")) {
      ...
      EffectBus17.emit("website:build", {
        mode: "upgrade",
        scope,
        scopeName,
