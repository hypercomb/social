...
var PairedChannelDrone = class extends Drone {
  ...
  async #maybeAutoApprove(channelId, share) {
    ...
    if (share.requesterPubkey !== hostPubkey) return;
    const myPubkey = await this.#myPubkey();
    if (!myPubkey) return;
    if (myPubkey !== hostPubkey) return;
  ...
  async materialiseFromSig(channelId, sig, parentDir, opts = {}) {
    ...
    const missing = [];
    let written = 0;
    const walk = async (s, dir, parentSegments) => {
      ...
      if (!existed) {
        ...
        const existing = await readCellProperties2(cellDir).catch(() => ({}));
      ...
      written++;
      if (!existed) {
        EffectBus2.emit("cell:added", { cell: content.name, segments: [...parentSegments] });
      }
...
var EXPOSE_ICON_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 4v10"/><path d="M8 8l4-4 4 4"/><path d="M5 14v6h14v-6"/></svg>`;
var SYNC_ICON_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20v-10"/><path d="M16 16l-4 4-4-4"/><path d="M5 4v6h14V4"/></svg>`;
var MERGE_ICON_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 4l4 6"/><path d="M19 4l-4 6"/><path d="M12 10v10"/><path d="M8 16l4 4 4-4"/></svg>`;
var ExposeDrone = class extends Drone2 {
   ...
   * than misbehaving.
   */
  async #onUnlockTile(tileLabel, mode = "create") {
    const lineage = window.ioc.get("@hypercomb.social/Lineage");
    const dir = await lineage?.explorerDir?.();
    if (!dir) {
    ...
    try {
      ...
      this.#toast("warning", `${mode === "merge" ? "Merge" : "Sync"} failed`, String(err?.message ?? err));
      return;
    }
    try {
