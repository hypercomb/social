...
// src/diamondcoreprocessor.com/substrate/substrate-organizer.drone.ts
import { EffectBus as EffectBus13 } from "@hypercomb/core";
...
function stringify(token2) {
  if (typeof token2 === "string") {
    return token2;
  }
  if (Array.isArray(token2)) {
    return `[${token2.map(stringify).join(", ")}]`;
  }
  if (token2 == null) {
    return "" + token2;
  }
  const name = token2.overriddenName || token2.name;
  if (name) {
    return `${name}`;
  }
  const result = token2.toString();
  if (result == null) {
    return "" + result;
  }
...
function emitProviderConfiguredEvent(eventProvider, isViewProvider = false) {
  ...
  if (typeof eventProvider === "function") {
    token2 = eventProvider;
  } else if (eventProvider instanceof InjectionToken) {
    token2 = eventProvider;
  } else {
    token2 = resolveForwardRef(eventProvider.provide);
  }
...
function emitInjectorToCreateInstanceEvent(token2) {
  !ngDevMode && throwError("Injector profiler should never be called in production mode");
  injectorProfiler({
    type: 5,
    context: getInjectorProfilerContext(),
...
function emitInjectEvent(token2, value, flags) {
  !ngDevMode && throwError("Injector profiler should never be called in production mode");
  injectorProfiler({
    type: 0,
    context: getInjectorProfilerContext(),
...
function runInInjectorProfilerContext(injector, token2, callback) {
...
function cyclicDependencyError(token2, path) {
  const message = ngDevMode ? `Circular dependency detected for \`${token2}\`.` : "";
  return createRuntimeError(message, -200, path);
}
function cyclicDependencyErrorWithDetails(token2, path) {
  return augmentRuntimeError(cyclicDependencyError(token2, path), null);
}
...
function throwProviderNotFoundError(token2, injectorName) {
  const errorMessage = ngDevMode && `No provider for ${stringifyForError(token2)} found${injectorName ? ` in ${injectorName}` : ""}`;
  throw new RuntimeError(-201, errorMessage);
}
function prependTokenToDependencyPath(error, token2) {
  error[NG_TOKEN_PATH] ??= [];
  const currentPath = error[NG_TOKEN_PATH];
  let pathStr;
  if (typeof token2 === "object" && "multi" in token2 && token2?.multi === true) {
    assertDefined(token2.provide, "Token with multi: true should have a provide property");
    pathStr = stringifyForError(token2.provide);
  } else {
    pathStr = stringifyForError(token2);
  }
...
function injectRootLimpMode(token2, notFoundValue, flags) {
  const injectableDef = getInjectableDef(token2);
  if (injectableDef && injectableDef.providedIn == "root") {
    return injectableDef.value === void 0 ? injectableDef.value = injectableDef.factory() : injectableDef.value;
  }
  if (flags & 8) return null;
  if (notFoundValue !== void 0) return notFoundValue;
  throwProviderNotFoundError(token2, typeof ngDevMode !== "undefined" && ngDevMode ? "Injector" : "");
}
...
var RetrievingInjector = class {
  ...
  retrieve(token2, options) {
    const flags = convertToBitFlags(options) || 0;
    try {
      return this.injector.get(token2, flags & 8 ? null : THROW_IF_NOT_FOUND, flags);
    } catch (e) {
      if (isNotFound(e)) {
        return e;
      }
...
function injectInjectorOnly(token2, flags = 0) {
  const currentInjector = getCurrentInjector();
  if (currentInjector === void 0) {
    throw new RuntimeError(-203, ngDevMode && `The \`${stringify(token2)}\` token injection failed. \`inject()\` function must be called from an injection context such as a constructor, a factory function, a field initializer, or a function used with \`runInInjectionContext\`.`);
  } else if (currentInjector === null) {
    return injectRootLimpMode(token2, void 0, flags);
  } else {
    const options = convertToInjectOptions(flags);
    const value = currentInjector.retrieve(token2, options);
    ngDevMode && emitInjectEvent(token2, value, flags);
    if (isNotFound(value)) {
      if (options.optional) {
        return null;
      }
...
function \u0275\u0275inject(token2, flags = 0) {
  return (getInjectImplementation() || injectInjectorOnly)(resolveForwardRef(token2), flags);
}
...
function inject2(token2, options) {
  return \u0275\u0275inject(token2, convertToBitFlags(options));
}
...
function getInjectFlag(token2) {
  return token2[DI_DECORATOR_FLAG];
}
...
var NullInjector = class {
  get(token2, notFoundValue = THROW_IF_NOT_FOUND) {
    if (notFoundValue === THROW_IF_NOT_FOUND) {
      const message = ngDevMode ? `No provider found for \`${stringify(token2)}\`.` : "";
...
var R3Injector = class extends EnvironmentInjector {
  ...
  retrieve(token2, options) {
    const flags = convertToBitFlags(options) || 0;
    try {
      return this.get(token2, THROW_IF_NOT_FOUND, flags);
    } catch (e) {
      if (isNotFound(e)) {
        return e;
      }
  ...
  get(token2, notFoundValue = THROW_IF_NOT_FOUND, options) {
    assertNotDestroyed(this);
    if (token2.hasOwnProperty(NG_ENV_ID)) {
      return token2[NG_ENV_ID](this);
    }
    ...
    try {
      if (!(flags & 4)) {
        let record = this.records.get(token2);
        if (record === void 0) {
          const def = couldBeInjectableType(token2) && getInjectableDef(token2);
          if (def && this.injectableDefInScope(def)) {
            if (ngDevMode) {
              runInInjectorProfilerContext(this, token2, () => {
                emitProviderConfiguredEvent(token2);
              });
            }
            record = makeRecord(injectableDefOrInjectorDefFactory(token2), NOT_YET);
          } else {
            record = null;
          }
          this.records.set(token2, record);
        }
        if (record != null) {
          return this.hydrate(token2, record, flags);
        }
      }
      ...
      return nextInjector.get(token2, notFoundValue);
    } catch (error) {
      const errorCode = getRuntimeErrorCode(error);
      if (errorCode === -200 || errorCode === -201) {
        if (ngDevMode) {
          prependTokenToDependencyPath(error, token2);
          if (previousInjector) {
            throw error;
          } else {
  ...
  toString() {
    if (ngDevMode) {
      ...
      for (const token2 of records.keys()) {
        tokens.push(stringify(token2));
      }
  ...
  processProvider(provider) {
    ...
    let token2 = isTypeProvider(provider) ? provider : resolveForwardRef(provider && provider.provide);
    const record = providerToRecord(provider);
    if (ngDevMode) {
      runInInjectorProfilerContext(this, token2, () => {
        if (isValueProvider(provider)) {
          emitInjectorToCreateInstanceEvent(token2);
    ...
    if (!isTypeProvider(provider) && provider.multi === true) {
      let multiRecord = this.records.get(token2);
      if (multiRecord) {
        ...
        this.records.set(token2, multiRecord);
      }
      token2 = provider;
      multiRecord.multi.push(provider);
    } else {
      if (ngDevMode) {
        const existing = this.records.get(token2);
  ...
  hydrate(token2, record, flags) {
    const prevConsumer = setActiveConsumer(null);
    try {
      if (record.value === CIRCULAR) {
        throw cyclicDependencyError(ngDevMode ? stringify(token2) : "");
...
function injectableDefOrInjectorDefFactory(token2) {
  const injectableDef = getInjectableDef(token2);
  const factory = injectableDef !== null ? injectableDef.factory : getFactoryDef(token2);
  if (factory !== null) {
    return factory;
  }
  if (token2 instanceof InjectionToken) {
    throw new RuntimeError(-204, ngDevMode && `Token ${stringify(token2)} is missing a \u0275prov definition.`);
  }
  if (token2 instanceof Function) {
    return getUndecoratedInjectableFactory(token2);
  }
  throw new RuntimeError(-204, ngDevMode && "unreachable");
}
function getUndecoratedInjectableFactory(token2) {
  const paramLength = token2.length;
  if (paramLength > 0) {
    throw new RuntimeError(-204, ngDevMode && `Can't resolve all parameters for ${stringify(token2)}: (${newArray(paramLength, "?").join(", ")}).`);
  }
  const inheritedInjectableDef = getInheritedInjectableDef(token2);
  if (inheritedInjectableDef !== null) {
    return () => inheritedInjectableDef.factory(token2);
  } else {
    return () => new token2();
  }
}
...
function signal(initialValue, options) {
  const [get3, set, update] = createSignal(initialValue, options?.equal);
  const signalFn = get3;
...
var Inject = attachInjectFlag(makeParamDecorator("Inject", (token2) => ({
  token: token2
})), -1);
...
function diPublicInInjector(injectorIndex, tView, token2) {
  bloomAdd(injectorIndex, tView, token2);
}
...
function notFoundValueOrThrow(notFoundValue, token2, flags) {
  if (flags & 8 || notFoundValue !== void 0) {
    return notFoundValue;
  } else {
    throwProviderNotFoundError(token2, "NodeInjector");
  }
}
function lookupTokenUsingModuleInjector(lView, token2, flags, notFoundValue) {
  if (flags & 8 && notFoundValue === void 0) {
    notFoundValue = null;
  }
  if ((flags & (2 | 1)) === 0) {
    ...
    try {
      if (moduleInjector) {
        return moduleInjector.get(token2, notFoundValue, flags & 8);
      } else {
        return injectRootLimpMode(token2, notFoundValue, flags & 8);
      }
    } finally {
      setInjectImplementation(previousInjectImplementation);
    }
  }
  return notFoundValueOrThrow(notFoundValue, token2, flags);
}
function getOrCreateInjectable(tNode, lView, token2, flags = 0, notFoundValue) {
  if (tNode !== null) {
    if (lView[FLAGS] & 2048 && !(flags & 2)) {
      const embeddedInjectorValue = lookupTokenUsingEmbeddedInjector(tNode, lView, token2, flags, NOT_FOUND2);
      if (embeddedInjectorValue !== NOT_FOUND2) {
        return embeddedInjectorValue;
      }
    }
    const value = lookupTokenUsingNodeInjector(tNode, lView, token2, flags, NOT_FOUND2);
    if (value !== NOT_FOUND2) {
      return value;
    }
  }
  return lookupTokenUsingModuleInjector(lView, token2, flags, notFoundValue);
}
function lookupTokenUsingNodeInjector(tNode, lView, token2, flags, notFoundValue) {
  const bloomHash = bloomHashBitOrFactory(token2);
  if (typeof bloomHash === "function") {
    if (!enterDI(lView, tNode, flags)) {
      return flags & 1 ? notFoundValueOrThrow(notFoundValue, token2, flags) : lookupTokenUsingModuleInjector(lView, token2, flags, notFoundValue);
    }
    try {
      let value;
      if (ngDevMode) {
        runInInjectorProfilerContext(new NodeInjector(getCurrentTNode(), getLView()), token2, () => {
          emitInjectorToCreateInstanceEvent(token2);
      ...
      if (value == null && !(flags & 8)) {
        throwProviderNotFoundError(token2);
      } else {
        return value;
      }
    } finally {
    ...
    while (injectorIndex !== -1) {
      ...
      if (bloomHasToken(bloomHash, injectorIndex, tView.data)) {
        const instance = searchTokensOnInjector(injectorIndex, lView, token2, previousTView, flags, hostTElementNode);
...
function searchTokensOnInjector(injectorIndex, lView, token2, previousTView, flags, hostTElementNode) {
  ...
  const injectableIdx = locateDirectiveOrProvider(tNode, currentTView, token2, canAccessViewProviders, isHostSpecialCase);
...
function locateDirectiveOrProvider(tNode, tView, token2, canAccessViewProviders, isHostSpecialCase) {
  ...
  for (let i = startingIndex; i < endIndex; i++) {
    ...
    if (i < directivesStart && token2 === providerTokenOrDef || i >= directivesStart && providerTokenOrDef.type === token2) {
      return i;
    }
  }
  if (isHostSpecialCase) {
    const dirDef = tInjectables[directivesStart];
    if (dirDef && isComponentDef(dirDef) && dirDef.type === token2) {
...
function getNodeInjectable(lView, tView, index, tNode, flags) {
  ...
  if (value instanceof NodeInjectorFactory) {
    ...
    if (factory.resolving) {
      let token3 = "";
      if (ngDevMode) {
        token3 = stringifyForError(tData[index]);
        throw cyclicDependencyErrorWithDetails(token3, injectionPath);
      } else {
        throw cyclicDependencyError(token3);
      }
    }
    const previousIncludeViewProviders = setIncludeViewProviders(factory.canSeeViewProviders);
    factory.resolving = true;
    const token2 = tData[index].type || tData[index];
    let prevInjectContext;
    if (ngDevMode) {
    ...
    try {
      ngDevMode && emitInjectorToCreateInstanceEvent(token2);
...
function bloomHashBitOrFactory(token2) {
  ngDevMode && assertDefined(token2, "token must be defined");
  if (typeof token2 === "string") {
    return token2.charCodeAt(0) || 0;
  }
  const tokenId = token2.hasOwnProperty(NG_ELEMENT_ID) ? token2[NG_ELEMENT_ID] : void 0;
...
var NodeInjector = class {
  ...
  get(token2, notFoundValue, flags) {
    return getOrCreateInjectable(this._tNode, this._lView, token2, convertToBitFlags(flags), notFoundValue);
  }
};
...
function lookupTokenUsingEmbeddedInjector(tNode, lView, token2, flags, notFoundValue) {
  let currentTNode = tNode;
  let currentLView = lView;
  while (currentTNode !== null && currentLView !== null && currentLView[FLAGS] & 2048 && !isRootView(currentLView)) {
    ngDevMode && assertTNodeForLView(currentTNode, currentLView);
    const nodeInjectorValue = lookupTokenUsingNodeInjector(currentTNode, currentLView, token2, flags | 2, NOT_FOUND2);
    ...
    if (!parentTNode) {
      ...
      if (embeddedViewInjector) {
        const embeddedViewInjectorValue = embeddedViewInjector.get(token2, NOT_FOUND2, flags & ~4);
...
var ChainedInjector = class {
  ...
  get(token2, notFoundValue, options) {
    const value = this.injector.get(token2, NOT_FOUND_CHECK_ONLY_ELEMENT_INJECTOR, options);
    if (value !== NOT_FOUND_CHECK_ONLY_ELEMENT_INJECTOR || notFoundValue === NOT_FOUND_CHECK_ONLY_ELEMENT_INJECTOR) {
      return value;
    }
    return this.parentInjector.get(token2, notFoundValue, options);
  }
};
...
function \u0275\u0275directiveInject(token2, flags = 0) {
  const lView = getLView();
  if (lView === null) {
    ngDevMode && assertInjectImplementationNotEqual(\u0275\u0275directiveInject);
    return \u0275\u0275inject(token2, flags);
  }
  const tNode = getCurrentTNode();
  const value = getOrCreateInjectable(tNode, lView, resolveForwardRef(token2), flags);
  ngDevMode && emitInjectEvent(token2, value, flags);
  return value;
}
...
function getDependenciesFromInjectable(injector, token2) {
  const instance = injector.get(token2, null, {
    self: true,
    optional: true
  });
  if (instance === null) {
    throw new Error(`Unable to determine instance of ${token2} in given injector`);
  }
  const unformattedDependencies = getDependenciesForTokenInInjector(token2, injector);
...
function getDependenciesForTokenInInjector(token2, injector) {
  const {
    resolverToTokenToDependencies
  } = getFrameworkDIDebugData();
  if (!(injector instanceof NodeInjector)) {
    return resolverToTokenToDependencies.get(injector)?.get?.(token2) ?? [];
  }
  const lView = getNodeInjectorLView(injector);
  const tokenDependencyMap = resolverToTokenToDependencies.get(lView);
  const dependencies = tokenDependencyMap?.get(token2) ?? [];
...
function getEnvironmentInjectorProviders(injector) {
  ...
  for (const providerRecord of providerRecordsWithoutImportPaths) {
    ...
    const token2 = provider.provide;
    if (token2 === ENVIRONMENT_INITIALIZER || token2 === INJECTOR_DEF_TYPES) {
      continue;
    }
...
var chromeDevToolsInjectorProfiler = (event) => {
  ...
  if (eventType === 5) {
    ...
    const token2 = event.context.token;
    measureEnd(100, getProviderTokenMeasureName(token2), "tertiary-dark");
  }
};
...
function getProviderTokenMeasureName(token2) {
  if (isTypeProvider(token2)) {
    return token2.name;
  } else if (token2.provide != null) {
    return getProviderTokenMeasureName(token2.provide);
  }
...
var Testability = class _Testability {
  ...
  registerApplication(token2) {
    this.registry.registerApplication(token2, this);
  }
  unregisterApplication(token2) {
    this.registry.unregisterApplication(token2);
  }
...
var TestabilityRegistry = class _TestabilityRegistry {
  ...
  registerApplication(token2, testability) {
    this._applications.set(token2, testability);
  }
  unregisterApplication(token2) {
    this._applications.delete(token2);
  }
...
function resolveProvider(provider, tInjectables, lInjectablesBlueprint, isComponent2, isViewProvider) {
  ...
  if (Array.isArray(provider)) {
    ...
    const lView = getLView();
    const tNode = getCurrentTNode();
    let token2 = isTypeProvider(provider) ? provider : resolveForwardRef(provider.provide);
    const providerFactory = providerToFactory(provider);
    if (ngDevMode) {
      const injector = new NodeInjector(tNode, lView);
      runInInjectorProfilerContext(injector, token2, () => {
    ...
    if (isTypeProvider(provider) || !provider.multi) {
      ...
      const existingFactoryIndex = indexOf(token2, tInjectables, isViewProvider ? beginIndex : beginIndex + cptViewProvidersCount, endIndex);
      if (existingFactoryIndex === -1) {
        diPublicInInjector(getOrCreateNodeInjectorForNode(tNode, lView), tView, token2);
        registerDestroyHooksIfSupported(tView, provider, tInjectables.length);
        tInjectables.push(token2);
      ...
      const existingProvidersFactoryIndex = indexOf(token2, tInjectables, beginIndex + cptViewProvidersCount, endIndex);
      const existingViewProvidersFactoryIndex = indexOf(token2, tInjectables, beginIndex, beginIndex + cptViewProvidersCount);
...
var _inputRef_dec, _CommandShellComponent_decorators, _init;
_CommandShellComponent_decorators = [Component({
  selector: "hc-command-shell",
  standalone: true,
  templateUrl: "./command-shell.component.html",
  styleUrls: ["./command-shell.component.scss"]
})], _inputRef_dec = [ViewChild("shellInput", { read: ElementRef })];
var CommandShellComponent = class {
  constructor() {
    __publicField(this, "inputRef", __runInitializers(_init, 8, this)), __runInitializers(_init, 11, this);
    // ── inputs from parent ──────────────────────────────────
    /** Filtered suggestion list to display in the dropdown. */
    __publicField(this, "suggestions", input([]));
    /** Placeholder text when input is empty. */
    __publicField(this, "placeholder", input(""));
    /** Full ghost text (overlaid as dim autocomplete hint). */
    __publicField(this, "ghostValue", input(""));
    /** Whether to show the suggestion dropdown. Parent controls this. */
    __publicField(this, "showSuggestions", input(false));
    /** Prefix of each suggestion that the user has typed (for highlight split). */
    __publicField(this, "typedPrefix", input(""));
    /** Optional descriptions keyed by suggestion name (shown right-aligned). */
    __publicField(this, "descriptionMap", input(/* @__PURE__ */ new Map()));
    /** Optional color swatches keyed by suggestion name (CSS color string). */
    __publicField(this, "colorMap", input(/* @__PURE__ */ new Map()));
    /** Active status indicators shown as pills on the right side of the input. */
    __publicField(this, "indicators", input([]));
    // ── outputs to parent ───────────────────────────────────
    /** Emitted on every input change (after leading-space strip). */
    __publicField(this, "valueChange", output());
    /** Emitted when Enter is pressed (not Shift+Enter). */
    __publicField(this, "commit", output());
    /** Emitted when a suggestion is accepted via Tab/ArrowRight/click. */
    __publicField(this, "completionAccepted", output());
    /**
     ...
     */
    __publicField(this, "shellKeydown", output());
    /** Emitted when an indicator pill is clicked (to turn it off). */
    __publicField(this, "indicatorDismiss", output());
    // ── internal state ────────────────────────────────���─────
    __publicField(this, "value", signal(""));
    __publicField(this, "activeIndex", signal(0));
    __publicField(this, "suppressed", signal(false));
    __publicField(this, "effectiveShowCompletions", computed(
      () => this.showSuggestions() && this.suggestions().length > 0 && !this.suppressed()
    ));
    // ── public API for parent ───────────────────────────────
    __publicField(this, "getActiveIndex", () => this.activeIndex());
    // ── template helpers ────────────────────────────────────
    __publicField(this, "typedPart", (suggestion) => {
      const prefix = this.typedPrefix();
      if (!prefix) return "";
      return suggestion.slice(0, Math.min(prefix.length, suggestion.length));
    });
    __publicField(this, "restPart", (suggestion) => {
      const prefix = this.typedPrefix();
      if (!prefix) return suggestion;
      return suggestion.slice(Math.min(prefix.length, suggestion.length));
    });
    __publicField(this, "descriptionFor", (suggestion) => {
      return this.descriptionMap().get(suggestion) ?? "";
    });
    __publicField(this, "colorFor", (suggestion) => {
    ...
    __publicField(this, "onShellMouseDown", (e) => {
      if (e.target === this.inputElement) return;
      e.preventDefault();
      this.inputElement?.focus();
    });
    __publicField(this, "onSuggestionMouseDown", (e, suggestion, index) => {
      e.preventDefault();
  ...
  handleCompletionKeys(e) {
    const list2 = this.suggestions();
    if (!list2.length || this.suppressed()) return false;
    if (e.key === "Escape") {
      e.preventDefault();
      this.suppressed.set(true);
      return true;
    ...
    if (e.key === "ArrowDown") {
      e.preventDefault();
      this.activeIndex.update((v) => Math.min(v + 1, list2.length - 1));
      return true;
    }
    if (e.key === "ArrowUp") {
      e.preventDefault();
    ...
    if (e.key === "Tab" || e.key === "ArrowRight") {
      e.preventDefault();
      const best = list2[this.activeIndex()] ?? list2[0];
...
__decorateElement(_init, 5, "inputRef", _inputRef_dec, CommandShellComponent);
CommandShellComponent = __decorateElement(_init, 0, "CommandShellComponent", _CommandShellComponent_decorators, CommandShellComponent);
__runInitializers(_init, 1, CommandShellComponent);

// ../hypercomb-shared/ui/hint-bar/hint-bar.component.ts
var _HintBarComponent_decorators, _init2;
_HintBarComponent_decorators = [Component({
  selector: "hc-hint-bar",
  standalone: true,
  templateUrl: "./hint-bar.component.html",
  styleUrls: ["./hint-bar.component.scss"]
})];
var HintBarComponent = class {
  ...
  chosen = input(/* @__PURE__ */ new Set());
  /** Optional color swatches keyed by item name (CSS color string). */
  colorMap = input(/* @__PURE__ */ new Map());
  ...
  isMatched = (item) => this.matched().has(item);
  isChosen = (item) => this.chosen().has(item);
  colorFor = (item) => this.colorMap().get(item) ?? "";
  onPick(item, event) {
...
HintBarComponent = __decorateElement(_init2, 0, "HintBarComponent", _HintBarComponent_decorators, HintBarComponent);
__runInitializers(_init2, 1, HintBarComponent);

// ../hypercomb-shared/core/from-runtime.ts
function fromRuntime(target, getter, event = "change") {
  const initial = target ? getter() : void 0;
...
var TAG_PROPS_FILE = "0000";
async function readTagProps(cellDir) {
  try {
...
async function writeTagProps(cellDir, updates) {
...
async function persistTagOps(ops, dir) {
...
import { EffectBus as EffectBus7, hypercomb as hypercomb6 } from "@hypercomb/core";

// ../hypercomb-shared/core/i18n.pipe.ts
var _TranslatePipe_decorators, _init3;
_TranslatePipe_decorators = [Pipe({ name: "t", standalone: true, pure: false })];
...
TranslatePipe = __decorateElement(_init3, 0, "TranslatePipe", _TranslatePipe_decorators, TranslatePipe);
__runInitializers(_init3, 1, TranslatePipe);

// ../hypercomb-shared/core/voice-input.service.ts
import { EffectBus as EffectBus2 } from "@hypercomb/core";
var VoiceInputService = class _VoiceInputService extends EventTarget {
  #recognition = null;
  #active = false;
  #finalText = "";
  #wantActive = false;
  ...
  start() {
    ...
    if (!_VoiceInputService.supported()) return;
    ...
    this.#recognition.onstart = () => {
      this.#active = true;
      EffectBus2.emit("voice:active", { active: true });
    ...
    this.#recognition.onresult = (event) => {
      ...
      if (preview) {
        EffectBus2.emit("voice:interim", { text: preview });
      }
    };
    this.#recognition.onerror = (event) => {
      if (event.error !== "no-speech" && event.error !== "aborted") {
        EffectBus2.emit("voice:error", { message: event.error });
      }
    };
  ...
  stop() {
    ...
    if (text) {
      EffectBus2.emit("voice:final", { text });
      EffectBus2.emit("voice:submit", { text });
    }
    this.#cleanup();
  }
  #cleanup() {
    ...
    EffectBus2.emit("voice:active", { active: false });
    this.dispatchEvent(new CustomEvent("change"));
  }
};
var _voiceInput = new VoiceInputService();
...
var ShiftEnterNavigateBehavior = class {
  name = "shift-enter-navigate";
  operations = [
    {
      trigger: "Shift+Enter",
      pattern: /^.+$/,
  ...
  match(event, input2) {
    return event.key === "Enter" && event.shiftKey && input2.length > 0;
  }
  async execute(input2) {
    const completions = get("@hypercomb.social/CompletionUtility");
    const lineage = get("@hypercomb.social/Lineage");
    const navigation = get("@hypercomb.social/Navigation");
    const parts = input2.replace(/\/+$/, "").split("/").map((s) => completions.normalize(s.trim())).filter(Boolean);
...
import { EffectBus as EffectBus3, hypercomb as hypercomb2 } from "@hypercomb/core";

// ../hypercomb-shared/core/array-parser.ts
function parseArrayItems(raw, normalize) {
  const items = [];
...
function parseOneItem(raw, normalize) {
  ...
  const segments = pathPart.split("/").map((s) => normalize(s.trim())).filter(Boolean);
...
var BatchCreateBehavior = class {
  ...
  match(event, input2) {
    return event.key === "Enter" && !event.shiftKey && input2.includes("[") && input2.includes("]");
  }
  async execute(input2) {
    const completions = get("@hypercomb.social/CompletionUtility");
    const lineage = get("@hypercomb.social/Lineage");
    ...
    if (!dir) return;
    const expanded = this.#expand(input2, completions);
    if (!expanded.length) return;
    const topCells = /* @__PURE__ */ new Set();
    const tagOps = [];
    for (const item of expanded) {
      if (item.op === "delete") {
        await this.#deleteTarget(dir, item.segments);
        if (item.segments[0]) {
          EffectBus3.emit("cell:removed", { cell: item.segments[0] });
        }
    ...
    for (const cell of topCells) {
      EffectBus3.emit("cell:added", { cell });
    }
    await new hypercomb2().act();
  }
  /**
  ...
  async #deleteTarget(root, segments) {
...
import { EffectBus as EffectBus4, hypercomb as hypercomb3 } from "@hypercomb/core";
var RemoveCellBehavior = class {
  name = "remove-cell";
  operations = [
    {
      trigger: "Enter",
  ...
  match(event, input2) {
    return event.key === "Enter" && !event.shiftKey && input2.startsWith("~");
  }
  async execute(input2) {
    const completions = get("@hypercomb.social/CompletionUtility");
    const lineage = get("@hypercomb.social/Lineage");
    const body = input2.slice(1).trim();
    if (!body) return;
    const dir = await lineage.explorerDir();
    if (!dir) return;
    const bracketMatch = body.match(/^\[(.+)\]$/);
    const items = bracketMatch ? parseArrayItems(bracketMatch[1], completions.normalize) : (() => {
  ...
  async #removeTarget(root, segments) {
    ...
    try {
      await parent.removeEntry(name, { recursive: true });
      EffectBus4.emit("cell:removed", { cell: name });
    } catch {
    }
  }
};
...
var GoParentBehavior = class {
  ...
  match(event, input2) {
    return event.key === "Enter" && !event.shiftKey && /^\.\.(?:\/|$)/.test(input2);
  }
  execute(input2) {
    const navigation = get("@hypercomb.social/Navigation");
    const segments = navigation.segmentsRaw();
    const parts = input2.replace(/\/+\s*$/, "").split("/").filter(Boolean);
    let levels = 0;
...
import { EffectBus as EffectBus5, SignatureService, hypercomb as hypercomb4 } from "@hypercomb/core";

// ../hypercomb-shared/ui/command-line/select-ops.ts
var SELECT_OPS = /* @__PURE__ */ new Set([
  "select",
  "cut",
  "copy",
  "move",
...
var CutPasteBehavior = class {
  ...
  match(event, input2) {
    ...
    const close = input2.indexOf("]");
    if (!(input2.startsWith("[") && close > 1 && close < input2.length - 1 && input2[close + 1] === "/")) return false;
    const afterSlash = input2.slice(close + 2);
  ...
  async execute(input2) {
    ...
    const close = input2.indexOf("]");
    const itemsPart = input2.slice(1, close);
    const pathPart = input2.slice(close + 2);
    ...
    const pathSegments = pathRaw.split("/").map((s) => completions.normalize(s.trim())).filter(Boolean);
  ...
  async #deleteTarget(root, segments) {
    ...
    try {
      await parent.removeEntry(name, { recursive: true });
      EffectBus5.emit("cell:removed", { cell: name });
    } catch {
    }
  }
  async #computeDestSig(lineage, extraSegments) {
    ...
    return await SignatureService.sign(new TextEncoder().encode(key).buffer);
  }
};

// ../hypercomb-shared/ui/command-line/hash-marker.behavior.ts
import { EffectBus as EffectBus6, hypercomb as hypercomb5 } from "@hypercomb/core";
var HashMarkerBehavior = class {
  name = "hash-marker";
  operations = [
    {
      trigger: "Enter",
  ...
  match(event, input2) {
    if (event.key !== "Enter" || event.shiftKey) return false;
    const hashIndex = input2.indexOf("#");
    return hashIndex > 0;
  }
  async execute(input2) {
    const completions = get("@hypercomb.social/CompletionUtility");
    const lineage = get("@hypercomb.social/Lineage");
    const hashIndex = input2.indexOf("#");
    const cellRaw = input2.slice(0, hashIndex).trim();
    const markerRaw = input2.slice(hashIndex + 1).trim();
    ...
    if (!markerRaw) {
      const props2 = await readProps(cellDir);
      const existing2 = Array.isArray(props2["markers"]) ? props2["markers"] : [];
      EffectBus6.emit("marker:list", { cell: cellName, markers: existing2 });
      return;
    }
    ...
    EffectBus6.emit("cell:added", { cell: cellName });
    EffectBus6.emit("marker:added", { cell: cellName, marker });
    await new hypercomb5().act();
  }
};
var PROPS_FILE = "0000";
...
var SlashBehaviourBehavior = class {
  name = "slash-behaviour";
  operations = [
    {
      trigger: "Enter",
      pattern: /^\/.+/,
  ...
  match(event, input2) {
    if (event.key !== "Enter" || event.shiftKey) return false;
    if (!input2.startsWith("/") || input2.length < 2) return false;
    const behaviourName = this.#extractBehaviourName(input2);
    return this.#findQueen(behaviourName) !== null;
  }
  async execute(input2) {
    const behaviourName = this.#extractBehaviourName(input2);
    const args = this.#extractArgs(input2);
  ...
  #extractBehaviourName(input2) {
    const stripped = input2.slice(1).trim();
    const bracketIdx = stripped.indexOf("[");
    const spaceIdx = stripped.indexOf(" ");
    const end = bracketIdx >= 0 && (spaceIdx < 0 || bracketIdx < spaceIdx) ? bracketIdx : spaceIdx >= 0 ? spaceIdx : stripped.length;
...
var TAG_ASSIGN_RE = /^([^:]+):([^(]+)(?:\(([^)]+)\))?$/;
var BRACKET_TAG_RE = /^([^\[\/!#~]+):\[(.+?)\](.*)$/;
var BRACKET_CMD_RE = /^\/(select|format|fmt|fp)\[/i;
function normalizeSelectInput(v) {
  ...
  if (v.startsWith("[")) {
    ...
    if (close > 1 && v[close + 1] === "/") {
      ...
      if (SELECT_OPS.has(firstSeg)) {
        ...
        const tail = firstSeg === "select" ? nextSlash === -1 ? "" : afterSlash.slice(nextSlash) : "/" + afterSlash;
...
var _CommandLineComponent = class _CommandLineComponent {
  constructor() {
    __privateAdd(this, _CommandLineComponent_instances);
    __publicField(this, "shell", __runInitializers(_init4, 8, this)), __runInitializers(_init4, 11, this);
    __publicField(this, "value", signal(""));
    __publicField(this, "cellSubPath", signal([]));
    __publicField(this, "cellLeaf", signal(""));
    /** Tags currently assigned to the cell in bracket-tag mode (for intellisense filtering). */
    __privateAdd(this, _bracketCellTags, signal(/* @__PURE__ */ new Set()));
    __privateAdd(this, _bracketCellLabel, "");
    ...
    __privateAdd(this, _slashMatches, computed(() => {
      const ctx = this.context();
      if (!ctx.active || ctx.mode !== "slash") return [];
      const drone = get("@diamondcoreprocessor.com/SlashBehaviourDrone");
      const droneMatches = drone?.match ? drone.match(ctx.normalized) : [];
      const t = __privateGet(this, _CommandLineComponent_instances, i18n_get);
      const builtinMatches = BUILTIN_SLASH.filter(
        (b) => !ctx.normalized || b.behaviour.name.startsWith(ctx.normalized)
      ).map((b) => ({
    ...
    __publicField(this, "slashDescriptionMap", computed(() => {
      const map2 = /* @__PURE__ */ new Map();
      const ctx = this.context();
      if (!ctx.active || ctx.mode !== "slash") return map2;
      for (const m of __privateGet(this, _slashMatches).call(this)) {
        map2.set(m.behaviour.name, m.behaviour.description);
      }
      return map2;
    }));
    /** Prefix of the current suggestion fragment — used by shell for highlight split. */
    __publicField(this, "completionTypedPrefix", computed(() => {
      const ctx = this.context();
      if (!ctx.active) return "";
      const bracketPhase = __privateGet(this, _bracketPhase).call(this);
    ...
    __publicField(this, "resourceCount$", fromRuntime(
      get("@hypercomb.social/ScriptPreloader"),
      () => this.preloader.resourceCount
    ));
    __publicField(this, "actionNames$", fromRuntime(
      get("@hypercomb.social/ScriptPreloader"),
      () => this.preloader.actionNames
    ));
    __publicField(this, "cellNames$", fromRuntime(
      get("@hypercomb.social/CellSuggestionProvider"),
      () => this.cellProvider.suggestions()
    ));
    // pluggable behaviors — validated at construction, no overlapping operations
    __privateAdd(this, _behaviors, __privateMethod(this, _CommandLineComponent_instances, validateBehaviors_fn).call(this, [
    ...
    // readiness / locking
    // -------------------------------------------------
    __publicField(this, "hasAnyResources", computed(() => this.resourceCount$() > 0));
    __publicField(this, "locked", computed(() => !this.hasAnyResources()));
    __publicField(this, "placeholder", computed(() => {
      const t = __privateGet(this, _CommandLineComponent_instances, i18n_get);
    ...
    // ── status indicators ─────────────────────────────────
    __privateAdd(this, _indicators, signal(/* @__PURE__ */ new Map()));
    __publicField(this, "activeIndicators", computed(() => [...__privateGet(this, _indicators).call(this).values()]));
    __privateAdd(this, _indicatorUnsubs, []);
    __publicField(this, "openDcp", () => {
      window.dispatchEvent(new CustomEvent("portal:open", { detail: { target: "dcp" } }));
    });
    // -------------------------------------------------
    // completion context
    // -------------------------------------------------
    __publicField(this, "context", computed(() => {
      const v = this.value();
      if (v.startsWith(">?")) {
        const raw = v.slice(2);
      ...
      if (isSelectInput(v)) {
        return __privateMethod(this, _CommandLineComponent_instances, parseSelectContext_fn).call(this, normalizeSelectInput(v));
      }
      if (v.startsWith("/")) {
        const raw = v.slice(1);
        const slashArgs = __privateMethod(this, _CommandLineComponent_instances, extractSlashCommandArgs_fn).call(this, raw);
        if (slashArgs !== null) {
    ...
    // suggestions
    // -------------------------------------------------
    __publicField(this, "suggestions", computed(() => {
      ...
      if (ctx.mode === "slash") {
        ...
        return __privateGet(this, _slashMatches).call(this).map((m) => m.behaviour.name);
      }
      if (ctx.mode === "tag") {
        const registry = get("@hypercomb.social/TagRegistry");
      ...
      if (ctx.mode === "select") {
        const phase = __privateGet(this, _selectPhase).call(this);
        if (phase === "selection") {
          let cells2 = this.cellNames$();
          const excluded = __privateGet(this, _selectExcluded).call(this);
          if (excluded.size) cells2 = cells2.filter((n) => !excluded.has(n));
          if (!ctx.normalized) return cells2;
          return cells2.filter((n) => n.startsWith(ctx.normalized));
        }
        if (phase === "operation") {
      ...
      if (ctx.mode === "remove") {
        ...
        let cells2 = this.cellNames$();
        if (already.size) cells2 = cells2.filter((n) => !already.has(n));
        if (!ctx.normalized) return cells2;
        return cells2.filter((n) => n.startsWith(ctx.normalized));
      }
      const bracketPhase = __privateGet(this, _bracketPhase).call(this);
      ...
      if (bracketPhase === "items" || bracketPhase === "path") {
        if (leaf.startsWith(":") || leaf.startsWith("~:")) {
          ...
          const registry = get("@hypercomb.social/TagRegistry");
          const allTags = registry?.names ?? [];
          const cellTags = __privateGet(this, _bracketCellTags).call(this);
          const pending = __privateGet(this, _bracketPendingTags);
          let candidates;
          if (isRemove) {
    ...
    __publicField(this, "showCompletions", computed(() => {
    ...
    __publicField(this, "hintItems", computed(() => {
      const ctx = this.context();
      if (!ctx.active || ctx.mode !== "slash") return [];
      ...
      return _CommandLineComponent.ACCENT_PRESETS;
    }));
    /** Current filter for the hint bar — typed fragment. */
    __publicField(this, "hintFilter", computed(() => {
      const ctx = this.context();
      if (!ctx.active) return "";
      return ctx.normalized;
    }));
    /** Items already chosen in the hint bar. */
    __publicField(this, "hintChosen", computed(() => {
      return /* @__PURE__ */ new Set();
    }));
    /** Accent color map — active when in accent command context. */
    __publicField(this, "accentColorMap", computed(() => {
      const ctx = this.context();
      if (!ctx.active || ctx.mode !== "slash") return /* @__PURE__ */ new Map();
      const isAccent = ctx.head.match(/^\/(accent|ac)[\s\[]/i);
      if (!isAccent) return /* @__PURE__ */ new Map();
      return _CommandLineComponent.ACCENT_COLOR_MAP;
    }));
    // -------------------------------------------------
    // ghost mirror (second input layer)
    // -------------------------------------------------
    __publicField(this, "ghostValue", computed(() => {
      if (!this.suggestions().length) return "";
      const ctx = this.context();
      if (!ctx.active) return "";
      const list2 = this.suggestions();
      const best = list2[this.shell?.activeIndex() ?? 0] ?? list2[0];
      if (!best) return "";
      const subPath = this.cellSubPath();
      const leaf = this.cellLeaf();
      ...
      if (ctx.mode === "select") {
        ...
        if (phase === "operation") {
          ...
          if (bracketClose >= 0) {
            const prefix2 = current.slice(0, bracketClose + 1);
            const op = best.startsWith("/") ? best : "/" + best;
            return prefix2 + op;
          }
        }
        if (!best.startsWith(ctx.normalized) && ctx.normalized) return "";
        const suffix2 = best.slice(ctx.normalized.length);
        if (!suffix2) return "";
        return current + suffix2;
      }
      if (bracketPhase === "items" || bracketPhase === "path") {
        if (leaf.startsWith(":") || leaf.startsWith("~:")) {
          const prefix2 = leaf.startsWith("~:") ? leaf.slice(2) : leaf.slice(1);
          if (!best.startsWith(prefix2)) return "";
          const suffix3 = best.slice(prefix2.length);
          if (!suffix3) return "";
          return current + suffix3;
        }
        if (!best.startsWith(leaf)) return "";
        const suffix2 = best.slice(leaf.length);
        if (!suffix2) return "";
        return current + suffix2;
      }
      if (subPath.length > 0) {
    ...
    __privateAdd(this, _pushToTalkUnsub);
    __privateAdd(this, _prefillUnsub);
    __privateAdd(this, _commandFocusUnsub);
    __privateAdd(this, _commandLineToggleUnsub);
    __privateAdd(this, _touchDraggingUnsub);
    __privateAdd(this, _viewActiveUnsub);
    __privateAdd(this, _selectionSyncUnsub);
    __privateAdd(this, _voiceInterimUnsub);
    __privateAdd(this, _voiceSubmitUnsub);
    __privateAdd(this, _onNavigate, () => {
      this.clear();
    });
    __publicField(this, "startVoice", (event) => {
      ;
      event.target?.setPointerCapture?.(event.pointerId);
      this.voiceService?.start();
    });
    __publicField(this, "stopVoice", () => {
    ...
    __publicField(this, "onShellValueChange", (v) => {
      this.value.set(v);
      if (__privateMethod(this, _CommandLineComponent_instances, autoPopulateMoveIndex_fn).call(this, v)) {
      }
      const ctx = this.context();
      if (ctx.active && ctx.mode === "filter") {
        EffectBus7.emit("search:filter", { keyword: ctx.normalized });
      } else if (this.lastFilterKeyword) {
        EffectBus7.emit("search:filter", { keyword: "" });
      ...
      if (ctx.active && ctx.mode === "select") {
        __privateMethod(this, _CommandLineComponent_instances, handleSelectInputEffects_fn).call(this);
      } else if (__privateGet(this, _lastSelectMode)) {
        __privateSet(this, _syncDirection, "command");
        const selection = get("@diamondcoreprocessor.com/SelectionService");
        if (selection?.count > 0) selection.clear();
        __privateSet(this, _syncDirection, "idle");
      }
      __privateSet(this, _lastSelectMode, ctx.active && ctx.mode === "select");
      this.updateCellSubPath();
    });
    __publicField(this, "lastFilterKeyword", "");
    /** Bridge: shell forwarded a keydown it didn't consume (not Escape/Up/Down/Tab/Enter). */
    __publicField(this, "onShellKeydown", (e) => {
      const v = this.value();
      if (e.key === "Escape" && __privateGet(this, _selectPhase).call(this) !== "none") {
        e.preventDefault();
        __privateMethod(this, _CommandLineComponent_instances, cancelSelectOperation_fn).call(this);
        return;
      }
      if (__privateMethod(this, _CommandLineComponent_instances, isInMoveParen_fn).call(this, v)) {
        if ((e.ctrlKey || e.metaKey) && __privateMethod(this, _CommandLineComponent_instances, handleMoveScrub_fn).call(this, e)) return;
        if (!e.ctrlKey && !e.metaKey && (e.key === "ArrowUp" || e.key === "ArrowDown")) {
          e.preventDefault();
          __privateMethod(this, _CommandLineComponent_instances, scrubMoveIndex_fn).call(this, e.key === "ArrowUp" ? -1 : 1);
          return;
        }
      }
      if ((e.key === "ArrowUp" || e.key === "ArrowDown") && __privateGet(this, _selectPhase).call(this) === "move-target-index") {
        e.preventDefault();
    ...
    __publicField(this, "onShellCommit", (v) => {
      void __privateMethod(this, _CommandLineComponent_instances, preprocessTagsThenExecute_fn).call(this, v);
    });
    // -------------------------------------------------
    // create cell in place
    // -------------------------------------------------
    __publicField(this, "commitCreateCellInPlace", async () => {
      ...
      EffectBus7.emit("cell:added", { cell: parts[0] });
      ...
      if (navigateAfterCreate) {
        ...
        __privateMethod(this, _CommandLineComponent_instances, setShellValue_fn).call(this, prefix + "/", true);
      } else {
        this.clear();
      }
    });
    ...
    // -------------------------------------------------
    __privateAdd(this, _executeSlashBehaviour, async () => {
      ...
      const delimIdx = spaceIdx >= 0 && (parenIdx < 0 || spaceIdx < parenIdx) ? spaceIdx : parenIdx >= 0 ? parenIdx : -1;
    ...
    // -------------------------------------------------
    __privateAdd(this, _executeSelectCommand, async () => {
      ...
      const labels = inner.split(",").map((s) => this.completions.normalize(__privateMethod(this, _CommandLineComponent_instances, stripTagSuffix_fn).call(this, s.trim()))).filter(Boolean);
      if (labels.length === 0) {
        this.clear();
        return;
      }
      ...
      __privateSet(this, _syncDirection, "idle");
      if (op === "cut") {
        EffectBus7.emit("controls:action", { action: "cut" });
        this.clear();
        return;
      }
      if (op === "copy") {
        EffectBus7.emit("controls:action", { action: "copy" });
        __privateMethod(this, _CommandLineComponent_instances, collapseToSelect_fn).call(this, labels);
        return;
      }
      if (op === "move") {
        ...
        if (indexMatch) {
          ...
          __privateSet(this, _lastMoveLabels, []);
          __privateMethod(this, _CommandLineComponent_instances, collapseToSelect_fn).call(this, labels);
          return;
        }
        ...
        if (dir) {
          for (const label of labels) {
            try {
              ...
              EffectBus7.emit("cell:removed", { cell: label });
            } catch {
            }
          }
        }
        selection.clear();
        await new hypercomb6().act();
        this.clear();
        if (dir) {
          let hasCells = false;
          for await (const [name, handle] of dir.entries()) {
    ...
    __publicField(this, "onShellCompletionAccepted", (best) => {
      const ctx = this.context();
      if (!ctx.active) return;
      if (ctx.mode === "tag") {
        const full = ctx.head + best;
        const head = ctx.head;
        void __privateMethod(this, _CommandLineComponent_instances, extractAndPersistTags_fn).call(this, full).then(() => {
          __privateMethod(this, _CommandLineComponent_instances, setShellValue_fn).call(this, head, false);
        });
        return;
      }
      if (ctx.mode === "slash") {
        if (ctx.head.match(/^\/(remove|rm|delete|del)[\s\[]/i)) {
          __privateMethod(this, _CommandLineComponent_instances, setShellValue_fn).call(this, ctx.head + best, false);
          return;
        }
        ...
        if (ctx.head === "/") {
          __privateMethod(this, _CommandLineComponent_instances, setShellValue_fn).call(this, "/" + best, true);
        } else {
          __privateMethod(this, _CommandLineComponent_instances, setShellValue_fn).call(this, ctx.head + best, false);
        }
        return;
      ...
      if (ctx.mode === "select") {
        ...
        if (phase === "selection") {
          const lastSep = Math.max(raw2.lastIndexOf(","), raw2.lastIndexOf("["));
          const before = raw2.slice(0, lastSep + 1);
          const spacer = raw2.lastIndexOf(",") >= 0 ? " " : "";
          __privateSet(this, _syncDirection, "command");
          __privateMethod(this, _CommandLineComponent_instances, setShellValue_fn).call(this, before + spacer + best, false);
          __privateSet(this, _syncDirection, "idle");
          return;
        }
        if (phase === "operation") {
          const bracketClose = raw2.indexOf("]");
          const prefix = raw2.slice(0, bracketClose + 1);
          const op = best.startsWith("/") ? best : "/" + best;
          __privateMethod(this, _CommandLineComponent_instances, setShellValue_fn).call(this, prefix + op, true);
          return;
        }
        if (phase === "move-path") {
          __privateMethod(this, _CommandLineComponent_instances, setShellValue_fn).call(this, ctx.head + best + "/", false);
        ...
        if (phase === "move-target-swap") {
          __privateMethod(this, _CommandLineComponent_instances, setShellValue_fn).call(this, ctx.head + best + "]", true);
          return;
        }
        return;
      }
      ...
      if (bracketPhase === "items") {
        ...
        if (isTagLeaf && __privateGet(this, _colonBracketMode)) {
          const removePrefix = leaf.startsWith("~:") ? "~" : "";
          __privateMethod(this, _CommandLineComponent_instances, setShellValue_fn).call(this, before + spacer + removePrefix + best, false);
        } else if (isTagLeaf) {
          const tagPrefix = leaf.startsWith("~:") ? "~:" : ":";
          __privateMethod(this, _CommandLineComponent_instances, setShellValue_fn).call(this, before + spacer + tagPrefix + best, false);
        } else {
          __privateMethod(this, _CommandLineComponent_instances, setShellValue_fn).call(this, before + spacer + best, false);
        }
        this.updateCellSubPath();
      ...
      if (bracketPhase === "path") {
        ...
        if (subPath.length > 0) {
          __privateMethod(this, _CommandLineComponent_instances, setShellValue_fn).call(this, bracketPrefix + subPath.join("/") + "/" + best + "/", false);
        } else {
          __privateMethod(this, _CommandLineComponent_instances, setShellValue_fn).call(this, bracketPrefix + best + "/", false);
        }
        this.updateCellSubPath();
        return;
      }
      if (subPath.length > 0) {
        ...
        __privateMethod(this, _CommandLineComponent_instances, setShellValue_fn).call(this, pathPrefix + best + "/", false);
        this.updateCellSubPath();
        return;
      }
      const rendered = this.completions.render(best, ctx.style);
      const newValue = ctx.mode === "marker" || ctx.mode === "remove" ? ctx.head + rendered : rendered;
      __privateMethod(this, _CommandLineComponent_instances, setShellValue_fn).call(this, newValue, true);
    });
    __publicField(this, "clear", () => {
      ...
      if (__privateGet(this, _lastSelectMode)) {
        __privateSet(this, _syncDirection, "command");
        const selection = get("@diamondcoreprocessor.com/SelectionService");
        if (selection?.count > 0) selection.clear();
        __privateSet(this, _syncDirection, "idle");
        __privateSet(this, _lastSelectMode, false);
      }
      const moveDrone = get("@diamondcoreprocessor.com/MoveDrone");
      if (moveDrone?.moveCommandActive) moveDrone.cancelCommandMove();
      __privateSet(this, _lastMoveLabels, []);
      if (__privateGet(this, _selectOriginalSegments)) {
        this.navigation.replaceRaw(__privateGet(this, _selectOriginalSegments));
        __privateSet(this, _selectOriginalSegments, null);
      }
    });
    __publicField(this, "requestSynchronize", () => {
      void new hypercomb6().act();
    });
    // -------------------------------------------------
    // /select[...] context parsing
    // -------------------------------------------------
    /** Original navigation segments stored before real-time navigation (for rollback) */
    __privateAdd(this, _selectOriginalSegments, null);
    /** Labels last passed to beginCommandMove — detect changes and restart */
    __privateAdd(this, _lastMoveLabels, []);
    /** Phase derived from value — computed, no signal writes */
    __privateAdd(this, _selectPhase, computed(() => {
      const v = this.value();
      if (!isSelectInput(v)) return "none";
      return __privateMethod(this, _CommandLineComponent_instances, deriveSelectPhase_fn).call(this, normalizeSelectInput(v));
    }));
    ...
    __privateAdd(this, _selectLabels, computed(() => {
      ...
      if (bracketClose >= 0) {
        ...
        return inner.split(",").map((s) => this.completions.normalize(__privateMethod(this, _CommandLineComponent_instances, stripTagSuffix_fn).call(this, s.trim()))).filter(Boolean);
      }
      const body = v.slice(bracketOpen + 1);
      const allParts = body.split(",").map((s) => this.completions.normalize(__privateMethod(this, _CommandLineComponent_instances, stripTagSuffix_fn).call(this, s.trim()))).filter(Boolean);
    ...
    __privateAdd(this, _selectExcluded, computed(() => {
      const v = normalizeSelectInput(this.value());
      if (!v.match(/^\/select\[/)) return /* @__PURE__ */ new Set();
      const bracketClose = v.indexOf("]");
      if (bracketClose >= 0) return /* @__PURE__ */ new Set();
      const body = v.slice(v.indexOf("[") + 1);
      const lastComma = body.lastIndexOf(",");
      if (lastComma < 0) return /* @__PURE__ */ new Set();
      const already = /* @__PURE__ */ new Set();
      for (const item of body.slice(0, lastComma).split(",")) {
        const n = this.completions.normalize(__privateMethod(this, _CommandLineComponent_instances, stripTagSuffix_fn).call(this, item.trim()));
        if (n) already.add(n);
      }
      return already;
    }));
    /** Sync direction flag to prevent feedback loops in bi-directional sync */
    __privateAdd(this, _syncDirection, "idle");
    /** Tracks whether we were in select mode last input — used to detect exit and clear selection */
    __privateAdd(this, _lastSelectMode, false);
    // -------------------------------------------------
    // cell sub-path tracking
    // -------------------------------------------------
    // bracket mode: 'none' | 'items' (inside []) | 'path' (after ]/)
    __privateAdd(this, _bracketPhase, signal("none"));
    /** Whether current bracket mode is colon-bracket (cell:[...]) — items are plain tag names. */
    __privateAdd(this, _colonBracketMode, false);
    /** Pending tag adds/removes typed in the current bracket input (not yet persisted). */
    __privateAdd(this, _bracketPendingTags, { adds: /* @__PURE__ */ new Set(), removes: /* @__PURE__ */ new Set() });
    __publicField(this, "updateCellSubPath", () => {
      const raw = this.value().trim();
      const bracketOpen = raw.indexOf("[");
      const bracketClose = raw.indexOf("]");
      if (bracketOpen >= 0 && bracketClose < 0) {
        __privateGet(this, _bracketPhase).set("items");
        ...
        if (isColonBracket) {
          __privateSet(this, _colonBracketMode, true);
          const label = this.completions.normalize(raw.slice(0, bracketOpen - 1).trim());
          if (label && label !== __privateGet(this, _bracketCellLabel)) {
            __privateSet(this, _bracketCellLabel, label);
            void __privateMethod(this, _CommandLineComponent_instances, loadCellTags_fn).call(this, label);
          }
          const lastCommaIdx = inner.lastIndexOf(",");
          const committed = lastCommaIdx >= 0 ? inner.slice(0, lastCommaIdx) : "";
          __privateSet(this, _bracketPendingTags, __privateMethod(this, _CommandLineComponent_instances, parseBracketTagItems_fn).call(this, committed));
          this.cellSubPath.set([]);
          this.cellLeaf.set(fragment.startsWith("~") ? "~:" + fragment.slice(1) : ":" + fragment);
          this.cellProvider.query([]);
          return;
        }
        __privateSet(this, _colonBracketMode, false);
        if (fragment.startsWith(":") || fragment.startsWith("~:")) {
          const label = bracketOpen > 0 ? this.completions.normalize(raw.slice(0, bracketOpen).trim()) : "";
          if (label && label !== __privateGet(this, _bracketCellLabel)) {
            __privateSet(this, _bracketCellLabel, label);
            void __privateMethod(this, _CommandLineComponent_instances, loadCellTags_fn).call(this, label);
          }
          const lastCommaIdx = inner.lastIndexOf(",");
          const committed = lastCommaIdx >= 0 ? inner.slice(0, lastCommaIdx) : "";
          __privateSet(this, _bracketPendingTags, __privateMethod(this, _CommandLineComponent_instances, parsePendingBracketTags_fn).call(this, committed));
          this.cellSubPath.set([]);
          this.cellLeaf.set(fragment);
          this.cellProvider.query([]);
          return;
        }
        if (__privateGet(this, _bracketCellLabel)) {
          __privateSet(this, _bracketCellLabel, "");
          __privateGet(this, _bracketCellTags).set(/* @__PURE__ */ new Set());
          __privateSet(this, _bracketPendingTags, { adds: /* @__PURE__ */ new Set(), removes: /* @__PURE__ */ new Set() });
        }
        const leaf2 = this.completions.normalize(fragment);
        this.cellSubPath.set([]);
        this.cellLeaf.set(leaf2);
        this.cellProvider.query([]);
        return;
      }
      if (bracketOpen === 0 && bracketClose > 0 && bracketClose < raw.length - 1 && raw[bracketClose + 1] === "/") {
        __privateGet(this, _bracketPhase).set("path");
        const pathPart = raw.slice(bracketClose + 2);
        const clean2 = pathPart.replace(/\/+$/, "");
        if (!clean2.includes("/")) {
          const leaf3 = this.completions.normalize(clean2);
          this.cellSubPath.set([]);
          this.cellLeaf.set(leaf3);
          this.cellProvider.query([]);
          return;
        }
        const parts2 = clean2.split("/");
        const leaf2 = this.completions.normalize((parts2.pop() ?? "").trim());
        const subPath2 = parts2.map((p) => this.completions.normalize(p.trim())).filter(Boolean);
        this.cellSubPath.set(subPath2);
        this.cellLeaf.set(leaf2);
        this.cellProvider.query(subPath2);
        return;
      }
      __privateGet(this, _bracketPhase).set("none");
    ...
    __privateGet(this, _indicatorUnsubs).push(
      EffectBus7.on("indicator:set", (p) => {
        if (!p?.key) return;
        __privateGet(this, _indicators).update((m) => {
          const n = new Map(m);
          n.set(p.key, p);
          return n;
        });
      }),
      EffectBus7.on("indicator:clear", (p) => {
        if (!p?.key) return;
        __privateGet(this, _indicators).update((m) => {
          const n = new Map(m);
          n.delete(p.key);
    ...
    if (saved) {
      try {
        const list2 = JSON.parse(saved);
        const m = /* @__PURE__ */ new Map();
        for (const ind of list2) m.set(ind.key, ind);
        __privateGet(this, _indicators).set(m);
      } catch {
      }
    }
  }
  ...
  get completions() {
    return get("@hypercomb.social/CompletionUtility");
  }
  get lineage() {
    return get("@hypercomb.social/Lineage");
  }
  get movement() {
    return get("@hypercomb.social/MovementService");
  }
  get navigation() {
    return get("@hypercomb.social/Navigation");
  }
  get preloader() {
    return get("@hypercomb.social/ScriptPreloader");
  }
  get cellProvider() {
    return get("@hypercomb.social/CellSuggestionProvider");
  }
  /** All behavior metadata — pluggable + built-in */
  get behaviorReference() {
    return [
      ...__privateGet(this, _behaviors),
      ..._CommandLineComponent.builtinBehaviors
    ];
  }
  ...
  onIndicatorDismiss(key) {
    EffectBus7.emit("indicator:dismiss", { key });
    __privateGet(this, _indicators).update((m) => {
      const n = new Map(m);
      n.delete(key);
      return n;
    });
    __privateMethod(this, _CommandLineComponent_instances, persistIndicators_fn).call(this);
  }
  /** Handle a hint-bar crumb click — accept that preset. */
  onHintPick(preset) {
    const ctx = this.context();
    if (!ctx.active) return;
    __privateMethod(this, _CommandLineComponent_instances, setShellValue_fn).call(this, ctx.head + preset, true);
  }
  // -------------------------------------------------
  // lifecycle
  // -------------------------------------------------
  ngAfterViewInit() {
    this.shell?.focus();
    window.addEventListener("navigate", __privateGet(this, _onNavigate));
    window.addEventListener("popstate", __privateGet(this, _onNavigate));
    __privateSet(this, _commandLineToggleUnsub, EffectBus7.on("keymap:invoke", (payload) => {
      if (payload?.cmd !== "ui.commandLineToggle") return;
      this.shell?.focus();
    }));
    __privateSet(this, _prefillUnsub, EffectBus7.on("search:prefill", ({ value }) => {
      __privateMethod(this, _CommandLineComponent_instances, setShellValue_fn).call(this, value, false);
    }));
    __privateSet(this, _commandFocusUnsub, EffectBus7.on("command:focus", ({ cell }) => {
      __privateMethod(this, _CommandLineComponent_instances, setShellValue_fn).call(this, cell, false);
      this.shell?.focus();
    }));
    __privateSet(this, _touchDraggingUnsub, EffectBus7.on("touch:dragging", ({ active }) => {
      this.touchDragging.set(active);
      if (active) {
        this.shell?.suppress();
      }
    }));
    __privateSet(this, _viewActiveUnsub, EffectBus7.on("view:active", ({ active }) => {
      this.viewActive.set(active);
    }));
    __privateSet(this, _selectionSyncUnsub, EffectBus7.on("selection:changed", (payload) => {
      if (__privateGet(this, _syncDirection) === "command") return;
      if (!payload?.selected) return;
      const selected = payload.selected;
      if (selected.length === 0) {
        if (__privateGet(this, _selectPhase).call(this) !== "none") {
          this.clear();
        }
        return;
      }
      ...
      if (!ctx.active || ctx.mode === "select" || this.value() === "") {
        __privateSet(this, _syncDirection, "visual");
        const currentValue = this.value();
        const bracketCloseIdx = currentValue.indexOf("]");
        const tail = bracketCloseIdx >= 0 ? currentValue.slice(bracketCloseIdx + 1) : "";
        __privateMethod(this, _CommandLineComponent_instances, setShellValue_fn).call(this, __privateMethod(this, _CommandLineComponent_instances, buildSelectValue_fn).call(this, selected, __privateMethod(this, _CommandLineComponent_instances, shouldTruncate_fn).call(this, selected)) + tail, true);
        __privateSet(this, _syncDirection, "idle");
      }
    }));
    __privateSet(this, _voiceInterimUnsub, EffectBus7.on("voice:interim", ({ text }) => {
      __privateMethod(this, _CommandLineComponent_instances, setShellValue_fn).call(this, text, false);
    }));
    __privateSet(this, _voiceSubmitUnsub, EffectBus7.on("voice:submit", ({ text }) => {
      __privateMethod(this, _CommandLineComponent_instances, setShellValue_fn).call(this, text, false);
      void __privateMethod(this, _CommandLineComponent_instances, preprocessTagsThenExecute_fn).call(this, text);
    }));
    __privateSet(this, _voiceActiveUnsub, EffectBus7.on("voice:active", ({ active }) => {
      this.voiceActive.set(active);
    }));
    __privateSet(this, _pushToTalkUnsub, EffectBus7.on("push-to-talk:toggle", ({ enabled }) => {
...
_bracketCellTags = new WeakMap();
_bracketCellLabel = new WeakMap();
_slashMatches = new WeakMap();
_CommandLineComponent_instances = new WeakSet();
/**
 ...
 */
extractSlashCommandArgs_fn = function(raw) {
  const spaceIdx = raw.indexOf(" ");
  ...
  if (bStart >= 0 || raw[sepIdx] === "[") {
    ...
    if (bracketClose < 0) {
      const lastComma = inner.lastIndexOf(",");
      const fragment2 = lastComma >= 0 ? inner.slice(lastComma + 1).trimStart() : inner.trimStart();
      return { command, fragment: fragment2, fullArgs };
    }
 ...
 */
validateBehaviors_fn = function(behaviors) {
  ...
  for (const b of behaviors) {
    for (const op of b.operations) {
      for (const ex of op.examples) {
        ...
        if (existing) {
          console.warn(
            `[command-line] overlap: "${b.name}" and "${existing.behavior}" both claim trigger="${op.trigger}" for input "${ex.input}". "${existing.behavior}" wins (registered first).`
          );
        } else {
...
i18n_get = function() {
  return get("@hypercomb.social/I18n");
};
_indicators = new WeakMap();
_indicatorUnsubs = new WeakMap();
persistIndicators_fn = function() {
  const list2 = [...__privateGet(this, _indicators).call(this).values()];
  if (list2.length > 0) {
    localStorage.setItem("hc:indicators", JSON.stringify(list2));
  } else {
...
_commandFocusUnsub = new WeakMap();
_commandLineToggleUnsub = new WeakMap();
_touchDraggingUnsub = new WeakMap();
_viewActiveUnsub = new WeakMap();
_selectionSyncUnsub = new WeakMap();
_voiceInterimUnsub = new WeakMap();
_voiceSubmitUnsub = new WeakMap();
_onNavigate = new WeakMap();
preprocessTagsThenExecute_fn = async function(original) {
  const cleaned = await __privateMethod(this, _CommandLineComponent_instances, extractAndPersistTags_fn).call(this, original);
  if (cleaned !== original) {
  ...
  if (isSelectInput(v)) {
    ...
    this.value.set(this.shell?.value() ?? "");
    void __privateGet(this, _executeSelectCommand).call(this);
    return;
  }
  if (v.startsWith("/")) {
    void __privateGet(this, _executeSlashBehaviour).call(this);
    return;
  }
  const raw = v.trim();
  for (const behavior of __privateGet(this, _behaviors)) {
...
_executeSlashBehaviour = new WeakMap();
_executeSelectCommand = new WeakMap();
extractAndPersistTags_fn = async function(input2) {
  const ops = [];
  const bracketTagMatch = input2.match(BRACKET_TAG_RE);
  if (bracketTagMatch) {
    ...
    for (const raw of items) {
      ...
      if (!trimmed2 || !label) continue;
      if (trimmed2.startsWith("~")) {
        const tag = trimmed2.slice(1).trim();
        if (tag) ops.push({ label, tag, remove: true });
      } else {
        const colorMatch = trimmed2.match(/^([^(]+)(?:\(([^)]+)\))?$/);
        if (colorMatch) {
    ...
    if (ops.length > 0) {
      await __privateMethod(this, _CommandLineComponent_instances, persistTagOps_fn).call(this, ops);
      return label + suffix;
    }
    return input2;
  }
  const labelBracketMatch = input2.match(/^([^\[\/!#~]+)\[(.+?)\](.*)$/);
  if (labelBracketMatch) {
    ...
    for (const raw of items) {
      const trimmed2 = raw.trim();
      const removeMatch2 = trimmed2.match(/^~:([^(]+)(?:\(([^)]+)\))?$/);
      if (removeMatch2 && label) {
        const tag = removeMatch2[1].trim();
        if (tag) ops.push({ label, tag, remove: true });
        continue;
      }
      const addMatch2 = trimmed2.match(/^:([^(]+)(?:\(([^)]+)\))?$/);
      if (addMatch2 && label) {
        const tag = addMatch2[1].trim();
        const color = addMatch2[2]?.trim();
        if (tag) ops.push({ label, tag, color, remove: false });
        continue;
      }
  ...
  const normalizedInput = normalizeSelectInput(input2);
  ...
  if (selectMatch) {
    ...
    for (const raw of items) {
      const trimmed2 = raw.trim();
      const removeMatch2 = trimmed2.match(/^~([^:]+):([^(]+)(?:\(([^)]+)\))?$/);
      const addMatch2 = trimmed2.match(TAG_ASSIGN_RE);
      if (removeMatch2) {
        const label = this.completions.normalize(removeMatch2[1]);
        const tag = removeMatch2[2].trim();
        if (label && tag) ops.push({ label, tag, remove: true });
      } else if (addMatch2) {
        const label = this.completions.normalize(addMatch2[1]);
        const tag = addMatch2[2].trim();
        const color = addMatch2[3]?.trim();
        if (label && tag) {
...
shouldTruncate_fn = function(labels) {
 ...
 */
autoPopulateMoveIndex_fn = function(v) {
...
scrubMoveIndex_fn = function(delta) {
  const v = this.value();
  const parenIdx = v.lastIndexOf("(");
  if (parenIdx < 0) return;
  const currentIndex = parseInt(v.slice(parenIdx + 1).replace(/\)$/, ""), 10);
  if (isNaN(currentIndex)) return;
  const axialSvc = get("@diamondcoreprocessor.com/AxialService");
...
collapseToSelect_fn = function(labels) {
  if (labels.length > 0) {
    __privateMethod(this, _CommandLineComponent_instances, setShellValue_fn).call(this, __privateMethod(this, _CommandLineComponent_instances, buildSelectValue_fn).call(this, [...labels], __privateMethod(this, _CommandLineComponent_instances, shouldTruncate_fn).call(this, labels)), true);
  } else {
    this.clear();
  }
};
/** Cancel select operation — collapse back to /select[tiles] or clear */
cancelSelectOperation_fn = function() {
  const phase = __privateGet(this, _selectPhase).call(this);
  const labels = __privateGet(this, _selectLabels).call(this);
  EffectBus7.emit("move:preview", null);
  const moveDrone = get("@diamondcoreprocessor.com/MoveDrone");
  if (moveDrone?.moveCommandActive) moveDrone.cancelCommandMove();
  __privateSet(this, _lastMoveLabels, []);
  if (__privateGet(this, _selectOriginalSegments)) {
...
setShellValue_fn = function(v, suppress) {
...
_selectOriginalSegments = new WeakMap();
_lastMoveLabels = new WeakMap();
_selectPhase = new WeakMap();
/** Strip :tag(color) suffix from a raw select item, returning just the tile label. */
stripTagSuffix_fn = function(raw) {
  const colon = raw.indexOf(":");
  return colon >= 0 ? raw.slice(0, colon) : raw;
};
_selectLabels = new WeakMap();
_selectExcluded = new WeakMap();
/** Derive the select phase from the input string (pure, no side effects) */
deriveSelectPhase_fn = function(v) {
  const bracketOpen = v.indexOf("[");
  const bracketClose = v.indexOf("]");
...
parseSelectContext_fn = function(v) {
  const bracketOpen = v.indexOf("[");
  const bracketClose = v.indexOf("]");
  const phase = __privateMethod(this, _CommandLineComponent_instances, deriveSelectPhase_fn).call(this, v);
  if (phase === "selection") {
    const body = v.slice(bracketOpen + 1);
...
_syncDirection = new WeakMap();
_lastSelectMode = new WeakMap();
/**
 ...
 */
handleSelectInputEffects_fn = function() {
  const phase = __privateGet(this, _selectPhase).call(this);
  const labels = __privateGet(this, _selectLabels).call(this);
  __privateSet(this, _syncDirection, "command");
  const selection = get("@diamondcoreprocessor.com/SelectionService");
  ...
  if (phase === "move-target-index") {
    ...
    if (!isNaN(targetIndex) && labels.length > 0) {
      const moveDrone = get("@diamondcoreprocessor.com/MoveDrone");
      if (moveDrone) {
        const labelsChanged = labels.length !== __privateGet(this, _lastMoveLabels).length || labels.some((l, i) => l !== __privateGet(this, _lastMoveLabels)[i]);
        if (!moveDrone.moveCommandActive || labelsChanged) {
          if (moveDrone.moveCommandActive) moveDrone.cancelCommandMove();
          moveDrone.beginCommandMove([...labels]);
          __privateSet(this, _lastMoveLabels, labels);
        }
  ...
  if (phase === "move-path") {
    ...
    if (moveStart >= 0) {
      const afterMove = v.slice(moveStart + 5);
      if (afterMove.startsWith("/")) {
        ...
        if (pathPart) {
          ...
          if (!__privateGet(this, _selectOriginalSegments)) {
            __privateSet(this, _selectOriginalSegments, [...this.navigation.segmentsRaw()]);
          }
          const target = [...__privateGet(this, _selectOriginalSegments), ...segments];
...
_bracketPhase = new WeakMap();
_colonBracketMode = new WeakMap();
_bracketPendingTags = new WeakMap();
loadCellTags_fn = async function(label) {
  try {
    const dir = await this.lineage.explorerDir();
    if (!dir) {
      __privateGet(this, _bracketCellTags).set(/* @__PURE__ */ new Set());
    ...
    const props = await readTagProps(cellDir);
    const tags = Array.isArray(props["tags"]) ? props["tags"] : [];
    __privateGet(this, _bracketCellTags).set(new Set(tags));
  } catch {
    __privateGet(this, _bracketCellTags).set(/* @__PURE__ */ new Set());
  }
};
/** Parse tag items in cell:[...] syntax (no : prefix — items are plain names, ~ for removal). */
parseBracketTagItems_fn = function(inner) {
  const adds = /* @__PURE__ */ new Set();
  const removes = /* @__PURE__ */ new Set();
  for (const raw of inner.split(",")) {
    const t = raw.trim();
    if (t.startsWith("~")) {
      removes.add(t.slice(1).trim());
      continue;
    }
    if (t) {
      adds.add(t);
...
parsePendingBracketTags_fn = function(inner) {
  const adds = /* @__PURE__ */ new Set();
  const removes = /* @__PURE__ */ new Set();
  for (const raw of inner.split(",")) {
    const t = raw.trim();
...
__decorateElement(_init4, 5, "shell", _shell_dec, _CommandLineComponent);
_CommandLineComponent = __decorateElement(_init4, 0, "CommandLineComponent", _CommandLineComponent_decorators, _CommandLineComponent);
// built-in behaviors that are hardcoded in onKeyDown (not pluggable yet)
__publicField(_CommandLineComponent, "builtinBehaviors", [
  {
    name: "create",
    operations: [
      {
...
// -------------------------------------------------
__publicField(_CommandLineComponent, "ACCENT_PRESETS", ["glacier", "bloom", "aurora", "ember", "nebula"]);
/** CSS colors for each accent preset (derived from shader RGB values). */
__publicField(_CommandLineComponent, "ACCENT_COLOR_MAP", /* @__PURE__ */ new Map([
...
__runInitializers(_init4, 1, _CommandLineComponent);
var CommandLineComponent = _CommandLineComponent;

// ../hypercomb-shared/core/script-preloader.ts
import { EffectBus as EffectBus8 } from "@hypercomb/core";
var ScriptPreloader = class _ScriptPreloader extends EventTarget {
  get store() {
  ...
  #beeCache = /* @__PURE__ */ new Map();
  #loadedDeps = /* @__PURE__ */ new Set();
  // In-flight dedup: prevents two callers from loading the same bee concurrently
  #inFlight = /* @__PURE__ */ new Map();
  resolveBySignature = (signature) => this.#bySignature.get(signature);
  getActionName = (signature) => this.#bySignature.get(signature)?.name ?? null;
  // -------------------------------------------------
  ...
  find = async (_grammar) => {
    ...
    const run = async () => {
      const manifestBees = _ScriptPreloader.readManifestBees();
  ...
  #loadBeesFromList = async (sigs) => {
    const pending = sigs.filter((sig) => sig && this.#isSignature(sig) && !this.#beeCache.has(sig));
    if (!pending.length) return;
    EffectBus8.emit("loader:bees-progress", { loading: pending.length, total: this.#beeCache.size + pending.length });
    ...
    if (loaded) {
      ...
      EffectBus8.emit("loader:bees-done", { loaded, failed: pending.length - loaded, total: this.#beeCache.size });
    }
  };
  #loadBeeBySignature = async (signature) => {
  ...
  // -------------------------------------------------
  #ensureDeps = async (beeSig) => {
    const map2 = globalThis.__hypercombBeeDeps;
    if (!map2) return;
    const needed = map2[beeSig];
    if (!needed?.length) return;
    const aliasMap = globalThis.__hypercombAliasMap;
    if (!aliasMap) return;
  ...
  // -------------------------------------------------
  #refreshProjection = () => {
    const list2 = [...this.#bySignature.values()].sort((a, b) => a.name.localeCompare(b.name));
    this.#actions = list2;
    this.#actionNames = list2.map((a) => (a.name ?? "").replace(/-/g, " "));
    this.dispatchEvent(new CustomEvent("change"));
  };
  // -------------------------------------------------
  // utilities
  // -------------------------------------------------
  #isSignature = (name) => /^[a-f0-9]{64}$/i.test(name);
};
...
var Store = class _Store extends EventTarget {
  static BEES_DIRECTORY = "__bees__";
  static DEPENDENCIES_DIRECTORY = "__dependencies__";
  static LAYERS_DIRECTORY = "__layers__";
  static RESOURCES_DIRECTORY = "__resources__";
  static CLIPBOARD_DIRECTORY = "__clipboard__";
  static HISTORY_DIRECTORY = "__history__";
  static THREADS_DIRECTORY = "__threads__";
  static COMPUTATION_DIRECTORY = "__computation__";
  static CACHE_NAME = "hypercomb-modules-v2";
  opfsRoot;
  hypercombRoot;
  bees;
  dependencies;
  ...
  initialize = async () => {
    ...
    const dir = (name) => this.opfsRoot.getDirectoryHandle(name, { create: true });
    [
      this.hypercombRoot,
      this.bees,
      this.dependencies,
      ...
      dir("hypercomb.io"),
      dir(_Store.BEES_DIRECTORY),
      dir(_Store.DEPENDENCIES_DIRECTORY),
      dir(_Store.LAYERS_DIRECTORY),
      dir(_Store.RESOURCES_DIRECTORY),
      dir(_Store.CLIPBOARD_DIRECTORY),
      dir(_Store.HISTORY_DIRECTORY),
      dir(_Store.THREADS_DIRECTORY),
      dir(_Store.COMPUTATION_DIRECTORY)
    ]);
  };
  domainLayersDirectory = async (domain, create = false) => {
    return await this.layers.getDirectoryHandle(domain, { create });
  };
  // -------------------------------------------------
  // bee loader
  // -------------------------------------------------
  getBee = async (signature, buffer) => {
    ...
    try {
      ...
      for (const key of window.ioc.list()) {
        ...
        if (value != null && typeof value.pulse === "function") return value;
      }
  ...
  cellResourceCache = async (signature, buffer) => {
    const opfsUrl = new URL(`/opfs/${_Store.BEES_DIRECTORY}/${signature}.js`, location.origin).toString();
    try {
      const cache = await caches.open(_Store.CACHE_NAME);
  ...
  putResource = async (blob) => {
    ...
    const signature = await SignatureService2.sign(bytes);
...
var MovementService = class extends EventTarget {
...
var DependencyLoader = class extends EventTarget {
  get store() {
    return get("@hypercomb.social/Store");
  }
  ...
  get loadedSignatures() {
    return this.#loadedSignatures;
  }
  get failedSignatures() {
  ...
  load = async () => {
    ...
    const run = async () => {
      ...
      const beeDeps = globalThis.__hypercombBeeDeps;
      if (beeDeps) {
      ...
      EffectBus9.emit("loader:deps-start", { total: pending.length });
      const results = await Promise.allSettled(
        pending.map(
          ({ sig, alias }) => this.#verifyAndImport(sig, alias)
        )
      );
      const loadedSigs = [];
      const failedSigs = [];
      ...
      this.dispatchEvent(new CustomEvent("change"));
      EffectBus9.emit("loader:deps-done", {
  ...
  #collectPending = async () => {
    const cachedMap = globalThis.__hypercombAliasMap;
  ...
  #verifyAndImport = async (sig, alias) => {
  ...
  #readAliasFromFirstLine = (text) => {
    if (!text.startsWith("//")) return null;
    const parts = text.split(/\s+/);
    const token2 = (parts[1] ?? "").trim();
    return token2.startsWith("@") ? token2 : null;
  };
  #isSignature = (name) => /^[a-f0-9]{64}$/i.test(name.replace(".js", ""));
};
register("@hypercomb.social/DependencyLoader", new DependencyLoader());

// ../hypercomb-shared/core/layer-installer.ts
var LayerInstaller = class {
  #localManifestName = "__install_cache__.json";
  install = async (parsed) => {
    ...
    const store2 = get("@hypercomb.social/Store");
    const domainLayersDir = await store2.domainLayersDirectory(domainKey, true);
    ...
    await this.#installDependencies(store2, baseUrl, manifest.dependencies || []);
    await this.#installBees(store2, baseUrl, manifest.bees || []);
    const complete = await this.#isComplete(domainLayersDir, store2, manifest);
    if (complete) {
  ...
  // manifest
  // -------------------------------------------------
  #getOrFetchPackage = async (domainLayersDir, baseUrl, packageSig) => {
  ...
  #tryParseContentManifest = (text) => {
    try {
      const parsed = JSON.parse(text);
      if (parsed?.packages && typeof parsed.packages === "object") return parsed;
      return null;
    } catch {
      return null;
    }
  };
  #tryParseInstallManifest = (text) => {
    try {
  ...
  // install
  // -------------------------------------------------
  #installLayers = async (domainLayersDir, endpoint, layers) => {
    for (const sig of layers) {
      if (!sig) continue;
      const existing = await this.#tryGetFileHandle(domainLayersDir, sig) ?? await this.#tryGetFileHandle(domainLayersDir, `${sig}.json`);
      if (existing) {
  ...
  #installDependencies = async (store2, endpoint, deps) => {
    const depDir = store2.dependencies;
    for (const sig of deps) {
      if (!sig) continue;
      const name = `${sig}.js`;
      const existing = await this.#tryGetFileHandle(depDir, name) ?? await this.#tryGetFileHandle(depDir, sig);
      if (existing) {
  ...
  #installBees = async (store2, endpoint, bees) => {
    const beesDir = store2.bees;
    for (const sig of bees) {
      if (!sig) continue;
      const name = `${sig}.js`;
      const existing = await this.#tryGetFileHandle(beesDir, name) ?? await this.#tryGetFileHandle(beesDir, sig);
      if (existing) {
  ...
  #isComplete = async (domainLayersDir, store2, manifest) => {
  ...
  // io
  // -------------------------------------------------
  #tryGetFileHandle = async (dir, name) => {
    try {
  ...
  #safeRemove = async (dir, name) => {
    try {
      await dir.removeEntry(name);
    } catch {
    }
  };
  #tryReadText = async (dir, name) => {
  ...
  #writeBytesFile = async (dir, name, bytes) => {
...
var Lineage = class extends EventTarget {
  // -------------------------------------------------
  // dependencies
  // -------------------------------------------------
  get store() {
  ...
  // -------------------------------------------------
  explorerPath = [];
  explorerSegments = () => this.explorerPath;
  explorerEnter = (name) => {
    const seg = (name ?? "").trim();
  ...
  // -------------------------------------------------
  tryResolve = async (segments, start) => {
    if (!start) return null;
    return await this.tryResolveFrom(start, segments);
  };
  tryResolveFrom = async (start, segments) => {
    let dir = start;
  ...
  addMarker = async (_segments, _signature) => {
  };
  // -------------------------------------------------
  // internal
  // -------------------------------------------------
...
import { hypercomb as hypercomb7 } from "@hypercomb/core";
var Navigation = class extends hypercomb7 {
  get completions() {
    return get("@hypercomb.social/CompletionUtility");
  }
  listening = false;
  ...
  replaceSelections = (names) => {
    ...
    const hash = clean.length === 1 ? "#" + clean[0] : "#(" + clean.join(",") + ")";
  ...
  toggleSelection = (name) => {
    ...
    const next = current.includes(clean) ? current.filter((x) => x !== clean) : [...current, clean];
  ...
  dispatchSelection = (selected) => {
    window.dispatchEvent(
      new CustomEvent("selection", { detail: { selected } })
    );
  };
  safeDecode = (s) => {
    try {
...
var SIG_RE = /^[a-f0-9]{64}$/i;
var SYSTEM_DIRS = /* @__PURE__ */ new Set(["__bees__", "__dependencies__", "__resources__", "__history__", "__layers__"]);
var OpfsTreeLogger = class {
  log = async () => {
    console.clear();
    try {
  ...
  #walk = async (dir, indent) => {
    ...
    for (const e of entries) {
      if (e.kind === "directory") {
        ...
        const icon = isSystem ? "\u{1F4E6}" : "\u{1F4C1}";
        const childDir = e.handle;
        ...
        const label = await this.#describeFile(e.name, e.handle);
        console.log(`${indent}\u{1F4C4} ${e.name}${label}`);
      }
    }
  };
  #describeFile = async (name, handle) => {
    const bare = name.replace(/\.js$/i, "");
    if (SIG_RE.test(bare)) return "  \u{1F41D} marker";
    if (name === "0000") {
      try {
        const file = await handle.getFile();
...
var BootstrapHistory = class {
  #hasRun = false;
  run = async () => {
    if (this.#hasRun) return;
    this.#hasRun = true;
    const preloader = get("@hypercomb.social/ScriptPreloader");
    const utility = get("@hypercomb.social/CompletionUtility");
    ...
    try {
      window.history.replaceState({ i: 0, steps: [] }, "", "/");
  ...
  parsePath = (path, completions) => {
    const parts = (path ?? "").split("/").filter(Boolean);
    return parts.map((p) => this.cleanSegment(p, completions)).filter(Boolean);
  };
  cleanSegment = (s, completions) => {
    const decoded = this.safeDecode((s ?? "").trim());
    const noSlashes = decoded.replace(/[\/\\]+/g, " ");
  ...
  tryGetLineageSegments = (lineage) => {
    ...
    try {
      ...
      if (Array.isArray(segs) && segs.length) return segs.map((s) => (s ?? "").toString());
    } catch {
    }
    return [];
  };
  encounter = async (preloader, seg) => {
    let bees = [];
    try {
...
register("@hypercomb.social/DirectoryWalker", new DirectoryWalker());

// ../hypercomb-shared/core/completion-utility.ts
import { normalizeCell } from "@hypercomb/core";
var CompletionUtility = class {
  normalize = (s) => normalizeCell(s);
  render = (s, style) => style === "dot" ? s.replace(/\s+/g, ".") : s;
};
register("@hypercomb.social/CompletionUtility", new CompletionUtility());

// ../hypercomb-shared/core/i18n.service.ts
import { EffectBus as EffectBus10 } from "@hypercomb/core";
var STORAGE_KEY = "hc:locale";
var FALLBACK_LOCALE = "en";
var LocalizationService = class extends EventTarget {
  ...
  setLocale(locale) {
    ...
    EffectBus10.emit("locale:changed", { locale });
    this.dispatchEvent(new CustomEvent("change"));
  }
  registerTranslations(namespace, locale, catalog) {
    let localeMap = this.#catalogs.get(namespace);
    if (!localeMap) {
      localeMap = /* @__PURE__ */ new Map();
  ...
  // internals
  // -----------------------------------------------
  #resolve(key, params, namespace) {
    const localeMap = this.#catalogs.get(namespace);
    if (!localeMap) return void 0;
    const template = this.#lookup(localeMap, this.#locale, key, params) ?? this.#lookup(localeMap, this.#fallback, key, params);
    if (template === void 0) return void 0;
    return params ? this.#interpolate(template, params) : template;
  }
  #lookup(localeMap, locale, key, params) {
    const catalog = localeMap.get(locale);
    if (!catalog) return void 0;
    if (params && typeof params["count"] === "number") {
      const count = params["count"];
      if (count === 0 && catalog[`${key}.zero`] !== void 0) return catalog[`${key}.zero`];
      if (count === 1 && catalog[`${key}.one`] !== void 0) return catalog[`${key}.one`];
      if (catalog[`${key}.other`] !== void 0) return catalog[`${key}.other`];
    }
    return catalog[key];
  }
  #interpolate(template, params) {
    return template.replace(/\{(\w+)\}/g, (_, token2) => {
      const value = params[token2];
      return value !== void 0 ? String(value) : `{${token2}}`;
    });
  }
};
...
var ResourceCompletionService = class {
  get preloader() {
    return get("@hypercomb.social/ScriptPreloader");
  }
  // always live (no snapshot)
  get names() {
    return this.preloader.actionNames;
  }
};
...
var ResourceMessageHandler = class {
  get store() {
    return get("@hypercomb.social/Store");
  }
  get preloader() {
  ...
  handle = async (event) => {
    ...
    if (data.scope !== "dcp") return;
    if (data.type !== "compiled.code") return;
    await this.handleCompiled(data);
  };
  // -------------------------------------------------
  // compiled js → opfs (authoritative store)
  // -------------------------------------------------
  handleCompiled = async (msg) => {
...
var CellSuggestionProvider = class extends EventTarget {
  providerName = "cells";
  get lineage() {
    return get("@hypercomb.social/Lineage");
  }
...
function token(key, ngType) {
  return { key, ngType };
}
var COMPLETION_UTILITY = token("@hypercomb.social/CompletionUtility", CompletionUtility);
var I18N = token("@hypercomb.social/I18n", LocalizationService);
var LINEAGE = token("@hypercomb.social/Lineage", Lineage);
var MOVEMENT = token("@hypercomb.social/MovementService", MovementService);
var NAVIGATION = token("@hypercomb.social/Navigation", Navigation);
var RESOURCE_COMPLETION = token("@hypercomb.social/ResourceCompletionService", ResourceCompletionService);
var RESOURCE_MSG_HANDLER = token("@hypercomb.social/ResourceMessageHandler", ResourceMessageHandler);
var SCRIPT_PRELOADER = token("@hypercomb.social/ScriptPreloader", ScriptPreloader);
var CELL_SUGGESTION = token("@hypercomb.social/CellSuggestionProvider", CellSuggestionProvider);
function bridgeProviders(tokens) {
  return tokens.filter((t) => t.ngType).map((t) => ({
    provide: t.ngType,
...
var sharedProviders = bridgeProviders([
  COMPLETION_UTILITY,
  I18N,
  LINEAGE,
  MOVEMENT,
...
// ../hypercomb-shared/core/runtime-initializer.ts
import { EffectBus as EffectBus11 } from "@hypercomb/core";

// ../hypercomb-shared/core/secret-store.ts
var KEY = "hc:secret";
var CLEARED_KEY = "hc:secret-cleared";
var SecretStore = class _SecretStore extends EventTarget {
  #value;
  get value() {
    return this.#value;
  }
  constructor() {
    ...
    if (!this.#value && !this.#wasCleared()) {
      const extracted = _SecretStore.extractSubdomain();
...
var KEY2 = "hc:room";
var RoomStore = class _RoomStore extends EventTarget {
  #value;
  get value() {
    return this.#value;
  }
  constructor() {
    ...
    this.#value = this.#read();
    if (!this.#value) {
      const extracted = _RoomStore.extractSubdomain();
      if (extracted) this.set(extracted);
    }
  }
  set = (room) => {
  ...
  #read = () => {
    try {
      return (localStorage.getItem(KEY2) ?? "").trim();
    } catch {
      return "";
    }
  };
  #write = (v) => {
    try {
      if (v) localStorage.setItem(KEY2, v);
...
import { EffectBus as EffectBus12 } from "@hypercomb/core";
var PROPS_FILE2 = "0000";
var MASTER_KEY = "tags-master";
var TagRegistry = class extends EventTarget {
  #tags = {};
  #loaded = false;
  #loading = null;
  /** All tag entries. */
  ...
  async #load() {
    try {
      ...
      if (!store2) return;
      const rootProps = await this.#readRootProps(store2);
      ...
      const blob = await store2.getResource(sig);
      if (!blob) {
        this.#loaded = true;
        return;
      }
  ...
  async #save() {
    try {
      ...
      const sig = await store2.putResource(blob);
      await this.#writeRootProps(store2, { [MASTER_KEY]: sig });
    ...
    this.dispatchEvent(new Event("change"));
    EffectBus12.emit("tags:registry", { tags: this.#tags });
  }
  /** Migrate from legacy hc:tag-colors localStorage to master list. */
  #migrateFromLegacy() {
    try {
      const legacy = JSON.parse(localStorage.getItem("hc:tag-colors") ?? "{}");
      for (const [name, color] of Object.entries(legacy)) {
        if (name && typeof color === "string") {
          this.#tags[name] = { color: color || void 0, enabled: true };
        }
      }
    } catch {
    }
  ...
  // ── root 0000 helpers ──
  async #readRootProps(store2) {
    try {
      const root = store2.opfsRoot;
      const fh = await root.getFileHandle(PROPS_FILE2);
      const file = await fh.getFile();
      return JSON.parse(await file.text());
    } catch {
      return {};
  ...
  async #writeRootProps(store2, updates) {
    const root = store2.opfsRoot;
    const existing = await this.#readRootProps(store2);
    const merged = { ...existing, ...updates };
    const fh = await root.getFileHandle(PROPS_FILE2, { create: true });
    const writable = await fh.createWritable();
    await writable.write(JSON.stringify(merged));
    await writable.close();
  }
  #store() {
    return window.ioc?.get?.("@hypercomb.social/Store");
  }
};
register("@hypercomb.social/TagRegistry", new TagRegistry());

// ../hypercomb-shared/core/secret-strength.ts
var DefaultSecretStrength = class {
  evaluate(secret) {
    if (!secret) return 0;
...
// ../hypercomb-shared/core/folder-handles.ts
...
var MAX_THUMBS = 30;
var SubstrateOrganizerDrone = class {
  #root = null;
  #backdrop = null;
  #panel = null;
  #listEl = null;
  ...
  #footerEl = null;
  #visible = false;
  #thumbUrls = /* @__PURE__ */ new Map();
  // sig → object URL (for cleanup)
  #keydownHandler = null;
  constructor() {
    EffectBus13.on("substrate-organizer:open", () => {
      void this.#open();
    });
    EffectBus13.on("substrate-organizer:close", () => {
      this.#close();
    });
    EffectBus13.on("substrate:changed", () => {
      if (this.#visible) void this.#render();
    });
  }
  // ── DOM build (one-time) ──────────────────────────────────────────
  #build() {
  ...
  async #render() {
    ...
    if (isFolderAccessSupported()) {
      this.#footerEl.appendChild(this.#button("+ Link folder", async () => {
        ...
        if (source) {
          EffectBus13.emit("activity:log", { message: `linked ${source.label}`, icon: "\u25C8" });
        }
      }));
    }
    this.#footerEl.appendChild(this.#button("+ Use current hive", async () => {
      ...
      if (segments.length === 0) {
        EffectBus13.emit("activity:log", { message: "navigate into a hive first", icon: "\u25C8" });
        return;
      }
  ...
  #renderRow(source, isActive) {
    ...
    const typeLabel = source.type + (source.builtin ? " \xB7 built-in" : "");
    const countSuffix = isActive ? ` \xB7 ${this.#service()?.resolvedImageCount ?? 0} images` : "";
    ...
    if (!source.builtin) {
      ...
      removeBtn.addEventListener("mouseenter", () => {
        removeBtn.style.background = "rgba(220, 120, 120, 0.15)";
      });
  ...
  async #renderPreview() {
    ...
    const store2 = get2("@hypercomb.social/Store");
    if (!store2) return;
    const seen = /* @__PURE__ */ new Set();
    ...
    for (const sig of seen) {
      ...
      void store2.getResource(sig).then((blob) => {
        if (!blob) return;
  ...
  #button(text, onClick) {
    ...
    btn.addEventListener("mouseenter", () => {
      btn.style.background = "rgba(140, 170, 220, 0.18)";
    });
    btn.addEventListener("mouseleave", () => {
      btn.style.background = "rgba(140, 170, 220, 0.08)";
    });
  ...
  #service() {
    return get2("@diamondcoreprocessor.com/SubstrateService");
  }
};
var _substrateOrganizerDrone = new SubstrateOrganizerDrone();
window.ioc.register("@diamondcoreprocessor.com/SubstrateOrganizerDrone", _substrateOrganizerDrone);
export {
  SubstrateOrganizerDrone
};
