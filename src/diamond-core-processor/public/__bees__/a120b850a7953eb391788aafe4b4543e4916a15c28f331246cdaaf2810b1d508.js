...
import { Drone as Drone2, I18N_IOC_KEY } from "@hypercomb/core";
...
var HexImageAtlas = class _HexImageAtlas {
  ...
  async loadImage(sig, blob) {
    ...
    if (!_HexImageAtlas.#firstPaintMarked) {
      _HexImageAtlas.#firstPaintMarked = true;
      window.__hcBoot?.(`first tile painted to atlas (${loadMs.toFixed(0)}ms)`);
    }
    return uv;
  }
...
async function resolveChildNames(history, _parentSegments, _parentDir, content, parentLayerSig) {
  const out = /* @__PURE__ */ new Set();
  if (!content?.children?.length) return out;
  const store = parentLayerSig ? window.ioc?.get?.("@hypercomb.social/Store") : void 0;
  ...
  if (parentLayerSig && store?.writeChildrenManifest && content.children.length === children.length) {
    ...
    if (manifest.length > 0) {
      ...
      schedule(() => {
        void store.writeChildrenManifest(parentLayerSig, manifest);
      });
    }
  }
  return out;
}
var ShowCellDrone = class _ShowCellDrone extends Drone2 {
  ...
  #ephemeralCellSet = /* @__PURE__ */ new Set();
  ...
  computeSignatureLocation = async (lineage) => {
    const currentSig = lineage?.currentSig;
  ...
  publishLocalCells = async (lineage, mesh, sig, grammar = "") => {
    ...
    let localCells = [];
    if (typeof lineage?.currentLayer === "function" && historyService?.getLayerBySig) {
      try {
        ...
        localCells = resolved.filter((n) => n !== null);
      } catch {
      }
    }
  ...
  renderFromSynchronize = async () => {
    ...
    const locationKey = String(lineage.explorerLabel?.() ?? "/");
    if (!this.#forceNextRender && locationKey === this.renderedLocationKey && this.renderedCellsKey !== "" && !this.#clipboardView) {
      return;
    }
    this.#forceNextRender = false;
    if (!this.#clipboardView && (this.#activeRenderTarget === locationKey || this.streamActive && locationKey === this.renderedLocationKey)) {
      return;
    }
  ...
  #renderFromSynchronizeInner = async (lineage, locationKey, axial) => {
    if (!this.pixiApp || !this.pixiContainer || !this.pixiRenderer) {
      this.clearMesh();
      return;
    }
    ...
    if (locationKey !== this.renderedLocationKey && !this.#clipboardView && !(this.#tagFlattenResults && this.#tagFlattenResults.length > 0)) {
      ...
      if (cached && cached.cells.length > 0 && cachedDir) {
        ...
        for (const cell of cached.cells) {
          ...
          if (this.cellImageCache.has(label)) {
            const sig = this.cellImageCache.get(label) ?? void 0;
            cell.imageSig = sig;
        ...
        void (async () => {
          try {
            const cursorService2 = window.ioc?.get?.("@diamondcoreprocessor.com/HistoryCursorService");
            if (!cursorService2) return;
            const sigLoc = await this.computeSignatureLocation(lineage);
            if (sigLoc.sig) await cursorService2.load(sigLoc.sig);
          } catch {
          }
        })();
        return;
    ...
    if (this.#tagFlattenResults && this.#tagFlattenResults.length > 0) {
      ...
      if (dir) await this.loadCellImages(cells2, dir);
      ...
      this.emitEffect("render:cell-count", this.#buildCellCountPayload(cells2));
      this.rendering = false;
      return;
    }
    const localCells = [];
    ...
    const localCellSet = /* @__PURE__ */ new Set();
    const ephemeralCellSet = /* @__PURE__ */ new Set();
    const peerCellSet = /* @__PURE__ */ new Set();
    let branchSet = /* @__PURE__ */ new Set();
    const historyService = window.ioc?.get?.("@diamondcoreprocessor.com/HistoryService");
    ...
    let layerAllowed = null;
    if (!this.#clipboardView && historyService) {
      const sig = await this.computeSignatureLocation(lineage);
      if (cursorService) {
        const loadTimeout = new Promise((resolve) => setTimeout(resolve, 3e3));
        await Promise.race([cursorService.load(sig.sig).catch(() => {
        }), loadTimeout]);
      }
      if (cursorService) {
        ...
        if (!content && cursorState?.position === 0 && (cursorState?.total ?? 0) > 0) {
          ...
          try {
            const live = await lineage.currentLayer?.();
        ...
        if (content) {
          ...
          await Promise.all(childSigs.map(async (cs) => {
            try {
              ...
              const subChildren = childLayer?.children;
              if (Array.isArray(subChildren) && subChildren.length > 0) {
                const name = childLayer?.name;
    ...
    const peerIndices = /* @__PURE__ */ new Map();
    try {
      const registry = window.ioc?.get?.("@hypercomb.social/TileSourceRegistry");
      if (registry?.resolve) {
        const segs = lineage?.explorerSegments?.() ?? [];
        const entries = await registry.resolve({ segments: segs, dir });
        const mismatched = entries.filter(
          (e) => (e.kind === "ephemeral" || e.kind === "peer") && !localCellSet.has(e.name)
        );
    ...
    if (!this.#clipboardView) {
      ...
      for (const cell of this.#pendingRemoves) {
        const presentInTruth = layerAllowed ? layerAllowed.has(cell) : localCellSet.has(cell);
        if (presentInTruth) {
    ...
    const localHidden = JSON.parse(localStorage.getItem(hideStorageKey(locationKey)) ?? "[]");
    const bareHidden = JSON.parse(localStorage.getItem(`hc:hidden-tiles:${locationKey}`) ?? "[]");
    const hiddenSet = /* @__PURE__ */ new Set([...localHidden, ...bareHidden]);
    try {
      const swarm = window.ioc?.get?.("@diamondcoreprocessor.com/SwarmDrone");
      const swarmHidden = swarm?.hiddenAtCurrentSig?.() ?? /* @__PURE__ */ new Set();
    ...
    if (layerChanged) {
      ...
      const axialMax2 = typeof axial.items.size === "number" ? axial.items.size : cellNames.length;
      const maxCells2 = Math.min(cellNames.length, axialMax2);
      const eagerCells = this.buildCellsFromAxial(axial, cellNames, maxCells2, localCellSet, branchSet);
  ...
  #syncCacheFromVP = (locationKey) => {
    if (!locationKey) return;
    const vp = window.ioc?.get?.("@diamondcoreprocessor.com/ViewportPersistence");
    if (!vp) return;
    const existing = this.#layerViewportCache.get(locationKey) ?? {};
    const lp = vp.lastPan;
    if (lp) existing.pan = { dx: lp.dx, dy: lp.dy };
    const lz = vp.lastZoom;
    if (lz) existing.zoom = lz.fit ? { scale: lz.scale, cx: lz.cx, cy: lz.cy, fit: true } : { scale: lz.scale, cx: lz.cx, cy: lz.cy };
    const lm = vp.lastMeshOffset;
    if (lm) existing.meshOffset = { x: lm.x, y: lm.y };
  ...
  #applyViewportForLayerReadSnapshot = async (dir) => {
    let snap = {};
    try {
  ...
  applyGeometry = async (cells, final = true) => {
    ...
    if (this.hexMesh?.getLocalBounds && this.#pendingRecenter) {
      ...
      if (final) this.#pendingRecenter = false;
    }
  ...
  ensureListeners = () => {
    ...
    this.requestRender();
    this.onEffect("viewport:persisted", ({ dir, snapshot }) => {
    ...
    this.onEffect("tags:filter", ({ active }) => {
      ...
      if (this.filterTags.size > 0) {
        if (!wasFiltering) {
          const lineage2 = this.resolve("lineage");
          this.#preFilterSegments = lineage2?.explorerSegments?.() ? [...lineage2.explorerSegments()] : [];
        }
    ...
    this.onEffect("paired-channel:share-ephemeral", () => {
      this.#layerCellsCache.clear();
      this.renderedCellsKey = "";
      this.requestRender();
    });
    this.onEffect("paired-channel:share-installed", () => {
      this.#layerCellsCache.clear();
      this.renderedCellsKey = "";
      this.requestRender();
    });
    ...
    const invalidateForHide = () => {
      this.#layerCellsCache.clear();
      this.renderedCellsKey = "";
      this.requestRender();
    };
    this.onEffect("tile:hidden", invalidateForHide);
    this.onEffect("tile:unhidden", invalidateForHide);
    this.onEffect("swarm:hide-changed", invalidateForHide);
    this.onEffect("swarm:resource-arrived", ({ sig }) => {
      if (sig) {
    ...
    window.showCellsPoc = {
      ...
      signature: async () => {
        const lineage2 = this.resolve("lineage");
        return await this.computeSignatureLocation(lineage2);
      }
    };
  };
  /**
  ...
  #branchSetCache = /* @__PURE__ */ new WeakMap();
  #branchSetPending = /* @__PURE__ */ new WeakMap();
  #computeBranchSet = async (dir, localCells) => {
    const lineage = this.resolve("lineage");
  ...
  async #orderByIndexPinned(dir, names, localCellSet, readOnly = false, peerIndices) {
    const axial = this.resolve("axial");
    const maxSlot = axial?.count ?? 60;
    const sparse = new Array(maxSlot + 1).fill("");
    let nextFree = 0;
    const unindexed = [];
    const indexNurse = window.ioc?.get?.("@diamondcoreprocessor.com/IndexNurse");
    const lineage = this.resolve("lineage");
    const parentSegments = lineage?.explorerSegments?.() ?? [];
    const peerNames = [];
    for (const name of names) {
      ...
      try {
        let cellDir;
        try {
          cellDir = await dir.getDirectoryHandle(name, { create: false });
        } catch {
        }
        const cacheKey = await cellLocationSig(parentSegments, name);
        const idx = indexNurse ? await indexNurse.read(parentSegments, name, cellDir, cacheKey) : await readTilePropertiesAt(parentSegments, name).then(
          (p) => typeof p["index"] === "number" ? p["index"] : void 0
        );
    ...
    const slotTracker = window.ioc?.get?.("@diamondcoreprocessor.com/CenterSlotTracker");
    const axialAny = window.ioc?.get?.("@diamondcoreprocessor.com/AxialService");
    ...
    for (const name of unindexed) {
      ...
      if (scoreMap && adjacents) {
        ...
        for (let i = 0; i <= maxSlot; i++) {
          ...
          if (s.off < bestOff || s.off === bestOff && whitespace > bestWhitespace || s.off === bestOff && whitespace === bestWhitespace && s.center < bestCenter) {
            bestOff = s.off;
    ...
    try {
      const droneAny = window.ioc?.get?.("@diamondcoreprocessor.com/PairedChannelDrone");
   ...
   */
  async #resolveCellOrder(_mode, dir, union, localCellSet, _lineage, peerIndices) {
    if (this.#clipboardView) {
    ...
    if (isRewound && cursor) {
      ...
      const historyService = window.ioc?.get?.("@diamondcoreprocessor.com/HistoryService");
      const parentSegments = _lineage?.explorerSegments?.() ?? [];
      const orderedNames = content && historyService ? [...await resolveChildNames(historyService, parentSegments, dir, content)] : [];
  ...
  loadCellImages = async (cells, _dir, forceReload) => {
    ...
    const peerImageSigByLabel = /* @__PURE__ */ new Map();
    try {
      const swarm = window.ioc?.get?.("@diamondcoreprocessor.com/SwarmDrone");
    ...
    const loadOne = async (cell) => {
      ...
      try {
        ...
        if (smallSig && isSignature(smallSig)) {
          ...
          const subSvc = window.ioc?.get?.("@diamondcoreprocessor.com/SubstrateService");
