...
var SpacebarPanInput = class {
  ...
  get gate() {
    return window.ioc.get("@diamondcoreprocessor.com/InputGate") ?? null;
  }
  attach = (pan, canvas) => {
...
var PanningDrone = class extends Drone {
  ...
  panBy = (delta) => {
    if (!this.stage) {
      console.log("[pan] panBy: no stage, bail");
      return;
    }
    ...
    let vp = this.vp;
    if (!vp) {
      vp = window.ioc.get("@diamondcoreprocessor.com/ViewportPersistence") ?? null;
      if (vp) this.vp = vp;
    }
    if (this.renderer && vp) {
      const s = this.renderer.screen;
