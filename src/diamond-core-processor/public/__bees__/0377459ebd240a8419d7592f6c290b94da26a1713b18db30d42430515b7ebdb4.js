...
var LayerCommitter = class {
  ...
  get #slotRegistry() {
    return get("@diamondcoreprocessor.com/LayerSlotRegistry");
  }
  constructor() {
    ...
    EffectBus2.on("cell:added", (p) => this.#queueCommit(p?.segments, "add", p?.cell));
    EffectBus2.on("cell:removed", (p) => this.#queueCommit(p?.segments, "remove", p?.cell));
    EffectBus2.on("tile:saved", (p) => this.#queueCommit(p?.segments));
    EffectBus2.on("tile:hidden", (p) => this.#queueCommit(p?.segments));
    EffectBus2.on("tile:unhidden", (p) => this.#queueCommit(p?.segments));
    this.#slotRegistry?.onTrigger((trigger) => {
      EffectBus2.on(trigger, (p) => this.#queueCommit(p?.segments));
    });
  }
