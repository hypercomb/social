// hypercomb-essentials/src/diamondcoreprocessor.com/meeting/meeting.drone.ts
import { Drone } from "@hypercomb/core";

// hypercomb-essentials/src/diamondcoreprocessor.com/meeting/meeting-signaling.ts
...
// hypercomb-essentials/src/diamondcoreprocessor.com/meeting/meeting-peer.ts
var ICE_CONFIG = {
  iceServers: [
...
// hypercomb-essentials/src/diamondcoreprocessor.com/meeting/meeting-audio.ts
...
async function deriveRoomSig(cell) {
...
var HypercombMeetingDrone = class extends Drone {
  ...
  // cell → room
  #meetingCells = /* @__PURE__ */ new Set();
  // cells that have a meeting tag
  #cellTemplates = /* @__PURE__ */ new Map();
  // cell → tag (e.g. 'cascade')
  ...
  heartbeat = async () => {
    ...
    this.onEffect("render:host-ready", () => {
      ...
      this.emitEffect("overlay:register-action", [{
        ...
        hoverTint: 11075544,
        profile: "private",
        visibleWhen: (ctx) => this.#meetingCells.has(ctx.label)
      }]);
    });
  ...
  async #ensureRoomSubscription(cell, tag) {
