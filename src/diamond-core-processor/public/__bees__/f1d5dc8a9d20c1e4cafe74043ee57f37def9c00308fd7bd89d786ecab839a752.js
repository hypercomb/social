...
var ViewportPersistence = class extends EventTarget {
  ...
  setPan = (dx, dy) => {
    if (this.#suspended) {
      console.log("[vp] setPan: suspended, bail");
      return;
    }
    if (!this.#dir) this.#syncWithStore();
    if (!this.#dir) this.#ensureDirFromLineage();
  ...
  #persistTo = async (dir, pending) => {
    try {
      ...
      EffectBus2.emit("viewport:persisted", { dir, snapshot: viewport });
    } catch {
    }
  };
  #persist = async () => {
    const dir = this.#dir;
    if (!dir) {
      console.log("[vp] #persist: NO dir, bail");
      return;
    }
    const pending = { ...this.#pending };
    if (!pending.zoom && !pending.pan && !pending.meshOffset) {
      console.log("[vp] #persist: empty pending");
      return;
    }
    this.#pending = {};
...
var ZoomDrone = class extends Drone {
  ...
  heartbeat = async () => {
    ...
    this.onEffect("render:host-ready", (payload) => {
      ...
      window.ioc.whenReady("@diamondcoreprocessor.com/MousewheelZoomInput", (mouseWheel) => {
        if (!this.canvas) return;
        mouseWheel.attach(
          {
      ...
      window.ioc.whenReady("@diamondcoreprocessor.com/PinchZoomInput", (pinchZoom) => {
        pinchZoom.attach(this, this.minScale);
      });
      window.ioc.whenReady("@diamondcoreprocessor.com/TouchGestureCoordinator", (coordinator) => {
        if (!this.canvas) return;
        const touchPan = window.ioc.get("@diamondcoreprocessor.com/TouchPanInput");
        const pinchZoom = window.ioc.get("@diamondcoreprocessor.com/PinchZoomInput");
        coordinator.attach(
          this.canvas,
          touchPan ?? { panUpdate: () => {
          } },
      ...
      if (this.vp) {
        ...
        this.vp.addEventListener("dir-change", this.#cancelAnim);
      }
    });
  };
  #registerKeybindings() {
    ...
    EffectBus2.emit("keymap:add-layer", { layer });
  }
  ...
  zoomToScale = (scale, pivotClient) => {
    ...
    EffectBus2.emitTransient("viewport:manual", {});
    this.#cancelAnim();
