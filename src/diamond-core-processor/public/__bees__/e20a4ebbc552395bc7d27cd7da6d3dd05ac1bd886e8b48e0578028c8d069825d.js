...
var HistoryService = class _HistoryService {
  ...
  commitLayer = async (locationSig, layer) => {
    ...
    EffectBus.emit("content:wrote", { sig: layerSig, kind: "layer", bytes: bytes.buffer });
    return layerSig;
  };
  /**
...
var LayerCommitter = class {
  ...
  constructor() {
    EffectBus2.on("render:set-orientation", (p) => {
      if (p) {
        this.#layout = { ...this.#layout, orientation: p.flat ? "flat-top" : "point-top" };
        this.#schedule();
      }
    });
    EffectBus2.on("render:set-pivot", (p) => {
      if (p != null) {
        this.#layout = { ...this.#layout, pivot: !!p.pivot };
        this.#schedule();
      }
    });
    EffectBus2.on("overlay:neon-color", (p) => {
      if (p?.name) {
        this.#layout = { ...this.#layout, accent: p.name };
        this.#schedule();
      }
    });
    EffectBus2.on("render:set-gap", (p) => {
      if (p?.gapPx != null) {
        this.#layout = { ...this.#layout, gapPx: p.gapPx };
        this.#schedule();
      }
    });
    EffectBus2.on("render:set-text-only", (p) => {
      if (p?.textOnly != null) {
        this.#layout = { ...this.#layout, textOnly: !!p.textOnly };
        this.#schedule();
      }
    });
    EffectBus2.on("cell:added", (p) => {
      if (p?.fromCascade) return;
      this.#queueChildName(p?.segments, "add", p?.cell);
    });
    EffectBus2.on("cell:removed", (p) => {
      if (p?.fromCascade) return;
      this.#queueChildName(p?.segments, "remove", p?.cell);
    });
    EffectBus2.on("tile:saved", (p) => this.#queueBare(p?.segments));
    EffectBus2.on("tile:hidden", (p) => this.#queueBare(p?.segments));
    EffectBus2.on("tile:unhidden", (p) => this.#queueBare(p?.segments));
    window.ioc.whenReady(
      "@diamondcoreprocessor.com/LayerSlotRegistry",
      (registry) => {
        registry.onTrigger((trigger, slotName) => {
          EffectBus2.on(trigger, (p) => {
  ...
  #queueChildName(segments, op, cell) {
  ...
  #queueSlotSig(segments, slot, op, sig) {
    this.#machine.request({
      segments: this.#cleanSegments(segments),
  ...
  #queueSlotSwap(segments, slot, from, to) {
    this.#machine.request({
      segments: this.#cleanSegments(segments),
  ...
  #queueSlotSet(segments, slot, sigs) {
    this.#machine.request({
      segments: this.#cleanSegments(segments),
  ...
  #cleanSegments(segments) {
    return Array.isArray(segments) ? segments.map((s) => String(s ?? "").trim()).filter(Boolean) : null;
  }
  /**
   ...
   */
  commitSlotSet(segments, slot, sigs) {
    return this.#machine.requestAndWait({
      segments: this.#cleanSegments(segments) ?? [],
      delta: { kind: "set", slot, sigs: sigs.slice() }
    });
  }
  commitSlotAppend(segments, slot, sig) {
    return this.#machine.requestAndWait({
      segments: this.#cleanSegments(segments) ?? [],
      delta: { kind: "sig", slot, op: "append", sig }
    });
  }
  commitSlotRemove(segments, slot, sig) {
    return this.#machine.requestAndWait({
      segments: this.#cleanSegments(segments) ?? [],
      delta: { kind: "sig", slot, op: "removeSig", sig }
    });
  }
  commitSlotSwap(segments, slot, from, to) {
    return this.#machine.requestAndWait({
      segments: this.#cleanSegments(segments) ?? [],
      delta: { kind: "sig-swap", slot, from, to }
    });
   ...
   */
  async update(segments, layer, nameSlots = /* @__PURE__ */ new Set(["children"])) {
    const slots = {};
    ...
    const lineageInst = get("@hypercomb.social/Lineage");
    if (!history || !lineageInst) return "";
    const ancestorName = layer.name ? String(layer.name) : cleaned.length === 0 ? ROOT_NAME : cleaned[cleaned.length - 1];
  ...
  async #commit(req = { segments: null }) {
    ...
    for (let depth = segments.length; depth >= 0; depth--) {
      ...
      const newSig = await history.commitLayer(ancestorLocSig, machine.output());
      if (depth === segments.length && req.delta && (req.delta.kind === "set" || req.delta.kind === "layer" || req.delta.kind === "name")) {
        try {
          const prevChildSigs = Array.isArray(prevLayer?.children) ? prevLayer.children : [];
          const newChildSigs = machine.getSlot("children");
          const prevNames = /* @__PURE__ */ new Set();
          ...
          for (const name of added) {
            EffectBus2.emit("cell:added", { cell: name, segments: sub, fromCascade: true });
          }
          for (const name of removed) {
            EffectBus2.emit("cell:removed", { cell: name, segments: sub, fromCascade: true });
          }
        } catch (err) {
