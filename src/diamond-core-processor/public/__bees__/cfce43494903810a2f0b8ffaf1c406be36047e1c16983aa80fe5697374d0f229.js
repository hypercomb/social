...
__export(tile_properties_exports, {
  ...
  TILE_PROPERTIES_SLOT: () => TILE_PROPERTIES_SLOT,
  cellLocationSig: () => cellLocationSig,
  isSignature: () => isSignature,
  readCellProperties: () => readCellProperties,
  readTilePropertiesAt: () => readTilePropertiesAt,
  resolveResourceSignatures: () => resolveResourceSignatures,
  writeCellProperties: () => writeCellProperties,
  writeTilePropertiesAt: () => writeTilePropertiesAt
});
import { EffectBus } from "@hypercomb/core";
var TILE_PROPERTIES_FILE, TILE_PROPERTIES_SLOT, isSignature, cellLocationSig, readCellProperties, writeCellProperties, HISTORY_KEY, STORE_KEY, COMMITTER_KEY, iocGet, readTilePropertiesAt, writeTilePropertiesAt, resolveResourceSignatures;
var init_tile_properties = __esm({
  "src/diamondcoreprocessor.com/editor/tile-properties.ts"() {
    "use strict";
    TILE_PROPERTIES_FILE = "0000";
    TILE_PROPERTIES_SLOT = "properties";
    ...
    HISTORY_KEY = "@diamondcoreprocessor.com/HistoryService";
    STORE_KEY = "@hypercomb.social/Store";
    COMMITTER_KEY = "@diamondcoreprocessor.com/LayerCommitter";
    iocGet = (key) => {
      const ioc = window.ioc;
      return ioc?.get?.(key);
    };
    readTilePropertiesAt = async (parentSegments, cellName) => {
    ...
    writeTilePropertiesAt = async (parentSegments, cellName, updates) => {
      const history = iocGet(HISTORY_KEY);
      const store = iocGet(STORE_KEY);
...
var PairedChannelDrone = class _PairedChannelDrone extends Drone {
  ...
  #sessionBootSec = Math.floor(Date.now() / 1e3);
  static CHANNEL_EVENT_GRACE_SEC = 60;
  /**
  ...
  async importTransientTree(cellName) {
    ...
    const { readTilePropertiesAt: readTilePropertiesAt2, writeTilePropertiesAt: writeTilePropertiesAt2 } = await Promise.resolve().then(() => (init_tile_properties(), tile_properties_exports));
    let cleared = 0;
    const walk = async (segments, name, layerSig) => {
      try {
        const props = await readTilePropertiesAt2(segments, name).catch(() => ({}));
        if (props["transient"] === true) {
          await writeTilePropertiesAt2(segments, name, { transient: false });
          cleared++;
        }
        if (!history?.getLayerBySig || !layerSig) return;
        const layer = await history.getLayerBySig(layerSig);
        const childSigs = layer && Array.isArray(layer.children) ? layer.children : [];
  ...
  #onChannelEvent(channelId, event) {
    const joined = this.#channels.get(channelId);
    if (!joined) return;
    const minAcceptableSec = this.#sessionBootSec - _PairedChannelDrone.CHANNEL_EVENT_GRACE_SEC;
  ...
  async #onExpose(tileLabel) {
    ...
    try {
      ...
      this.#toast(
        "warning",
        "Expose failed",
        `Couldn't read layer tree for "${tileLabel}": ${err?.message ?? err}`
      );
      return;
    }
...
async function buildSubtreeLayersFromLayerTree(parentSegments, tileName) {
  const iocGet2 = window.ioc?.get;
  const history = iocGet2?.("@diamondcoreprocessor.com/HistoryService");
  if (!history) throw new Error("HistoryService unavailable");
  const descendants = [];
  const visited = /* @__PURE__ */ new Set();
  const visit = async (segments, name) => {
    ...
    const childLayerSigs = Array.isArray(layer?.children) ? layer.children : [];
    const children = [];
    ...
    let rawProperties = {};
    try {
      rawProperties = await readTilePropertiesAt(segments, name);
    } catch {
    }
