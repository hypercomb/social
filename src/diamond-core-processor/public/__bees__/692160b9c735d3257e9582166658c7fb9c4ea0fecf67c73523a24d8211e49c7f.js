...
function parsePairedLayerContent(value) {
  ...
  if (!name) return null;
  const properties = obj.properties && typeof obj.properties === "object" && !Array.isArray(obj.properties) ? obj.properties : null;
  ...
  for (const c of childrenRaw) {
    ...
    if (typeof r.sig !== "string" || !/^[0-9a-f]{64}$/.test(r.sig)) return null;
    children.push({ name: r.name, sig: r.sig });
  }
...
var ExposeDrone = class extends Drone2 {
  ...
  // ── receive path ───────────────────────────────────────────────────
  #offerSync(channelId, share) {
    EffectBus3.emit("toast:show", {
      type: "tip",
      title: `Share available: ${share.branchName}`,
      message: share.byteEstimate ? `${share.tileCount ?? 1} tile \xB7 ~${formatBytes(share.byteEstimate)}` : `${share.tileCount ?? 1} tile available to sync.`,
      duration: 0,
      // sticky — user has to act
      actionLabel: "Sync",
      actionEffect: SHARE_ACCEPT_EFFECT,
      actionPayload: { channelId, share }
    });
  }
