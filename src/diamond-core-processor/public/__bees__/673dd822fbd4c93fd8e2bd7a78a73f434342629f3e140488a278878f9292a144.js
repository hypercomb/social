...
import { Drone, EffectBus as EffectBus2, hypercomb } from "@hypercomb/core";
...
var readTilePropertiesAt = async (parentSegments, cellName) => {
  ...
  const cleanParent = parentSegments.map((s) => String(s ?? "").trim()).filter(Boolean);
  const cleanName = String(cellName ?? "").trim();
  if (!cleanName) return {};
  const cellSig = await history.sign({
    explorerSegments: () => [...cleanParent, cleanName]
  });
  if (!cellSig) return {};
...
var writeTilePropertiesAt = async (parentSegments, cellName, updates) => {
  ...
  const cleanName = String(cellName ?? "").trim();
  if (!cleanName) return;
  const existing = await readTilePropertiesAt(cleanParent, cleanName);
  const merged = { ...existing, ...updates };
  ...
  const cellSegments = [...cleanParent, cleanName];
  await committer.commitSlotSet(cellSegments, TILE_PROPERTIES_SLOT, [propSig]);
  EffectBus.emit("cell:0000-changed", {
    cacheKey: await cellLocationSig(cleanParent, cleanName),
    keys: Object.keys(updates)
  });
};
...
var MoveDrone = class extends Drone {
  ...
  async #commitDropIntoUnsafe(axial, source) {
    ...
    const sourceSegments = lineage?.explorerSegments?.() ?? [];
    ...
    for (const label of movedLabels) {
      EffectBus2.emit("cell:removed", { cell: label });
    }
    this.emitEffect("move:preview", null);
    this.emitEffect("move:drop-into", null);
  ...
  #reorderNames(placements) {
    ...
    const labels = this.#dragLabels.length > 0 ? this.#dragLabels : this.#cellLabels;
    const coords = this.#dragLabels.length > 0 ? this.#dragCoords : this.#cellCoords;
    const names = new Array(Math.max(gridSize, labels.length)).fill("");
    for (let i = 0; i < labels.length; i++) {
