...
var ShowCellDrone = class _ShowCellDrone extends Drone {
  ...
  description = "Renders the hex grid \u2014 maps seeds to cells, manages geometry, and syncs with the Nostr mesh.";
  ...
  renderFromSynchronize = async () => {
    ...
    if (this.#tagFlattenResults && this.#tagFlattenResults.length > 0) {
      ...
      const seedNames2 = flatResults.map((r) => r.label);
      const flatSeedSet = new Set(seedNames2);
      const axial2 = this.resolve("axial");
      if (!axial2) {
        this.rendering = false;
        return;
      }
      const maxCells2 = Math.min(seedNames2.length, typeof axial2.items.size === "number" ? axial2.items.size : seedNames2.length);
      const cells2 = this.buildCellsFromAxial(axial2, seedNames2, maxCells2, flatSeedSet);
      if (cells2.length === 0) {
        this.clearMesh();
        this.rendering = false;
        return;
      ...
      this.cachedSeedNames = seedNames2;
      ...
      this.emitEffect("render:cell-count", { count: cells2.length, labels: seedNames2 });
      this.rendering = false;
      return;
    }
    const localSeeds = await this.listSeedFolders(dir);
  ...
  #emitRenderTags(cells) {
    ...
    for (const cell of cells) {
      const tags2 = this.seedTagsCache.get(cell.label);
      if (tags2) {
    ...
    for (const cell of cells) {
      ...
      try {
        ...
        if (Array.isArray(cellTags)) {
          this.seedTagsCache.set(cell.label, cellTags.filter((t) => typeof t === "string"));
        } else {
          this.seedTagsCache.set(cell.label, []);
        }
