// hypercomb-essentials/src/diamondcoreprocessor.com/substrate/substrate-organizer.drone.ts
...
var SubstrateOrganizerDrone = class {
  ...
  #keydownHandler = null;
  #cardStep = 138;
  // card width + gap, set on build
  #dragSuppressClick = false;
  constructor() {
    EffectBus.on("substrate-organizer:open", () => {
      void this.#open();
    });
    EffectBus.on("substrate-organizer:close", () => {
      this.#close();
    });
    EffectBus.on("substrate:changed", () => {
  ...
  #build() {
    ...
    const mkArrow = (dir) => {
      ...
      btn.addEventListener("mouseenter", () => {
        btn.style.background = "rgba(140, 170, 220, 0.25)";
      });
  ...
  #renderCard(source, isActive) {
    ...
    if (!source.builtin) {
      ...
      removeBtn.addEventListener("mouseenter", () => {
        removeBtn.style.background = "rgba(220, 120, 120, 0.3)";
      });
      removeBtn.addEventListener("mouseleave", () => {
        removeBtn.style.background = "rgba(12, 16, 24, 0.7)";
      });
    ...
    const countSuffix = isActive ? ` \xB7 ${this.#service()?.resolvedImageCount ?? 0}` : "";
