...
import { Drone as Drone2, SignatureService as SignatureService2, I18N_IOC_KEY } from "@hypercomb/core";
...
var ShowCellDrone = class _ShowCellDrone extends Drone2 {
  ...
  computeSignatureLocation = async (lineage) => {
    ...
    const sig = sigStore ? await sigStore.signText(key) : await SignatureService2.sign(new TextEncoder().encode(key).buffer);
  ...
  #listCellFoldersCache = /* @__PURE__ */ new WeakMap();
  #listCellFoldersPending = /* @__PURE__ */ new WeakMap();
  ...
  async #orderByIndexPinned(dir, names, localCellSet, readOnly = false, peerIndices) {
    ...
    for (const name of names) {
      ...
      try {
        ...
        const idx = indexNurse ? await indexNurse.read(cellDir, cacheKey) : await readCellProperties(cellDir).then(
