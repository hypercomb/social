...
import { Drone as Drone2, EffectBus as EffectBus3, consumePointerGesture, I18N_IOC_KEY } from "@hypercomb/core";
...
var TileActionsDrone = class extends Drone {
  ...
  async #removeTile(label) {
    ...
    for (const sig of childSigs) {
      ...
      if (child.name !== label) survivorSigs.push(sig);
    }
    const nextLayer = { ...parent, children: survivorSigs };
    await committer.update(segments, nextLayer, /* @__PURE__ */ new Set());
...
var TileOverlayDrone = class _TileOverlayDrone extends Drone2 {
  ...
  heartbeat = async () => {
    if (!this.#effectsRegistered) {
      ...
      EffectBus3.emit("keymap:add-layer", { layer: editLayer });
  ...
  #hasNotesFor(cellLabel) {
    const notesService = get("@diamondcoreprocessor.com/NotesService");
  ...
  // ── Arrange mode ────────────────────────────────────────────────────
    ...
    EffectBus3.emit("keymap:suppress", { reason: "arrange-mode" });
  ...
  #exitArrangeMode() {
    ...
    EffectBus3.emit("keymap:unsuppress", { reason: "arrange-mode" });
  ...
  #onArrangeKeyDown = (e) => {
    if (e.key === "Escape") {
      e.preventDefault();
      e.stopPropagation();
      EffectBus3.emit("overlay:arrange-mode", { active: false });
    }
  };
