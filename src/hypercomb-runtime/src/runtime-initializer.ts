...
// TOOL WINDOWS ARE THE SHELL'S. This used to be a side-effect import of
// `../ui/tool-windows`, which is the only line that ever tied the runtime to
// hypercomb-shared/ui. Each shell imports it now (web, dev and the shim all
// do), so the escape cascade behaves exactly as before and this package
...
export type RuntimeInitializerOptions = {
   ...
   *
   * Omit it and the shipped catalogs below are used — which is what the web
   * and dev shells do, unchanged. A SHIM passes a resolver instead, because a
   * locale is CONTENT: it lives on a host in a pool of meaning and resolves by
   * signature like everything else, rather than being bundled into the shell
   * or handed out as an installer resource. Fourteen catalogs are 2.9 MB;
   * bundling them puts every language into every host's entry bundle to serve
   * the one language a visitor actually reads.
   */
  catalogs?: (locale: string) => Promise<Record<string, string> | null>
  /** Which locales exist, for the post-paint idle warm. Omit it and only the
   *  active locale plus the 'en' fallback ever load — which is the right
   *  default for a host, where a language is fetched when someone picks it. */
...
const _runInitializeRuntime = async (
  ...
  if (logOpfs) {
    const logger = get('@hypercomb.social/OpfsTreeLogger') as TreeLoggerLike | undefined
    await logger?.log?.()
  }
  ...
  const i18n = get('@hypercomb.social/I18n') as LocalizationLike | undefined
  if (i18n) {
    const loadedLocales = new Set<string>()
    const resolveCatalog = options.catalogs
    const loadLocale = async (locale: string): Promise<void> => {
      if (loadedLocales.has(locale)) return
      // A supplied resolver REPLACES the shipped catalogs rather than
      // supplementing them: two sources for one locale is two answers to
      // "what does this key say", and the shim's whole point is that the
      // answer arrives as verified content.
      if (resolveCatalog) {
        loadedLocales.add(locale)
        try {
          const catalog = await resolveCatalog(locale)
          if (catalog) i18n.registerTranslations('app', locale, catalog)
        } catch { /* unreachable locale — graceful degradation */ }
        return
      }
      // No resolver, no host catalogs. The runtime ships none of its own —
      // a locale is content, and which languages exist is the HOST's answer,
      // not this package's. Shells that still bundle theirs pass
      // `bundledCatalogs` from hypercomb-shared.
    }
    ...
    // Detached idle task for the remaining catalogs — never on the paint path.
    {
      const loadRest = (): void => {
        ...
        // Every locale still loads on demand via 'locale:changed' above.
        // Nothing to sweep: the shell owns the list of locales, and every one
        // still loads on demand via 'locale:changed' above.
        for (const locale of options.locales?.() ?? []) void loadLocale(locale)
      }
  ...
  const navigation = get('@hypercomb.social/Navigation') as NavigationLike | undefined
  navigation?.listen?.()
  ...
  const history = get('@hypercomb.social/BootstrapHistory') as BootstrapHistoryLike | undefined
  await history?.run?.()
