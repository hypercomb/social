...
// src/diamondcoreprocessor.com/keyboard/pivot-toggle.ts
import { EffectBus as EffectBus3 } from "@hypercomb/core";
var pivotOn = localStorage.getItem("hc:hex-pivot") === "true";
EffectBus3.on("keymap:invoke", ({ cmd }) => {
  if (cmd !== "render.togglePivot") return;
  pivotOn = !pivotOn;
  localStorage.setItem("hc:hex-pivot", String(pivotOn));
  EffectBus3.emit("render:set-pivot", { pivot: pivotOn });
});
export {
  KeyMapService,
  defaultKeyMap,
