...
A published door opened onto a black cover for four to six seconds while the
visitor's browser fetched the pack, installed the package and evaluated the
engine ("they seem broken when you get there" — jwize). The fix is to show the
site at HTML-parse time and let the engine hydrate behind it.
...
- **a takeover view on screen** (`body.hc-view-covered`, a website page) → the
  ...
  rewritten to heap URLs exactly as the site view mounts them → `<sig>/landing.html`.
