...
> **STEPS 1, 2, 4 AND 6 BUILT 2026-09-13; nothing deployed.** Key rotation was
> decided first, as the order below requires: **a lost operator key is recovered
> by redeploying the worker's operator config** — the same deploy that already
> puts a zone's routes on the worker.
>
> - **Parity vector** — `hypercomb-relay/blossom-worker/site-bindings.vector.json`,
>   generated by an implementation independent of both sides: the two pool
>   addresses, one zone's SITE_BINDINGS, its canonical record and signature.
>   Essentials asserts its encoder and core's `registerPoolMeaning` against it;
>   the worker asserts its reader.
> - **Steps 1–2** — `sharing/community-hosts.ts`: `visual:binding:artifact`,
>   `normalizeBoundSite` (mirrors the worker's rule), `canonicalJson`,
>   `bindingArtifactRecord/Bytes/Sig`, `bindingRecordsFromSiteBindings` (splits
>   a var by outermost zone, keeping its order), `parseBindingRecord`,
>   `listZoneBindings`, `addZoneBinding`. Binding members never read as hosts.
> - **Step 4** — `worker.js`: `SITE_OPERATORS = {"<zone>": "<pubkey>"}` names
>   whose index speaks for a zone. The index's `binding:<zone>` names the
>   record, whose bytes must hash to it (flat heap, then the `community:hosts`
>   pool at its derived address). A verified record replaces that zone's var
>   entries in var order; anything less leaves SITE_BINDINGS in charge. One
>   `normalizeBinding` serves both sources. Resolution is per request on a view
...
>
> **Owed:** step 3 (the operator writes a record into the pool, uploads it and
> names it from their own index — a participant act, not a bridge op), step 5
> (set `SITE_OPERATORS`, confirm the live directory is unchanged, then drop the
> zone from the var — a worker deploy), and the two-operators rule before a
> zone ever has two.
