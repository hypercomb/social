...
drones self-organize through the effect bus. when `PixiHostDrone` emits `'render:host-ready'`, it is not addressing `ShowHoneycombDrone` by name. it is broadcasting a colony-wide signal: *the rendering infrastructure is available.* any drone that cares about rendering responds. any drone that doesn't, ignores it.
