...
- **Type tag**: the responder serves `t=layer` and `t=resource` (resources re-opened 2026-08-20 — a live peer's pictures must paint for the zone with them, bounded by the 256 KB response cap so the base64 event fits the relay's 512 KB `max_message_length`). `t=dependency` is silently dropped — code stays HTTP-direct only.
