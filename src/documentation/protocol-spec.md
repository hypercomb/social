...
- **Cancel** is published by the asker once a sig has been resolved with verified bytes. Other peers preparing a response for the same sig abort before committing bandwidth. Two things make this converge rather than merely hope to (both 2026-09-19): a participant's relay filter MUST admit kind 20402 alongside 20400/30401 — the swarm's `configureKinds` omitted it, so every cancel was dropped and N holders published N copies; and holders **stand down in tiers** before committing: each ranks itself among the participants it has seen on the channel by mixing `(sig, pubkey)` into an ordering key; rank 0 answers at once, rank 1 after one cancel round trip (350 ms), everyone else after two. The first copy is never delayed, the worst wait (700 ms) sits inside the asker's 2 s mesh window, and only when both lowest-ranked participants are non-holders does the crowd publish together. A quiet swarm (nobody else seen) waits nothing. Measured with `scripts/drive-swarm-scale.cjs` (one browser process per participant).
- **Type tag**: the responder serves `t=layer` and `t=resource` (resources re-opened 2026-08-20 — a live peer's pictures must paint for the zone with them). `t=dependency` is silently dropped — code stays HTTP-direct only.
- **Chunked responses** (2026-09-20): one event carries at most 192 KB of raw bytes (base64 inflates by 4/3 and the event must fit the relay's 512 KB `max_message_length`). Larger bytes, up to 8 MB, travel as an ordered run: chunk *i* of *n* carries `["c", "i/n"]` and its own d-tag `<sig>#i` — each chunk keeps its own replaceable relay slot while the `x` tag (the channel the asker subscribes on) stays the bare sig. The asker assembles a run per responder, keeps its clock alive while chunks arrive (60 s hard bound), and sha256-gates the concatenation exactly like a single-event response. Runs from different responders never mix; a cancel that lands mid-run stops the rest of it. Tile pictures should not need this: the stored small picture is the canonical 346×400 WebP (`hex-capture` `TILE_PICTURE_SIDE`), tens of KB, at every writer — editor, drop, link, substrate, assistant. Chunking is for what predates that and for other resources.
