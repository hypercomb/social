...
- **The participant's words (2026-09-21, `assistant/module.queen.ts`).** `module` (or `module list`) names the drafts picked right now; `module drop <path>` puts the trunk's layer back at that path (reload); `module commit [<path>] [<name>]` is "deploy later" from inside the hive: the runtime (`module-drafts.ts` `commitDraft`) re-mints every layer from the trunk root down to the draft's parent so the new root names the draft's layer, appends that root to this host's `host:packages` pool under `<name>` (default `essentials`, the same member a build's `publish:revision` writes), and re-composes the selection on the new root with the pick gone; then the queen stamps `install:<name>` → the new root on the index host through `setHiveRoot`, signed by this browser. A stamp refusal leaves the commit standing: the package is published, only the pointer followers read has not moved. Only a draft commits — a picked revision already has a publisher and carries their dependencies. Nothing is overwritten: the old root and chain stay held, the pool is append-only.
