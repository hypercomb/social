// hypercomb-shim/public/hypercomb.worker.js
//
// THE FETCHER. The shim's own copy — deliberately DIVERGED from
// hypercomb-web/public/hypercomb.worker.js, which stays as it is for the live
// deploy. This one gained the module network fallback (see
// handleModuleRequest) that makes install cache-warming rather than a
// precondition. Do not "resync" the two: the shim is the survivor and the web
// copy is frozen until it retires.
...
async function handleModuleRequest(request, dirNames) {
  ...
  // NETWORK FALLBACK — install becomes CACHE-WARMING, not a precondition.
  // A module the heap does not hold is fetched from the origin (then the
  // known hosts), sha256-verified against the requested signature, written
  // into the pool, and served. What changes is not safety but SEQUENCING: a
  // node no longer has to complete an install before it can run anything, and
  // a heap with a hole in it repairs that hole the moment something asks for
  // it rather than failing the import.
  //
  // The verification below is what makes the fallback admissible at all. It
  // is the SAME admission boundary replication uses — bytes whose hash is not
  // their name are dropped, never cached, never executed — so a forged host,
  // a poisoned CDN or an SPA fallback page can only ever cost a 404.
  const fetched = await fetchModuleFromNetwork(sig)
  if (fetched) {
    await writeModuleToOpfs(dirNames[0], sig, fetched)
    const headers = new Headers()
    headers.set('content-type', 'application/javascript; charset=utf-8')
    headers.set('cache-control', 'no-store')
    const response = new Response(fetched, { status: 200, headers })
...
// Own origin first (a host serves its own package), then the known hosts the
// page told us about. Flat `<origin>/<sig>` is the canonical address; the
// `/content/` form is the shell-origin layout. Every candidate is verified.
async function fetchModuleFromNetwork(sig) {
  const candidates = [`/${sig}`, `/content/${sig}`]
  ...
  const fromHost = await fetchResourceFromHosts(sig)
  return fromHost ? fromHost.buf : null
}

// Write-through into the module POOL (not the flat root — modules are pool
// members, named `<sig>.js`). Skip if present: re-writing a content-addressed
// file invalidates any Blob already handed out for that sig.
async function writeModuleToOpfs(dirName, sig, buffer) {
  try {
    const root = await self.navigator.storage.getDirectory()
    const dir = await root.getDirectoryHandle(dirName, { create: true })
    try { await dir.getFileHandle(`${sig}.js`); return } catch { /* absent — create */ }
    const handle = await dir.getFileHandle(`${sig}.js`, { create: true })
    const writable = await handle.createWritable()
    try { await writable.write(buffer) } finally { await writable.close() }
  } catch { /* best-effort cache fill; the bytes are already served */ }
}
