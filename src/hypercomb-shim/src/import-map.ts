// hypercomb-shim/src/import-map.ts
//
// SHORT-LIVED BY DESIGN. A copy of hypercomb-web/src/setup/resolve-import-map,
// taken so the shim builds with no reference to hypercomb-web. Do not grow it:
// Phase 4 of documentation/everything-is-a-beehavior.md retires the import map
// outright — once bee bundles import their dependencies by `/<sig>` URL rather
// than by alias, this whole file, the index.html localStorage replay and the
// one-time reload all delete together. Until then the shim carries a copy
