...
const COURSES: Course[] = [
  ...
  },
  {
    ...
    note: 'The powerful surface: build a shape in one line, name a branch, see the same tiles as something else, meet the assistant and the swarm. Nothing outside the practice page is touched, and nothing is ever published.',
    lessons: [
      ['bracket-paths', 'A bracket item carrying a slash builds DEPTH — one line, a whole shape.', ['creation', 'structure']],
      ['hive', 'Any branch can be named, then taken by that name instead of a path.', ['structure']],
      ['tree', 'The sideways tree — trunk on the left, one column per ring.', ['view']],
      ['views', 'Websites, slides, study games: the tiles stay the tiles, the view is a behaviour you attach.', ['view']],
      ['assistant', 'Ask from the command line; a bee flies the work and answers land as notes.', ['assistant']],
      ['share', 'Your hive is private until you say otherwise — sharing is always deliberate.', ['swarm']],
    ],
  },
]
...
const PARTS: Part[] = [
  ...
  [`${T}/lessons/expert.lessons.ts`, 'the expert course — paths, hives, views, assistant, swarm'],
