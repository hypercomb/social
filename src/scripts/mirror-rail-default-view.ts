...
const DEFAULT_VIEW_PARTS: Part[] = [
  ...
  ['hypercomb-shared/ui/command-shell/command-shell.component.ts',
   'the three gestures — a modifier press emits `viewDefault` and latches, so the mouseup that follows does not ALSO toggle the view; the long press keeps the disable it always had'],
  ['hypercomb-shared/ui/command-shell/command-shell.component.html',
   'the button — one rail button per available view, `.is-default` for the standing mark, and a title that says "opens here by default" in the participant\'s language'],
  ['hypercomb-shared/ui/command-shell/command-shell.component.scss',
