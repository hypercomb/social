...
const CATEGORIES: Category[] = [
  ...
  },
  {
    ...
    members: [
      ...
      ['atomize', 'Go deeper — break each LEAF tile into the pieces that compose it, asked of Claude Haiku over the bridge (foreach the selection, else every leaf on the layer; does nothing to a tile that already has children; alias /expand)', `${E}/assistant/atomize.drone.ts`],
      ['organize', 'Insert a level — Haiku plans how to group a crowded layer over the bridge and the hive re-homes the tiles into the groups (the inverse of atomize; mints no new meaning)', `${E}/assistant/organize.drone.ts`],
