// Install site branch — /install
//
//   node scripts/bridge/_install-site-build.cjs [--dry]
//
// One ROOT-LEVEL cell carrying the self-hosting install guide as a POST-IT:
// `visual:postit:note` holds the one-pager (htmlSig), `view:default` opens the
// branch AS the page, the tile shares the existing sticky-note art (SHARE
// RESOURCES: same sig as /revolucion/meetup — one record, two refs), and the
// notes explain what the page is for. Top-level on purpose: the pluginthematrix
// worker's implicit-site rule maps a first-level subdomain to a TOP-LEVEL
// lineage, so publishing this branch makes install.pluginthematrix.com its
// website with zero worker changes.
//
...
const ASSETS = path.join(__dirname, '_install-site')
const SEGMENTS = ['dylan']
const KIND = 'visual:postit:note'
const MARKER = 'Without getting stuck'
// The sticky-note tile art already minted for /revolucion/meetup — referenced,
// never copied (N uses = N refs to ONE record).
...
// Root child SNAPSHOT sigs are historical and can be absent (census trap), so
// the fingerprint is a bag-resolved path only the authoring hive has.
...
// ── primitives ────────────────────────────────────────────────────────
// The `add` op is the by-name DELTA append (cell:added → committer queues one
// APPEND op; every prior child preserved verbatim). NEVER `update` root
// membership — that is a SET which wipes what it does not name.
...
async function wearSharedArt(segments) {
  ...
  // A tile that already wears art keeps it — the postit takeover replaces the
  // render anyway, and existing art is the tile's own identity.
...
// ── notes content ─────────────────────────────────────────────────────
const INSTRUCTION = 'The self-hosting install guide as a post-it: open it for the one-pager. Publishing this branch serves it at dylan.pluginthematrix.com — the page is made by the flow it describes.'

const NOTE = [
  'Install guide — run your own corner of the internet',
  '',
  'This cell carries the complete self-hosting guide: install the Hypercomb app (jwize.com/installer), put a domain on Cloudflare with one wildcard record, ride a tunnel to your own machine, deploy the content worker once, then publish — the branch name IS the subdomain.',
  '',
  'The post-it page ends with the traps that cost real time (X-Reason header first, Node 24 npx EINVAL, Git Bash slash-mangling, one-tab rule, unconfirmed-is-patience, wildcard covers one label) so the next person never gets stuck on them.',
  '',
  'The branch is deliberately top-level: the worker maps a first-level subdomain to a top-level lineage, so /dylan going public is the whole deployment — no DNS row, no worker change, no config.',
].join('\n')
...
async function main() {
  ...
  // The post-it page
  const html = fs.readFileSync(path.join(ASSETS, 'install-page.html'), 'utf8')
  let htmlSig = '(dry)'
  if (!DRY) {
    ...
    const deco = await ask({
      ...
      payload: { version: 1, title: 'Install', htmlSig },
      mark: 'persistent',
      replaceKind: true,
    })
    ...
    // The branch OPENS AS the page — same mark meetup wears.
  ...
  // replaceKind is removeSig-then-append, so a read in the gap sees the
  // decoration missing — poll until the commit settles.
  if (!DRY) {
    ...
    const rev = await ask({ op: 'build-record', segments: SEGMENTS, label: 'dylan install guide' })
