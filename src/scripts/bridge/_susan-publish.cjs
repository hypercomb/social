// Publish /susan to susan.pluginthematrix.com — drives the SANCTIONED panel
...
//
//   node scripts/bridge/_susan-publish.cjs
//
// Prereq: the `/susan` path is present in localStorage `hc:public-branches`
...
async function main() {
  ...
  // 2. Wait for the refresh to surface the susan row.
  let row = null
  for (let i = 0; i < 30; i++) {
    await sleep(2000)
    row = await renderRow()
