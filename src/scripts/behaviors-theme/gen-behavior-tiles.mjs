...
function lattice(color, seed) {
  ...
  for (let r = -1; r < 8; r++) for (let q = -1; q < 8; q++) {
    ...
    rows.push(`<polygon points="${hexPts(cx, cy, s, 30)}" fill="none" stroke="${color}" stroke-opacity="0.05" stroke-width="1.5"/>`)
  }
  return rows.join('')
}
...
function card(cell) {
  ...
  const rot = tier === 'part' ? 0 : (seed % 2 ? 0 : 30)
...
async function main() {
  ...
  for (const cell of cells) {
    ...
    await el.screenshot({ path: `tiles/${slug}.png` })
  ...
  await writeFile('tiles/manifest.json', JSON.stringify(manifest, null, 1))
  await browser.close()
