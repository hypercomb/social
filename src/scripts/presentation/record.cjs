...
//
// Read one and you can have the rest without reading them: voice.cjs clones
// from a single take and seeds these same slots. See voice/README.md.
//
// Mastering is the shared broadcast chain in master.cjs — the same one the
// cloned lines go through, so a read take and a spoken one sit at one level.
const fs = require('fs')
const path = require('path')
const crypto = require('crypto')
const { LUFS, master, durationOf } = require('./master.cjs')
