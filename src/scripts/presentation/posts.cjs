...
//   node scripts/presentation/posts.cjs --only <id>   # render one post, copy for all
...
A new way to be — so here is how to bee.`,
  ...
  {
    id: 'protocol', scene: 6,
    title: 'Hypercomb is a protocol',
    copy: `Strip the interface away and what is left of Hypercomb is not an app — it is a protocol, and a deliberately small one.

Independent implementations meet at exactly three places: signatures (the SHA-256 of the bytes, nothing else), the storage layout (flat signature-named entries, lineage bags, pools of meaning), and the mesh events that carry work between hives. Everything else — renderer, framework, UI — is implementation-local by design.

And the contract is executable, not aspirational: sixty-nine conformance vectors, generated by running the live implementation, never written by hand. Changing them is changing the protocol — and a second, native implementation already reads and writes the same hive by passing the same vectors.

That is the real answer to lock-in. An app can hold your work hostage. A protocol cannot.`,
    beats: [
      { kind: 'still', body: `<div class="eyebrow">under the app</div><h1>Hypercomb is a <b>protocol</b>.</h1>
          <p class="sub">The app you can see is only the first implementation of it.</p>`,
        say: `Hypercomb looks like an app. It isn't — it's a protocol, and the app is just the first implementation.` },
      { kind: 'still', body: `<div class="eyebrow">the whole agreement</div><h1>Implementations meet at <b>three</b> places.</h1>
          <div class="hexrow">
            <div class="hexb"><span class="g">#</span><span class="t">signatures</span></div>
            <div class="hexb"><span class="g">▤</span><span class="t">storage layout</span></div>
            <div class="hexb"><span class="g">⇄</span><span class="t">mesh events</span></div>
          </div>`,
        say: `Two implementations meet at exactly three places: how content is named, how it rests on disk, and how it travels between hives. Everything else is free.` },
      { kind: 'still', body: `<div class="eyebrow">proof, not prose</div><h1>The contract is <b>executable</b>.</h1>
          <p class="sub">Sixty-nine conformance vectors, generated by running the live code — never written by hand. Pass them, and your software opens the hive.</p>
          <div class="golink" style="margin-top:1.5vh">www.hypercomb.com</div>`,
        say: `The contract isn't prose you trust — it's the test vectors you run, generated from the living code. Any software that passes them opens your hive. That's what makes it yours.` },
    ],
  },
]
