...
// --- env.js generation ---
// Writes a stub env.js so index.html's unconditional <script src="env.js"> tag
// doesn't 404 in the console. This script deliberately never embeds any API
// key: an earlier version read ANTHROPIC_API_KEY from the environment and
// baked it into shipped builds, which leaked the key to every site visitor.
// To use an API key in local dev, set it manually in DevTools:
...
function generateEnvJs(): void {
  ...
  for (const app of ['hypercomb-web', 'hypercomb-dev', 'diamond-core-processor']) {
    ...
    if (existsSync(pubDir)) {
      ...
      console.log(`${TAG} wrote ${app}/public/env.js (stub)`)
    }
  }
}
