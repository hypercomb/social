// hypercomb-core/src/core/panels/docked-panel.ts
//
// THE DOCKED PANEL, as a primitive. Framework-free by construction: it lives in
// core because a module may only import from core, and because the harness
// (hypercomb-shim) cannot load Angular at all — field decorators throw in JIT
// and its build guard fails the bundle. So the chrome that makes a tool window
// a tool window had to stop being an Angular directive before a behaviour could
// ever own one.
//
// Attach it to any element:
//
//   const panel = attachDockedPanel(aside, {
//     id: 'files-viewer', dockSide: 'right',
//     minWidth: 280, maxWidth: 680, defaultWidth: 340,
//     onClose: () => this.close(),
//   })
//   panel.update({ pairWhen: true })   // an option changed
//   panel.dispose()                    // the element is going away
//
// The Angular shell reaches it through `hcDockedPanel`
// (hypercomb-shared/ui/docked-panel/hc-docked-panel.directive.ts), now a thin
// adapter that owns no behaviour: same inputs, same template usage, delegating
// to this class. Nothing in the 50-odd panels changed when this moved.
//
// docked panel — the shared "docked side panel" chrome: drag-to-resize +
...
// file. The original reason is spent — this module carried @angular/core, and
// tool-windows.ts is imported by runtime-initializer, so an import in that
// direction dragged Angular into every boot including the shim's, where the
// field decorators threw at module evaluation. There is no Angular here now,
// but the inversion STAYS: both files are in the same package and tool-windows
// is the one imported early, so importing back would be a plain cycle.
// Registering at module scope keeps the ordering safe either way — this module
// is the only thing that can open a popover, so nothing can be open before this
// line has run. See tool-windows.ts:setPopoverDismisser.
...
/** Everything a window declares about its own chrome. Every field is optional
 *  and carries the same default the directive's `@Input` did, so an adapter can
 *  hand over exactly what its template was given and nothing more. */
export interface DockedPanelOptions {
  id?: string
  dockSide?: 'left' | 'right'
  minWidth?: number
  maxWidth?: number
  ...
  minScale?: number
  maxScale?: number
  defaultText?: number | null
  ownsSize?: boolean
  sizeOwner?: PanelSizeOwner | null
  launcherControlId?: string
  ownSettings?: (() => SettingRow[]) | null
  pairWindow?: string
  pairOpenEffect?: string
  pairCloseEffect?: string
  pairWhen?: boolean | null
  pairLabel?: string
...
export class DockedPanel implements GroupMember, LaneMember {

  /** Options are plain fields, so an adapter assigns them and every existing
   *  `GroupMember` / `LaneMember` read (`this.id`, `this.dockSide`, …) still
   *  finds what it always found. */
  constructor(el: HTMLElement, options: DockedPanelOptions = {}) {
    this.#el = el
    assign(this, options)
  }
  ...
  sizeOwner: PanelSizeOwner | null = null
  ...
  ownSettings: (() => SettingRow[]) | null = null
  ...
  hcSession: WindowSession | null = null
  ...
  set dockExclusive(value: boolean) {
   ...
   *  to do: showing again remounts the element, and `init()` claims a place in
  ...
  evictFromLane(): void {
    if (!this.hcSession) { this.onClose?.(); return }
  ...
  /** The element is mounted and the options are set. Angular called this from
   *  `ngOnInit`; `attachDockedPanel` calls it for everyone else. */
  init(): void {
    live.add(this)
    members.add(this)
  ...
  /** The element is going away. Every listener, observer, lane place and
   *  session hold taken in `init` is given back here. */
  dispose(): void {
    ...
    // AND TAKE THE INJECTED CHROME BACK OUT. As a directive this could be
    // skipped: Angular only destroys the directive when it is destroying the
    // host element, so the grip and the gear went with it either way. A
    // primitive has no such guarantee — `dispose()` is called on an element
    // that may well stay on screen (a window leaving the dock for a float, a
    // behaviour swapping its chrome), and every one of those left a dead grip
    // and a dead gear behind, stacking another pair on the next attach.
    this.#grip?.remove()
    this.#grip = null
    this.#gearBtn?.remove()
    this.#gearBtn = null
    this.#line = null
    // The width and the scale were ours to set, so they are ours to give back;
    // the window's own stylesheet decides again from here.
    this.#el.style.removeProperty('--hc-panel-scale')
    this.#clearLanePlacement()
  }
  ...
  update(next: Partial<DockedPanelOptions>): void {
...
/** Copy declared options onto the panel. Anything `undefined` is left alone, so
 *  a partial update never wipes a field back to its default — the trap an
 *  `Object.assign` here would set for every caller that sends one key. */
function assign(panel: DockedPanel, options: Partial<DockedPanelOptions>): void {
  for (const [key, value] of Object.entries(options)) {
    if (value !== undefined) (panel as unknown as Record<string, unknown>)[key] = value
  }
}
